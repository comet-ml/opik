...
export const test = baseTest.extend<AnnotationQueueAutomationFixtures>({
  ...
  /**
   * Teardown for queues a test creates itself — the create round-trip is the
   * behaviour under test, so a seed fixture cannot own those ids. Registered
   * the moment the id exists (before the request, since the id is
   * caller-chosen), drained here whatever the test's outcome.
   */
