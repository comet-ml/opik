 ...
 * teardown leaves every rule behind. The run-prefix sweep in
 * `global-teardown.ts` does not know about them either — it only walks
 * experiments, datasets and projects. Any spec that creates online-evaluation
 * rules therefore has to delete them, and doing it in-test is fragile: a failing
 * assertion skips a trailing cleanup step, and moving that step into a `finally`
 * block invites two subtler bugs (see below).
 *
