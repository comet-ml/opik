 ...
 *
 * API-level throughout, and deliberately so even though the automation UI now
 * exists: what these two tests pin is the storage round-trip and the delete
 * cascade, neither of which any page renders. The UI half of the same feature
 * is covered by its siblings in this directory — the create form and its
 * provenance column in `annotation-queue-automation-provenance.spec.ts`, the
 * edit form in `annotation-queue-automation-edit.spec.ts`.
 *
 ...
 *
 * `max_items_in_queue` is absent from everything below. It IS in the build at
 * the top of this stack, unlike when these assertions were first written, so
 * the ceiling is now testable and simply untested — a gap, not a limitation.
 */
