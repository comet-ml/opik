 ...
 *
 * This file pins the routing ENGINE — which traces a set of conditions does and
 * does not collect. Its sibling
 * `annotation-queue-automation-provenance.spec.ts` covers the other half, the
 * journey of configuring automation through the create form and reading each
 * item's origin off the Source column; between them the feature is covered from
 * both ends.
 *
 * The queues here are seeded over REST rather than built through that form, and
 * that is a choice rather than a limitation now the form exists: each test needs
 * a different condition shape (a single threshold, an AND pair), and driving the
 * form three times to reach the same three configs would spend the budget on a
 * path the provenance spec already walks. The discriminating assertion in every
 * test below is queue MEMBERSHIP — and for the negative cases it is an absence,
 * which an empty items table is weak evidence for, since an empty table is also
 * what a queue that failed to load shows.
 */
    ...
    test(
      ...
      async ({ project, backendClient, createRoutingQueue, testNamespace, page }) => {
        const queue = await createRoutingQueue({
          ...
          automation: {
            enabled: true,
            groups: [{ conditions: [{ score: QUALITY_SCORE, operator: '<', value: THRESHOLD }] }],
          },
        });
        ...
        await test.step('Verify the reviewer sees exactly that one item', async () => {
          ...
          // The row is there AND the page says automation put it there. Presence
          // alone would read the same for an item a reviewer had added by hand,
          // which is the one other way a trace reaches this table.
          await expect(queuePage.itemSourceCell(belowThreshold)).toHaveText('Automated');
