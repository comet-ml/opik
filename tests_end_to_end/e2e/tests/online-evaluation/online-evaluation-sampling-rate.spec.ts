  ...
  test('A 50% rule scores roughly half of a 30-trace batch while a 100% rule scores all of it', { tag: ['@cap:online-evaluation.sampling-rate'] }, async ({
    ...
    const scoredByRule = await test.step(
      ...
      async () => {
        ...
        // other's progress; waitForTraceScoresSettled waits for each trace's
        // score set to go quiet.
        //
        // minScores: 0 is required here and is the one place this test differs
        // from the enable/disable spec. At 50%, some traces are legitimately
        // expected to end with ZERO scores from the partial rule and — since
        // the control writes under its own name — a trace the control scored
        // still has 1. But a trace could in principle be skipped by chance and
        // still needs to settle, so demanding >=1 score would turn a correct
        // skip into a timeout. The control's per-trace assertion below is what
        // keeps this from being vacuous: if the engine were simply dead, the
        // control would have scored nothing and that assertion fails loudly.
        const settled = await Promise.all(
          seededTraces.map((t) =>
            backendClient
              .waitForTraceScoresSettled(t.id, {
