...
/**
 * OPIK-3268 — a Playground run against a test suite can name the experiments it
 * creates, instead of every run landing under a generated name like
 * `nosy_hamster_5229`.
 *
 ...
 */
test.describe(
  'Test Suites — experiment naming from a suite run',
  ...
  () => {
    ...
    test(
      'a name typed into the Playground reaches the suite run\'s execute request, suffixed',
      { tag: ['@cap:test-suites.run-suite-playground'] },
      async ({
        project,
        testSuite,
        ...
        const runName = `${testNamespace}-suite`;
        const expectedName = `${runName}_a`;
        ...
        page.on('request', (request) => {
          ...
          executed.push(
            (request.postDataJSON() ?? {}) as { prompts?: Array<{ experiment_name?: string }> },
          );
        });
        ...
        await test.step('Name the run and re-run', async () => {
        ...
        await test.step('The request carried the composed name on that variant', async () => {
