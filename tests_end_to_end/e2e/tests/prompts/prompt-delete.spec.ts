...
test.describe('Prompt library — delete', { tag: ['@t2-cuj', '@area:prompts'] }, () => {
  test(
    ...
    async ({ textPrompt, bystanderPrompt, project, backendClient, page }) => {
      ...
      await test.step('Both prompts are listed', async () => {
        ...
        await expect(prompts.promptRow(bystanderPrompt.name)).toBeVisible();
      });
      ...
      await test.step('Server-side: the target is gone, the bystander remains', async () => {
        ...
        expect(await backendClient.promptExistsByName(bystanderPrompt.name, project.id)).toBe(true);
      });
    },
  );
});
