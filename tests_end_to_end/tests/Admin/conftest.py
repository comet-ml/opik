...
def temp_user_with_api_key(page: Page, browser_context, env_config: EnvConfig):
    ...
    yield {"username": username, "password": password, "email": email, "api_key": api_key}
    ...
    if response.status != 200 and response.status != 204:
        print(f"Warning: Failed to delete user {username}. Status: {response.status}, Response: {response.text()}")

def send_request_to_create_user(page: Page, base_url, username, password, env_config):
    """Create a new user with the given username and password"""
    response = page.request.post(f"{base_url}/api/auth/new", data={
        "userName": f"{username}",
        "email": f"{username}@test.com",
    ...
    if response.status != 200 and response.status != 201:
        raise Exception(f"Failed to create user. Status: {response.status}, Response: {response.text()}")
...
def get_user_info(page, base_url, env_config):
    ...
    if response.status != 200:
        raise Exception(f"Failed to get user info. Status: {response.status}, Response: {response.text()}")
...
def api_login_request(page, base_url, username, password, env_config):
    ...
    if response.status != 200:
        raise Exception(f"Login failed. Status: {response.status}, Response: {response.text()}")
