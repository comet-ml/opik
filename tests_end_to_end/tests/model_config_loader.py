...
class ModelConfigLoader:
    ...
    def get_enabled_models_for_playground(self) -> List[Tuple[str, ModelConfig, ProviderConfig]]:
        ...
        for provider_name, provider_config in providers.items():
            if not self._provider_has_api_keys(provider_config):
                if self.config.get('test_config', {}).get('skip_missing_api_keys', True):
                    logger.info(f"Skipping {provider_name} - missing API keys")
                    continue
            
            for model in provider_config.models:
                if (model.enabled and 
                    model.test_playground and 
                    self.config.get('test_config', {}).get('only_test_enabled', True)):
                    models.append((provider_name, model, provider_config))
        
        return models
    
    def get_enabled_models_for_online_scoring(self) -> List[Tuple[str, ModelConfig, ProviderConfig]]:
    ...
    def get_test_prompt(self) -> str:
        ...
        return self.get_test_config().get('test_prompt', 'Explain what is an LLM in one paragraph.')
