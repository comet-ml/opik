...
from datasets_utils import TEST_ITEMS, TEST_ITEMS_UPDATE, compare_item_lists, wait_for_dataset_to_be_visible, get_updated_items, insert_dataset_items_sdk, insert_dataset_items_ui, delete_one_dataset_item_sdk, delete_one_dataset_item_ui, wait_for_number_of_items_in_dataset
import opik
import time
...
class TestDatasetItemsCrud:
    ...
    def test_dataset_item_insertion(self, request, page: Page, client: opik.Opik, dataset_creation_fixture, dataset_insert):
        ...
        dataset = wait_for_dataset_to_be_visible(client=client, dataset_name=request.getfixturevalue(dataset_creation_fixture), timeout=10)
    ...
    @pytest.mark.browser_context_args(permissions=['clipboard-read'])
    def test_dataset_item_update(self, request, page: Page, client: opik.Opik, create_delete_dataset_sdk, insert_dataset_items_sdk):
        """
    ...
    def test_dataset_item_deletion(self, request, page: Page, client: opik.Opik, create_delete_dataset_sdk, insert_dataset_items_sdk, item_deletion):
        """
        ...
        deleted_item_not_in_sdk_list = not any(
            item['input'] == item_deleted['input'] and item['output'] == item_deleted['output']
    ...
    @pytest.mark.browser_context_args(permissions=['clipboard-read'])
    def test_dataset_clear(self, request, page: Page, client: opik.Opik, create_delete_dataset_sdk, insert_dataset_items_sdk):
        """
        ...
        expect(dataset_items_page.page.get_by_text('There are no dataset items yet')).to_be_visible()