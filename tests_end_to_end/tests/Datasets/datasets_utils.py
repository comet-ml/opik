...
def wait_for_number_of_items_in_dataset(expected_items_number: int, dataset, timeout=10):
...
def compare_item_lists(expected: list[dict], actual: list[dict]):
    ...
    set_actual = {frozenset((key, val) for key, val in d.items() if 'id' not in key.lower()) for d in actual}
