...
![Version: 2.2.30](https://img.shields.io/badge/Version-2.2.30-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 2.2.30](https://img.shields.io/badge/AppVersion-2.2.30-informational?style=flat-square)
...
| clickhouse.freeformSql | object | `{"enabled":false,"image":"","resources":{},"tables":["spans","traces","authored_feedback_scores"]}` | Agent Insights read-only free-form SQL user. When enabled, a post-install/post-upgrade Job creates the user, its settings profile, SELECT grants and per-workspace/project RESTRICTIVE row policies -- the Kubernetes equivalent of the provision_agent_insights_readonly_user.sh step docker-compose runs. Requires ANALYTICS_DB_READ_ONLY_FREEFORM_SQL_PASS in the backend env (component.backend.envFrom). |
| clickhouse.freeformSql.image | string | `""` | Image for the grants Job; needs clickhouse-client. Defaults to clickhouse.image. |
| clickhouse.freeformSql.resources | object | `{}` | Resource requests/limits for the grants Job. |
| clickhouse.freeformSql.tables | list | `["spans","traces","authored_feedback_scores"]` | Tables the free-form SQL user may read. Each gets a SELECT grant plus a workspace+project RESTRICTIVE row policy. |
