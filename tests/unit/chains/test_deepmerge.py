...
from comet_llm.chains import deepmerge
...
def test_deepmerge(dict1, dict2, result):
    assert deepmerge.deepmerge(dict1, dict2) == result
...
def test_deepmerge__recursion_limit_exceeded__next_dict_is_treated_as_non_dict_value():
    ...
    assert deepmerge.deepmerge(dict1, dict2, max_depth=2) == {
        "level1": {
            "level2": {
                "level3":{
                    "second-dict-level4": "value"
