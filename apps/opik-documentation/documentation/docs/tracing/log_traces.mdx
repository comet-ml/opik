...
You can log traces to the Opik plaform using either the REST API or the `opik` Python SDK.

## Using the Python SDK

To log traces to the Opik platform using the Python SDK, you will first need to install the SDK:
...
Once the SDK is installed, you can log traces to using one our Opik's integration, function decorators or manually.
...
:::tip
If the `track` function decorators are used in conjunction with the `track_openai` or `OpikTracer` callbacks, the LLM calls will be automatically logged to the corresponding trace.
:::
...
If you wish to log traces and spans manually, you can use the `Opik` client:
