...
Opik includes an integration with many popular Agent frameworks ([LangGrah](/docs/tracing/integrations/langgraph), [LLamaIndex](/docs/tracing/integrations/llama_index))
...
You can track the agent execution by using either one of [Opik's integrations](/docs/tracing/integrations/overview) or the `@track` decorator:

<Tabs>
    <Tab value="LangGraph" title="LangGraph">
    You can log the agent execution by using the [OpikTracer](/docs/tracing/integrations/langgraph) callback:
    ...
    To log a Haystack pipeline run, you can use the [`OpikConnector`](/docs/tracing/integrations/haystack). This connector will log the pipeline run to the Opik platform and add a `tracer` key to the pipeline run response with the trace ID:
    ...
    Opik has a built-in integration with [LLamaIndex](/docs/tracing/integrations/llama_index) that makes it easy to track the agent execution:
...
If you are using out [LangGraph](/docs/tracing/integrations/langgraph) integration, you can also track the agent definition by passing in the `graph` argument to the `OpikTracer` callback:
