...
- Released [online evaluation](/docs/opik/production/rules) rules - You can now define LLM as a Judge metrics that will automatically score all, or a subset, of your production traces.

![Online evaluation](/static/img/changelog/2025-01-13/online_evaluation.gif)

**SDK**:

- New integration with [CrewAI](/docs/opik/tracing/integrations/crewai)
...
- Released a new plugin for the [Kong AI Gateway](/docs/opik/production/gateway)
...
- New integration with [aisuite](/docs/opik/tracing/integrations/aisuite)
- New integration with [Haystack](/docs/opik/tracing/integrations/haystack)
...
- Added a new `evaluate_experiment` method in the SDK that can be used to re-score an existing experiment, learn more in the [Update experiments](/docs/opik/evaluation/update_existing_experiment) guide.
...
- Introduced the `Prompt` object in the SDK to manage prompts stored in the library. See the [Prompt Management](/docs/opik/prompt_engineering/managing_prompts_in_code) guide for more details.
- Introduced a `Opik.search_spans` method to search for spans in a project. See the [Search spans](/docs/opik/tracing/export_data#exporting-spans) guide for more details.
- Released a new integration with [AWS Bedrock](/docs/opik/tracing/integrations/bedrock) for using Opik with Bedrock models.
...
- Released new evaluation metric: [GEval](/docs/opik/evaluation/metrics/g_eval) - This LLM as a Judge metric is task agnostic and can be used to evaluate any LLM call based on your own custom evaluation criteria.
- Allow users to specify the path to the Opik configuration file using the `OPIK_CONFIG_PATH` environment variable, read more about it in the [Python SDK Configuration guide](/docs/opik/tracing/sdk_configuration#using-a-configuration-file).
...
- The `context` field is now optional in the [Hallucination](/docs/opik/tracing/integrations/overview) metric.
- LLM as a Judge metrics now support customizing the LLM provider by specifying the `model` parameter. See more in the [Customizing LLM as a Judge metrics](/docs/opik/evaluation/metrics/overview#customizing-llm-as-a-judge-metrics) section.
