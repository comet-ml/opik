...
<Frame>
  <img src="/static/img/home/traces_page_for_quickstart.png" />
</Frame>

## Set up

Getting started is as simple as creating an [account on Comet](https://www.comet.com/signup?from=llm) or [self-hosting the platform](/docs/opik/self-host/overview).
...
<Tip>If you are self-hosting the platform, simply set the `use_local` flag to True in the `opik.configure` method.</Tip>

</Tab>
<Tab value="Command Line" title="cli">

```bash
opik configure
```

<Tip>If you are self-hosting the platform, simply use the `opik configure --use_local` command.</Tip>
...
can use one of our [integrations](/docs/opik/tracing/integrations/overview):
...
<Tab value="LiteLLM" title="LiteLLM">
...
<Tab value="Any other provider" title="Any other provider">
...
as described in the [Traces / Logging Additional Data section](/docs/opik/tracing/log_traces#logging-additional-data).
...
like [LangChain](/docs/opik/tracing/integrations/langchain), [LangGraph](/docs/opik/tracing/integrations/langgraph) or [LLamaIndex](/docs/opik/tracing/integrations/llama_index),
...
You LlamaIndex calls from that point forward will be logged to Opik. You can learn more about the LlamaIndex integration in the [LLamaIndex integration docs](/docs/opik/tracing/integrations/llama_index).
...
<Info>
  ...
  logged data, see the [Log Traces](/docs/opik/tracing/log_traces) guide.
</Info>
...
1. [Opik's evaluation metrics](/docs/opik/evaluation/metrics/overview): Opik provides a suite of evaluation metrics (Hallucination, Answer Relevance, Context Recall, etc.) that you can use to score your LLM responses.
2. [Opik Experiments](/docs/opik/evaluation/concepts): Opik allows you to automated the evaluation process of your LLM application so that you no longer need to manually review every LLM response.
