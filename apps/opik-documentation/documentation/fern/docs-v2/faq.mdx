...
### Can my coding agent use Opik?

Yes. Claude Code, Cursor, VS Code Copilot, Codex, opencode and any other MCP client
can read your traces, score outputs and run evaluations through the
[Opik MCP server](/mcp-server). One command sets it up and needs no SDK:
