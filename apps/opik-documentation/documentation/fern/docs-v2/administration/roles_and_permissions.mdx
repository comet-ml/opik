...
<Note>
  **View unanonymized data** (shown as **Original data view** in the custom role
  editor) only has an effect when [data anonymization](/administration/data_anonymization)
...
For an example, see how to create a role that sees [anonymized data](/administration/data_anonymization#give-users-a-role-without-the-permission).
