...
<Note>
  **View unanonymized data** (shown as **Original data view** under **Opik Traces**
  in the custom role editor) only has an effect when [data anonymization](/administration/data_anonymization)
  is enabled. Users without it see sensitive values replaced by tokens such as
  `[EMAIL]`. Data anonymization is in preview; contact your Comet account team
  to enable it.
</Note>
...
3. Select a **Base role** (Write, Annotate, or Read) as a starting point, then add or exclude permissions.

For a worked example, see [Set up data anonymization](/administration/data_anonymization#set-up-data-anonymization).
