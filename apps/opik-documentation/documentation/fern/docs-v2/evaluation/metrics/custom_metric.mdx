...
Custom metrics are not limited to offline evaluation. Once a custom metric scores your production traces, you can also **alert** on it: Opik's feedback score alerts threshold on a score *name*, and any score logged to a project is eligible, whether Opik computed it or you did.

You can either run your metric as a custom [online evaluation rule](/production/online-evaluation/rules), or keep the metric code in your own pipeline and push the score to the trace with `client.log_traces_feedback_scores(...)`. Either way, the score name becomes selectable when you configure a feedback score alert. See [Alerting on custom metrics](/production/alerts/alerts#alerting-on-custom-metrics).
