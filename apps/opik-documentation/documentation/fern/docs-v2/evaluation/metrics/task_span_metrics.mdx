...
Task span metrics are evaluation metrics that include a `task_span` parameter in their `score` method. The Opik evaluation engine automatically detects that. These metrics are scored when an evaluation runs the task that produces the span; re-scoring an existing experiment with `evaluate_experiment()` runs no task, so a metric that requires `task_span` there raises `ScoreMethodMissingArguments` and aborts the run, while a scorer whose `task_span` parameter has a default still runs. To score a metric that requires the span, run the task with `evaluate()`; when re-scoring, leave that metric out of the call.
