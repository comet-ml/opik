...
#### Built-in classification scorers

For classification tasks - routing, labeling, filtering - Opik ships precision, recall and F1 as ready-made experiment score functions. They compare a predicted label in the task output against a reference label on the dataset item, across the whole experiment.
...
evaluation = evaluate(
    ...
    experiment_name="Router classification",
)
```

</CodeBlocks>

`output_key` is read from the task output, `reference_key` from the dataset item content. `average` accepts:

| `average` | Behavior |
| --- | --- |
| `"macro"` | Unweighted mean across classes. The default. |
| `"weighted"` | Mean across classes, weighted by how often each class appears in the reference labels. |
| `"micro"` | Pools counts across all classes first. For single-label classification this equals accuracy. |
| `"binary"` | Scores a single class. Requires `positive_label`. |
...
Each scorer returns one `ScoreResult`. The default name is `<metric>_<average>`, for example `f1_score_macro`, so several averages of the same metric can coexist in one experiment. `binary` appends the positive label, so the example above is named `f1_score_binary_spam`. Pass `name` to set the name yourself, which you need when two scorers in one experiment would otherwise share a default name. Rows missing either key are skipped and the count is reported in the score's `reason`.

<Tip>
Structurally invalid configuration - a blank key or `name`, an unknown `average`, a missing or unhashable `positive_label` - raises as soon as you build the scorer, so a misconfigured experiment fails before it runs instead of finishing without the score. A key that is well formed but simply wrong still builds; every row is then skipped and the scorer returns a failed score naming the key, rather than silently producing nothing.
</Tip>
