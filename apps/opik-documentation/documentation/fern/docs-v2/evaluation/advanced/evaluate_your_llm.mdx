...
<Tip>
Invalid configuration raises as soon as you build the scorer rather than during evaluation. Experiment score functions that raise mid-run are only logged as warnings, so failing early stops a mistyped key from silently producing no score.
</Tip>
