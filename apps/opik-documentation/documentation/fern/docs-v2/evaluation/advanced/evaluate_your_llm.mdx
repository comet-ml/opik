...
Each scorer returns one `ScoreResult` named `<metric>_<average>`, for example `f1_score_macro`, so several averages of the same metric can coexist in one experiment. Rows missing either key are skipped and the count is reported in the score's `reason`.
