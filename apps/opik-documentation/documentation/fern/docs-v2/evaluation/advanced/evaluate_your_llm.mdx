...
<Tip>
Structurally invalid configuration - a blank key or `name`, an unknown `average`, a missing or unhashable `positive_label` - raises as soon as you build the scorer, because experiment score functions that raise mid-run are only logged as warnings. A key that is well formed but simply wrong still builds; every row is then skipped and the scorer returns a failed score naming the key, rather than silently producing nothing.
</Tip>
