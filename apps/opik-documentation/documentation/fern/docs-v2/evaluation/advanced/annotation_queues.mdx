...
- All conditions must hold for an item to be added — they are combined with **AND**
- An item with no value for a score in the condition never matches it
- Tick **Cap automatically added items at** to bound how many items automation adds; once that many
  automated items are in the queue, automation stops adding until some are removed. Only items added by the
  automation count — adding by hand is unaffected. Leave it unticked for no limit
