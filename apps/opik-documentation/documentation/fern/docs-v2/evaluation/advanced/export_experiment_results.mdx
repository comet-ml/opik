...
dataset, or one experiment at a time.

## Comparing several experiments in one file

The comparison page prefixes each experiment's columns with its name. To do the same, export each experiment and
join on `dataset_item_id`, which is the same row across experiments run on one dataset.
...
for name in ["baseline", "candidate"]:
    ...
    for item in experiment.get_items(max_results=100_000):
        ...
        row.update({f"dataset.{key}": value for key, value in (item.dataset_item_data or {}).items()})
