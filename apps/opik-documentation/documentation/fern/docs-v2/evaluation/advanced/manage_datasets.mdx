...
when individual items are large enough that memory matters — both directions keep up to
`2 * num_threads` payloads in flight, chunks on a read and request bodies on a write.
...
<Warning>
  On a failed upload, the worker count decides what was already written. With `num_threads=1`
  nothing after the failed batch is sent. In parallel, batches already in flight are left to
  finish and only those not yet started are dropped, so batches *after* the failed one may still
  have landed. Neither case rolls back what already succeeded. Use `num_threads=1` when you need a
  failure to stop at a predictable point.
</Warning>
