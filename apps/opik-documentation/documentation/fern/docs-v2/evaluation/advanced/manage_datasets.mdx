...
#### Inserting more items than fit in memory

`insert` accepts any iterable, so a dataset larger than your available memory does not have to be built as a
list first. Hand it a generator and items are converted and uploaded in batches as they are read, holding only
the batches currently in flight:
...
dataset.insert(read_items("items.jsonl"), deduplication=False)
```

The iterable is consumed exactly once. If reading it fails partway through, the batches already uploaded stay in
the dataset — the same behaviour as a batch that fails to upload. Note that `deduplication=True` keeps a content
hash per inserted item in memory, so pass `deduplication=False` when the point is to bound memory.
