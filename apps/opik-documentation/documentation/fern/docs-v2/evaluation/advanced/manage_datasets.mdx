...
| Dataset write | `dataset.insert()` (and `insert_from_json`, `insert_from_pandas`, `read_jsonl_from_file`, `update`) | 8 |
| Experiment write | `experiment.batch_upload_items()` | 8 |

Each call takes a `num_threads` argument if you do want to change it:
...
<Note>
  ...
  rather than speed anything up. Thread count never changes the result of an operation, only how fast
  it arrives.
</Note>
