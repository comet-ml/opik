...
<Note>
<Note>
Because `coldTierMigration.enabled` defaults to `true`, upgrading a release that **already** has `clickhouse.tieredStorage.enabled: true` activates this job on the next upgrade without any action on your part. It is a no-op when every replica already carries the cache volume, but it does add a post-upgrade hook to that release — set `coldTierMigration.enabled: false` to opt out.
</Note>
