...
| `clickhouse.tieredStorage.coldTierMigration.pollTimeoutSeconds` | `1200` | Total budget for the run (all replicas: recreate + reschedule + wait Ready). |
...
| `clickhouse.tieredStorage.coldTierMigration.serviceAccountName` | `""` | Use an externally-managed ServiceAccount instead of the chart-created one. When set, the account must already have `get/list/delete` on `statefulsets` and `pods`. |

<Warning>
The migration ServiceAccount is granted `delete` on StatefulSets and pods. Leave `serviceAccountName` empty to use the chart-created, Job-scoped account. If you supply your own, use a dedicated, tightly-scoped ServiceAccount — a shared one would let other workloads delete those resources.
</Warning>

## Single-Replica Clusters

On a single-replica cluster the cold-tier enablement is briefly disruptive and would self-heal on its own in roughly 6–7 minutes. The migration job still runs and recovers the replica the same way (recreate the StatefulSet, reschedule the pod), typically faster than waiting for the passive self-heal. No action is required either way. If the replica already has the cache volume, the job is a no-op.
