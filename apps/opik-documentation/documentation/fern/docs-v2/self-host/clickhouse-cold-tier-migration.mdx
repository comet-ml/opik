...
Because the job only restarts the operator, it adds no downtime beyond the wedge itself — no ClickHouse StatefulSet, pod, or PersistentVolumeClaim is deleted.
