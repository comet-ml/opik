...
<Callout intent="info">
  On spans the check matters even more than the symptom suggests, because spans have no delete endpoint of their own. Every span delete arrives from somewhere else — the cascade behind a trace delete, or a data-retention sweep — so a spans mismatch that is not caught at readiness shows up as a trace delete returning 500, or as a background retention job that silently reclaims nothing.
</Callout>
...
Start from the health check message — it names both the setting and the engine it actually found. The readiness endpoint does not carry it (`/health-check` returns the verdict only), so read it from the admin connector, which listens on `SERVER_ADMIN_PORT` (8081 by default) and is not published by the Helm chart or Docker Compose:

```bash
# Kubernetes
kubectl port-forward deploy/<release-name>-backend 8081:8081
