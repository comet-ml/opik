...
If the repair itself fails — most often because the Docker daemon will not run the helper container — repair the volume manually:
...
<Steps>
  ...
  <Step title="Re-own the volume">
    ```bash
    docker run --rm -v opik-opik_minio-data:/data alpine chown -R 1001:1001 /data
    ```

    Substitute your own volume name if it differs — `docker volume ls | grep minio` will show it. The Compose project name prefixes the volume, so it is `opik-opik_minio-data` in a default installation.
  </Step>
  ...
  <Step title="Verify MinIO is running">
    ...
    MinIO reports `status=running` and the stack becomes healthy. Attachments stored before the upgrade remain accessible in the Opik UI.
  </Step>
</Steps>
