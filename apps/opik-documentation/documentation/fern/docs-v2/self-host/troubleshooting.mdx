...
```
[INFO] MinIO could not write to its data volume: the volume is owned by uid 0,
       but the MinIO image runs as 1001:1001. ...
...
If the repair itself fails — most often because the Docker daemon will not run the helper container — the command is printed so you can run it yourself:
...
<Warning>
Do not delete the volume to get past this error. Removing `minio-data` discards every attachment stored in it, and `./opik.sh --clean` goes further still — it removes **all** Opik data volumes, including ClickHouse and MySQL. The chown above resolves the problem without data loss.
</Warning>

#### Verification

MinIO reports `status=running` and the stack becomes healthy:

```bash
docker ps --filter name=minio --format '{{.Names}}\t{{.Status}}'
```

Attachments stored before the upgrade remain accessible in the Opik UI.
