...
<ParamField path="max_reflection_calls" type="int" optional={true} default="max_trials">Budget for reflection-LLM calls — the calls GEPA makes to propose new candidates, which are not metric calls and are billed on top of them. Defaults to `max_trials`; set to `0` to remove the cap. A run stopped by this cap reports `finish_reason="reflection_budget"`; a run that spent its full metric budget as well reports `max_trials`, since the trials are the reason it ended.</ParamField>
