...
Cost tracking is supported for the following LLM providers (pass the name in parentheses as the span `provider`; the most common ones are also available in the `opik.LLMProvider` enum):
...
- **TypeSafe AI** (`typesafe`) - Jev System One models hosted by TypeSafe AI (https://typesafe.ai)
