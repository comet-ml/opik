...
The remote service creates a span with the OpenTelemetry SDK as usual and then calls the Opik bridging helper with the incoming HTTP headers. The helper sets the `opik.trace_id` / `opik.parent_span_id` attributes on the span; once the span is exported to the Opik OTLP endpoint it is linked to the original Opik trace and parent span.
...
def generate_response(request: Request) -> str:
    with tracer.start_as_current_span("server-span") as span:
        ...
        # and sets the corresponding OTel span attributes.
...
import { attachToParent } from "opik";
...
const server = http.createServer((req, res) => {
  ...
  // and sets the corresponding OTel span attributes.
  attachToParent(span, req.headers);
  // ... handle the request, set additional span attributes ...
  span.end();
  res.end("ok");
});
