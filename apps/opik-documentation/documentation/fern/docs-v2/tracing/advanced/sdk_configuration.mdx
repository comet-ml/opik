...
For a headless setup — a Dockerfile, CI, a dotfiles script — use `opik mcp configure` instead. `opik configure` still needs a terminal for its own deployment and workspace questions:

```bash
export OPIK_API_KEY=... OPIK_WORKSPACE=...
opik mcp configure --host claude-code --skills
```
