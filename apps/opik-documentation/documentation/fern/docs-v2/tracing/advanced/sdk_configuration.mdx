...
The one-command path is `uvx opik mcp configure`, which needs no SDK; see
[Opik's MCP server](/mcp-server). If you are already running `opik configure`, it
offers the same step at the end: registering the MCP server with the AI clients it
finds on your machine, so your assistant can read traces and log scores directly
from the chat.
...
<Note>
**These flags are what make the step work without a terminal**, which is how a
coding agent asked to "set Opik up" runs it:
...
To set this up without re-running the whole configuration, use `uvx opik mcp configure`. See [Opik's MCP server](/mcp-server) for the `--ai-client` flag, the skill pack, and per-client instructions.
...
| Analytics Enable    | `OPIK_ANALYTICS_ENABLE`      | N/A                     | Send SDK usage analytics (Python only)         |
...
The Python SDK reports usage analytics that tell us which SDK features are being used,
so we know what to invest in. Each API you call is reported once per run, by name only,
alongside the same environment details the SDK already reports with error reports: the
SDK and Python versions, the OS, whether you are in Jupyter, Colab or CI, the kind of
installation (cloud, self-hosted or local), and the versions of the LLM libraries
installed next to Opik.

Usage is attributed to your **workspace name** — the same identifier used for error
reporting, so that an error and the usage around it describe the same user. Installations
...
```bash
export OPIK_ANALYTICS_ENABLE=false
```
...
| analytics_enable           | `OPIK_ANALYTICS_ENABLE`      | Send usage analytics about which SDK features are used - Defaults to `true`. See [Usage analytics](#usage-analytics) |
