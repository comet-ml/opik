...
<Note>
`--yes` on its own deliberately does **not** register the MCP server: it edits configuration files belonging to other tools, so it needs to be asked for explicitly. Combine the two (`opik configure --yes --install-mcp`) for a fully unattended setup, or use `--no-install-mcp` to skip the prompt entirely.
</Note>

To register the server without re-running the whole configuration, use `opik mcp configure` — see [Opik's MCP server](/mcp-server) for the `--host` flag and per-host instructions.
