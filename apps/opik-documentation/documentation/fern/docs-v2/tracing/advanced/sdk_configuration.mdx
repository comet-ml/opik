...
At the end of setup, `opik configure` offers to register [Opik's MCP server](/mcp-server) with the AI clients it finds on your machine, so your assistant can read traces and log scores directly from the chat.
...
To set this up without re-running the whole configuration, use `opik mcp configure` — see [Opik's MCP server](/mcp-server) for the `--ai-client` flag, the skill pack, and per-client instructions.
