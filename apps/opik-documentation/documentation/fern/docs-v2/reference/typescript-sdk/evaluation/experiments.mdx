...
<Note>
  In Opik 2.0, experiments are project-scoped. Make sure to specify a `projectName` when running experiments so they are associated with the correct project.
</Note>
