...
For asynchronous graph execution using `ainvoke()`, the same applies. `OpikTracer` sets `run_inline = True`, so LangChain runs its callbacks in the calling task and node code inherits the tracing context, exactly as under `invoke()`. `@track`-decorated functions nest under the node's span, and `opik_context.get_current_span_data()` returns that span.
...
async def my_async_node(state):
    current_trace_data = opik_context.get_current_trace_data()
    current_span_data = opik_context.get_current_span_data()  # will return the span for `my_async_node`, created by OpikTracer
...
# Asynchronous execution - tracked functions work automatically
result = await app.ainvoke({"value": 21})
```

<Accordion title="Explicit trace context propagation (optional)">
SDK versions before the fix for [#3175](https://github.com/comet-ml/opik/issues/3175) left `run_inline` at its default of `False`. LangChain then ran the tracer callbacks in a copied context under async execution, and node code could not see the trace. Node code had to pass the trace context on explicitly, with the `extract_current_langgraph_span_data` helper and distributed headers. The helper still works and is kept for compatibility. On an SDK from before the fix `OpikTracer.run_inline` is `False`; on a fixed one it is `True`.
...
@track
def process_data(value: int) -> int:
    """Custom tracked function, attached to the trace tree via distributed trace headers."""
    return value * 2

async def my_async_node(state, config):
    # Extract current span data from LangGraph config; None when OpikTracer is not configured
    span_data = extract_current_langgraph_span_data(config)
    headers = span_data.get_distributed_trace_headers() if span_data is not None else None

    # Pass distributed trace headers explicitly to the tracked function
    result = process_data(
        state["value"],
        opik_distributed_trace_headers=headers,  # the track decorator adds this parameter to every decorated function
    )
    return {"value": result}
...
# Asynchronous execution with explicit trace context propagation
result = await app.ainvoke({"value": 21})
```

The same with the `opik.start_as_current_span` context manager instead of the `@track` decorator:
