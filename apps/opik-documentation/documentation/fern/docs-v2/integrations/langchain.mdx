...
If a single chain or graph mixes providers, pass a callable that receives the LangChain run dict and returns the provider for that specific run. Return `None` to fall back to the auto-detected provider:

```python
def resolve_provider(run):
    metadata = run["outputs"]["generations"][-1][-1]["message"]["kwargs"]["response_metadata"]
