...
The `track_typesafe` integration automatically logs token usage and estimated cost for each traced call, based on the model and provider. TypeSafe AI bills input tokens only, so the estimated cost reflects the input tokens of each `system_one` call.
