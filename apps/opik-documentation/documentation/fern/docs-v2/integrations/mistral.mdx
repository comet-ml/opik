...
First, ensure you have both `opik` and `mistralai` packages installed:

```bash
pip install opik mistralai
```
...
`chat.parse()` (and its async / streaming variants) is tracked too. The parsed
structured output is captured on the span:
