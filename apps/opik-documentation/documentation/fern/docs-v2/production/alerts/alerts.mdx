...
Set up an alert when you want to be notified about an important, actionable event in your Opik workspace — a spike in trace errors, a quality score dropping, a prompt changing under you in production.

Setting one up is two decisions:

1. **When do you want to be alerted?** Pick one or more [event types](#supported-event-types) and set their filter conditions — the threshold, the comparison, the time window, and which projects to watch.
2. **Who needs to be notified, and how?** Point the alert at a destination:
   - **Slack** — native integration, messages formatted for Slack
   - **PagerDuty** — native integration, events formatted for PagerDuty's Events API v2
   - **General** — any webhook endpoint, no-code automation platform, or middleware service

Opik delivers each notification as an HTTP POST carrying the event data in the body.

<Frame>
  <img src="/img/production/alerts_configuration.png" alt="The Alerts list for a project in Opik" />
</Frame>
...
- A project to scope the alert to
...
1. **Navigate to Alerts**
   - Open your project and go to **Alerts**, under Production in the sidebar
   - Click **Create alert**

2. **Add triggers**
   - Click "Add trigger" and tick one or more event types
   - Configure project scope for observability events (optional)
   - For threshold-based triggers (errors, cost, latency, feedback scores):
     - **Threshold**: Set the threshold value that triggers the alert
     - **Operator**: Choose comparison operator (`>`, `<`) for feedback score alerts
     - **Window**: Pick the time window the metric is aggregated over
     - **Feedback Score Name**: Select which feedback score to monitor (for feedback score alerts only). Any score logged to the project can be selected, including [custom metrics](/evaluation/metrics/custom_metric) you define yourself.
   - Expand **Example payload** on a trigger to see the JSON it will send, and click **Test trigger** to send that payload to your endpoint
...
3. **Configure webhook settings**
   ...
   - Click **Test connection**, next to the Endpoint URL field, to send a sample webhook and check the response status
...
5. **Name the alert**
   - **Name**: the field sits at the top of the form, but you can leave it until last — Opik suggests a name from your first trigger as soon as you add one, for example `Trace errors > 5 in 5 mins`, or `Trace feedback score: policy_compliance < 0.8`. Additional triggers are summarised as `+N more`.
   - If the project already has an alert with that name, the suggestion is numbered — `Trace errors > 5 in 5 mins (2)` — so two alerts built from the same trigger stay distinguishable
   - Type your own name at any point to override the suggestion; clear the field to go back to the suggested one

6. **Create the alert**
   - Click "Create alert" to save your configuration
   - New alerts are enabled and start monitoring events immediately
   - To pause an alert later, open it and switch off **Enable alert** at the bottom of the form
...
2. **In Opik**:
   ...
   - Adjust the suggested alert name if you want a different one
   - Click "Create alert"
...
Opik supports ten types of alert events. Select one to jump to its full description.

| Event | Event type | Triggered when |
| --- | --- | --- |
| [Trace errors threshold exceeded](#trace-errors-threshold-exceeded) | `trace:errors` | Trace error count passes your threshold in a time window |
| [Trace feedback score threshold exceeded](#trace-feedback-score-threshold-exceeded) | `trace:feedback_score` | Average trace feedback score crosses your threshold |
| [Thread feedback score threshold exceeded](#thread-feedback-score-threshold-exceeded) | `trace_thread:feedback_score` | Average thread feedback score crosses your threshold |
| [Guardrails triggered](#guardrails-triggered) | `trace:guardrails_triggered` | A guardrail check fails on a trace |
| [Cost threshold exceeded](#cost-threshold-exceeded) | `trace:cost` | Total trace cost passes your threshold in a time window |
| [Latency threshold exceeded](#latency-threshold-exceeded) | `trace:latency` | Average trace latency passes your threshold in a time window |
| [New prompt added](#new-prompt-added) | `prompt:created` | A prompt is created in the prompt library |
| [New prompt version created](#new-prompt-version-created) | `prompt:committed` | A new version is committed to a prompt |
| [Prompt deleted](#prompt-deleted) | `prompt:deleted` | A prompt is removed from the prompt library |
| [Experiment finished](#experiment-finished) | `experiment:finished` | An experiment completes |
...
Both feedback score events threshold on a score **name**, and any score logged to your project is eligible — so custom metrics can be alerted on just like Opik's built-in ones. Pick the trigger above, then select your metric's name in **Feedback Score Name**.

The score has to be on a production trace, span or thread for an alert to see it. Scores that exist only inside an experiment cannot be thresholded. For how to get a custom metric scoring production data, see [Using your custom metric in production](/evaluation/metrics/custom_metric#using-your-custom-metric-in-production).

<Note>
  A score name appears in the **Feedback Score Name** dropdown only after at least one score with that name has been logged to the project.
</Note>
...
**Test thoroughly:**
- Use **Test connection** to check the endpoint, and **Test trigger** to check the payload a given event will send, before you save the alert
