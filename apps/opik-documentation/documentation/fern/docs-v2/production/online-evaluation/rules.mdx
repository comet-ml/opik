...
Because they are stored as ordinary feedback scores, the scores produced by your own custom rules can also drive [alerts](/production/alerts/alerts#alerting-on-custom-metrics) to Slack, PagerDuty or a webhook when the average crosses a threshold.

## Online thread evaluation rules

It is also possible to define LLM as a Judge and Custom Python metrics that run on threads. This is useful to score the entire conversations and not just the individual traces.
