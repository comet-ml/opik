...
subtitle: Install, configure, and run the backend that powers the model-based guards
headline: Guardrails Server | Opik Documentation
og:title: Guardrails Server - Opik
og:description: How to install, configure, start, and call the Opik guardrails backend that runs the PII, Topic, prompt injection, and custom classifier models.
...
- **Custom models** — models [fine-tuned on your own labeled examples](/guardrails/custom-guardrails). Training runs on this server, and it loads the results by name from its adapters directory.

The server is optional and disabled by default. You need it to run the built-in Topic, PII, or Prompt injection guardrails, or any custom model. The [LLM judge guardrail](/guardrails/guardrails#llm-judge) does not use this server: it calls the LLM provider configured in your workspace.
...
| `OPIK_GUARDRAILS_ADAPTERS_DIR` | `/adapters` | Directory the server loads [custom models](/guardrails/custom-guardrails) from. Must be the same directory training writes to. |
...
<AccordionGroup>
  <Accordion title="Request and response reference">
    **Request fields:**
...
<CardGroup cols={2}>
  <Card title="Guards" href="/guardrails/guardrails" icon="fa-regular fa-shield-check">
    The five guard types and how to configure each one.
  </Card>
