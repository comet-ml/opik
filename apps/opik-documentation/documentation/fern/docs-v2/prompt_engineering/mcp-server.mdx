...
Don't see your client in the list it prints? On Opik Cloud, any MCP client can take
the hosted server directly:
...
`npx skills add comet-ml/opik-skills`. On a self-hosted deployment, replace the URL
with your own API base plus `/v1/mcp`, or use the local server described in
[Manual setup](#local-server-uvx).
...
One click installs the Opik Cloud hosted server and opens a browser sign-in. It
does not install the skill pack; run the command above for that.
...
Credentials come from `~/.opik.config` (written by an earlier `uvx opik configure`) or
