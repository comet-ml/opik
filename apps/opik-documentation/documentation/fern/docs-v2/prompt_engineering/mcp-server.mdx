...
Or let your coding agent do it. Paste this into the agent you use, and it installs
what is missing, registers the server for itself, and shows you the proof:

```text title="Installer prompt for your coding agent"
Set up the Opik MCP server and the Opik skills for yourself, so you can read my
traces and experiments and fix code from them. Reference: https://www.comet.com/docs/opik/mcp-server

1. Detect, don't assume: my OS and shell, whether `uvx` is on PATH, and which
   coding agent you are running in.
2. If `uvx` is missing, install uv into my user directory
   (https://docs.astral.sh/uv/getting-started/installation/) and reload PATH.
   Ask me before anything that needs sudo or edits my shell rc files.
3. Run `uvx opik configure status`. If it reports no configuration, stop and ask
   me to set OPIK_API_KEY and OPIK_WORKSPACE in my environment (or to run
   `uvx opik configure` once; for a local Opik, `uvx opik configure --use-local --yes`
   needs no key), then continue. Never ask me to paste the key in chat, never
   print it, never write it into this repo.
4. Run `uvx opik mcp configure --skills --ai-client <the client you are running in>`.
   `uvx opik mcp configure --help` lists the accepted names. This picks the right
   server for my deployment and installs the skills.
5. Prove it: run `uvx opik mcp status` and paste the output. If anything failed
   or is out of sync, fix it and re-run until it is in sync.
6. Tell me to start a new session so you pick up the server and skills, and to
   test with "List my Opik projects".

Do not change anything else on my machine or in this repo.
```
