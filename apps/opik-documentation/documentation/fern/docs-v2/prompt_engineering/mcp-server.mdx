...
<Note>
Setup runs **only in an interactive terminal**. It writes into your AI
client's own configuration, so it is skipped in CI, a Docker build, or a cron
job rather than editing those files unattended. For an image or a script, use
[Manual setup](#manual-setup) to write the config block yourself.
</Note>
...
<Steps>
  ...
  <Step title="Restart your AI client">
    ...
    <Note>
    If your client isn't detected, name it with `--ai-client claude-code` (and see
...
### Clients the CLI doesn't cover

`opik mcp configure` handles Claude Code, Cursor, VS Code Copilot, Codex and
opencode. For any other client, install the skill pack with the community
[`skills`](https://github.com/vercel-labs/skills) CLI, which knows the skill
directories for 76+ agents:

```bash
npx skills add comet-ml/opik-skills
```

It needs Node.js, and it can install per-project rather than for your whole user
account — neither of which the Opik CLI does. Opik installs the pack only as part
of `opik configure` and `opik mcp configure`, so you never have to reach for a
separate command in the normal flow; `npx skills` covers everything outside it.

The two write to different places: Opik writes the shared `~/.agents/skills`
(plus a link for Claude Code), while `npx skills` writes each client's own
directory and defaults to project scope inside a project. To remove a pack Opik
installed, delete it from `~/.agents/skills`.
