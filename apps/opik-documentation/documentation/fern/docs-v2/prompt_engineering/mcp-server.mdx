...
data. These are the legal values. Everything here works on both Opik Cloud and
self-hosted; only the separate `run_experiment` tool is Cloud-only.
...
- **Prefer read-only work when you are only investigating.** The `opik-diagnose` and
  `opik-explain` skills never write. Review any write the agent proposes before you
  approve it.
