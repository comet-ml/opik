...
The local server runs on demand via `uvx opik-mcp@latest` (requires
...
<AccordionGroup>
  ...
  <Accordion title="The first tool call takes a long time, or the client says the server failed to start">
    With the local server the client runs `uvx opik-mcp@latest`, which downloads
    the package and a Python runtime on first use. Setup pre-warms that cache, but
    gives up after a timeout.

    - Let the first call finish once. Later calls start in about a second.
    - If it fails, run `uvx opik-mcp@latest --help` in a terminal to see the real
      error.
  </Accordion>
  <Accordion title="The server keeps starting an old version">
    ...
    - Re-run `uvx opik mcp configure`. It registers `opik-mcp@latest`, which
      resolves past an installed tool, and reports the leftover install if it finds
  ...
  <Accordion title="The local server fails to start with no network">
    `opik-mcp@latest` asks uv to resolve against the package index, so a machine
    with no route to PyPI and nothing in its uv cache cannot start the local
    server. A warm cache is enough — a laptop that has run the server before still
    starts it offline.

    - For an airgapped or firewalled deployment, point uv at an internal index
      (`UV_DEFAULT_INDEX`) that mirrors `opik-mcp`.
    - Note that a bare `uvx opik-mcp` fails the same way on a cold cache unless
      `opik-mcp` is installed as a uv tool.
  </Accordion>
