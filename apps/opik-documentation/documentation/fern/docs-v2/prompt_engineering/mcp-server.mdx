...
default: turn on `mcpOAuth.enabled` in the Helm chart, or set
`MCP_OAUTH_ENABLED=true` with a public `OPIK_BASE_URL`. Where it is off, the
endpoint has no sign-in routes, so use the [local server](#local-server-uvx)
instead.
