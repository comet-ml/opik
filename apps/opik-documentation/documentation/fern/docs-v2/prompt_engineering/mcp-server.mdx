...
The server URL, which is what every client below asks for:
...
server connects over HTTP and signs in through the browser on first use, so no API
key is stored in the client config.
...
<Tabs>
    ...
    Claude then opens the Opik sign-in in your browser. Until you finish it, the
    connector lists only the `authenticate` and `complete_authentication` tools;
    the rest appear once you are signed in.
    ...
    is not: use the [local server](#local-server-uvx) with a desktop client
    instead.
    </Note>
...
<AccordionGroup>
  ...
  <Accordion title="The client lists only `authenticate` and `complete_authentication`">
    You are connected but not signed in. Those two tools are the sign-in itself.

    - Ask your assistant to authenticate, or run the `authenticate` tool, and
      finish in the browser tab it opens.
    - In Claude Code, `/mcp` and then Authenticate does the same.
...
<AccordionGroup>
  ...
  <Accordion title="Can I use Opik from Claude.ai, Claude Desktop or Cowork?">
    Yes, as a custom connector. Add
    `https://www.comet.com/opik/api/v1/mcp` under **Customize → Connectors → Add
    custom connector**, or under **Organization settings → Connectors** on Team and
    Enterprise, then sign in when Claude opens the browser. Details and the
    self-hosted caveat are in [Manual setup](#hosted-server-opik-cloud). This gives
    you the MCP server; the skill pack is for coding agents only.
  </Accordion>
