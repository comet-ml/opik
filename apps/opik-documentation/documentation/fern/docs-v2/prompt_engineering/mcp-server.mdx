...
Beyond the tools, Opik ships a set of **skills** — Markdown guides that teach a
coding agent _how_ to use Opik for real tasks: tracing, evaluation, the SDK, and
an end-to-end `instrument` routine. Unlike ubiquitous CLIs, a model doesn't know
Opik from training, so the skills are what let it use Opik without fumbling.

The skills are authored once in
[`opik-mcp`](https://github.com/comet-ml/opik-mcp/tree/main/src/opik_mcp/skills)
and reach your agent a few ways:

- **Standard install (any agent)** — the ecosystem-standard skills installer,
  which works across Claude Code, Cursor, Codex, VS Code, Gemini CLI, and more:
...
- **Claude Code plugin** — get the skills **and** the MCP in one install:

  ```text
  /plugin install opik
  ```

- **With the MCP** — if you're already connected, the agent pulls a skill on
  demand with `read_skill`; nothing to install:
...
Already have the Opik Python package? `uvx opik-mcp skills install` exports the
same skills to disk as a convenience.
