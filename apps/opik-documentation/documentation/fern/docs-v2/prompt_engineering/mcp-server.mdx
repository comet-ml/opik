...
Verify with a real call: reload your MCP servers if you can and call the Opik
`list` tool for projects; if the server is not reachable until a restart, confirm
`curl -s -o /dev/null -w '%{http_code}' <url>` returns 401 and tell me what to run
after restarting. Report what you configured, where, and whether it is verified.
