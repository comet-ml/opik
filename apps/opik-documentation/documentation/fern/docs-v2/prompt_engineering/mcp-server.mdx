...
<AccordionGroup>
  <Accordion title="The client asks for authentication, or the browser sign-in never opened">
    ...
    - On a corporate network, allow `www.comet.com`.
  ...
  <Accordion title="Opik does not show up in the client, or shows no tools">
    ...
    - Start a new session.
    - Run `uvx opik mcp status`. It lists every client that has the server and the
      config file it lives in. If your client is missing, run
...
<AccordionGroup>
  ...
  <Accordion title="Hosted or local server: which one do I get, and where do credentials live?">
    ...
    the client's `env` block. `uvx opik mcp status` shows which one each client
    uses.
  </Accordion>
  ...
  <Accordion title="How do I update?">
    `uvx opik@latest mcp configure`. It rewrites the client config and refreshes the
    skill pack. Plain `uvx opik` reuses the version it already has cached. Start a
