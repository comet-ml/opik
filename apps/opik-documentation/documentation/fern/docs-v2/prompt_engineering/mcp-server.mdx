...
`opik mcp status` reports what each config *says*. To see what it *does* — which
matters when an `opik-mcp` installed as a uv tool has taken over a registration —
run:
...
It starts each configured local server once, reports the version that came back
and the version published on PyPI, and tells you whether they match:

```text
  Claude Code
    Config      ~/.claude.json
    Connection  Local (uvx)
    Launches    /opt/homebrew/bin/uvx --isolated opik-mcp
    Running     opik-mcp 0.2.35 (resolved from the package index)
    Published   opik-mcp 0.2.35
    Verdict     ✓ running the published version
```

A client served by a leftover tool install is reported as
`(from \`uv tool install\` — pinned)` with a `STALE` verdict and the fix.

<Note>
Doctor's launches are excluded from Opik's usage analytics, so checking does not
register as real sessions. Don't use the MCP `initialize` handshake to check this
yourself — the `serverInfo.version` it returns is the `mcp` library's version, not
opik-mcp's.
</Note>
...
The local server runs on demand via `uvx --isolated opik-mcp` (requires
...
<AccordionGroup>
  ...
  <Accordion title="The first tool call takes a long time, or the client says the server failed to start">
    With the local server the client runs `uvx --isolated opik-mcp`, which
    downloads the package and a Python runtime on first use. Setup pre-warms that cache, but
    gives up after a timeout.

    - Let the first call finish once. Later calls start in about a second.
    - If it fails, run `uvx --isolated opik-mcp --help` in a terminal to see the
      real error.
  </Accordion>
  <Accordion title="The server keeps starting an old version">
    ...
    the MCP server, which leaves `opik-mcp` permanently installed as a uv tool. A
    client configured to run `uvx opik-mcp` then starts that installed copy rather
    than the published one, on every restart.

    - Re-run `uvx opik mcp configure`. It registers `uvx --isolated opik-mcp`,
      which ignores an installed tool, and reports the leftover install if it finds
      one. `uvx opik mcp status` reports it too.
    - To clear the install itself: `uv tool upgrade opik-mcp` to update it, or
      `uv tool uninstall opik-mcp` to remove it. Opik will not do either for you —
      it is your environment.
  </Accordion>
  <Accordion title="The local server fails to start with no network">
    The local server resolves `opik-mcp` through uv, so a machine with no route to
    PyPI and nothing in its uv cache cannot start it. A warm cache is enough — a
    laptop that has run the server before still starts it offline.

    - For an airgapped or firewalled deployment, point uv at an internal index
      (`UV_DEFAULT_INDEX`) that mirrors `opik-mcp`.
    - `--isolated` deliberately ignores any `opik-mcp` installed as a uv tool, so
      an install no longer covers a cold cache offline. `uv tool uninstall
      opik-mcp` and a warm cache is the supported path.
  </Accordion>
