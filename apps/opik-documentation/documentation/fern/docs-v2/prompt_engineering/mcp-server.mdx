...
It detects your AI client — Claude Code, Cursor, VS Code Copilot, Codex or
opencode — picks the right server for your Opik deployment, installs the Opik
skill pack, and checks that what it wrote works.

Then **start a new session**: assistants read their configuration at startup, so
a newly added server needs a fresh one. On Opik Cloud, complete the browser
sign-in when your client prompts for it, then ask **"list my Opik projects"** to
confirm.

<Tip>
No `uv`? Install it with `curl -LsSf https://astral.sh/uv/install.sh | sh` on
macOS and Linux, or
`powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"`
on Windows, then open a new terminal.
</Tip>
