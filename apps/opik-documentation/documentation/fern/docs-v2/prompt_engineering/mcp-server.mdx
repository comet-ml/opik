...
use Opik — add tracing, evaluate, debug — installable in one command and usable
with or without this MCP. Install them the standard way:

```bash
npx skills add comet-ml/opik-skills
```

See **[Agent Skills](/agent-skills)** for the full list and usage. If your agent
is connected to this MCP, it can also pull skills on demand via `read_skill` —
nothing to install.
