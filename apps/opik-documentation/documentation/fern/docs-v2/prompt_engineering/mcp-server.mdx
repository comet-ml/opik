...
<Steps>
  ...
  <Step title="Restart your AI client">
    ...
    before trying the prompts in [Start using it](#start-using-it). In Claude Code,
    `/mcp` and **Reconnect** picks the server up without leaving the session.
...
On Opik Cloud this needs no API key and no terminal. Paste this into the agent
you use:

```text title="Installer prompt"
Connect my coding agents to Opik (https://www.comet.com/docs/opik/mcp-server) so
they can read my traces and experiments and fix code from them. Detect which
coding agents are installed here and which one you are; ask me whether to set up
only you or all of them, then wait. Register the Opik MCP server for the chosen
agents in their user-level config: name `opik-mcp`, Streamable HTTP, URL
https://www.comet.com/opik/api/v1/mcp, no credentials (sign-in happens in my
browser), using each client's own add command or `npx add-mcp <url> --name opik-mcp`.
Install the Opik skills for the same agents with `npx skills add comet-ml/opik-skills -g -y`.
Verify with a real call: call the Opik `list` tool for projects and show me the
result. If you cannot see the new server yet, ask me to reload MCP servers in my
client (`/mcp` and Reconnect in Claude Code) and try again; only if it still does
not appear, confirm `curl -s -o /dev/null -w '%{http_code}' <url>` returns 401 and
tell me to start a new session. Report what you configured, where, and whether it
is verified.
Change nothing else. For self-hosted Opik use my API base plus /v1/mcp, and fall
back to `uvx opik mcp configure` if it returns 404.
```
