...
`--ai-client` takes `claude-code`, `cursor`, `vscode`, `codex`, `opencode`, or
`all` for every client detected on the machine; repeat it for several. `--skills`
...
<AccordionGroup>
  ...
  <Accordion title='"Opik is not configured yet" or "needs either a terminal or an explicit client"'>
    ...
    Name the client and pass credentials through the environment:
