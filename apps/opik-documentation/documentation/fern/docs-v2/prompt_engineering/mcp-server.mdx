...
terminal: paste this into the agent you use, and it registers the hosted server
for itself, installs the skills, and hands you the browser sign-in.

```text title="Installer prompt for your coding agent"
Connect yourself to Opik so you can read my traces and experiments and fix code
from them. Reference: https://www.comet.com/docs/opik/mcp-server

1. Detect, don't assume: which coding agent you are running in, and whether
   `npx` is available.
2. Register the Opik MCP server for yourself. Server name `opik-mcp`, transport
   Streamable HTTP, URL https://www.comet.com/opik/api/v1/mcp, no headers and no
   credentials; sign-in happens in my browser on first connect. Use your own
   client's way of adding an MCP server if it has one (for example
   `claude mcp add --transport http opik-mcp <url>` or `codex mcp add opik-mcp --url <url>`),
   otherwise `npx add-mcp <url> --name opik-mcp`. Write it to my user-level config,
   not into this repo.
3. Install the Opik skills for yourself: `npx skills add comet-ml/opik-skills -g -y`.
4. Prove it: show me the MCP entry you wrote (your client's list command or the
   config file) and the list of installed skills. If either step failed, fix it
   and re-run.
5. Tell me to start a new session, complete the Opik sign-in when the browser
   opens, and test with "List my Opik projects".

If I say I use a self-hosted Opik, use my deployment's API base plus /v1/mcp as
the URL instead, and fall back to `uvx opik mcp configure` if that endpoint
returns 404. Do not change anything else on my machine or in this repo.
```
