...
Or skip the terminal: paste this into Claude Code, Cursor, VS Code Copilot, Codex
or opencode and the agent does the setup for itself.

```text title="Installer prompt for your coding agent"
Set up the Opik MCP server and the Opik skills for yourself.

1. Check that uv is installed: `uvx --version`. If it is missing, install it
   (macOS/Linux: `curl -LsSf https://astral.sh/uv/install.sh | sh`; Windows:
   `powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"`)
   and use a fresh shell afterwards.
2. Run `uvx opik configure status`. If it says the configuration is not found,
   ask me for my Opik API key and workspace name (on https://www.comet.com/opik
   they are in Account settings or the user menu, top right), then run
   `uvx opik configure --yes --no-install-mcp --no-install-skills` with
   OPIK_API_KEY and OPIK_WORKSPACE set in the environment of that one command.
   Do not write the key anywhere else. If I run Opik locally or self-hosted,
   ask for its URL and add `--use-local` or OPIK_URL_OVERRIDE instead of a key.
3. Run `uvx opik mcp configure --skills --ai-client <the client you are running
   in: claude-code, cursor, vscode, codex or opencode>`.
4. Run `uvx opik mcp status`, show me the output, and tell me to start a new
   session so you pick up the server and the skills.
```
