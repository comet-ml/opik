...
Or let your coding agent do it. On Opik Cloud this needs no API key and no
terminal: paste this into the agent you use. It finds the coding agents on your
machine, asks which ones to set up, installs the server and the skills, and
verifies with a real call.

```text title="Installer prompt for your coding agent"
Connect my coding agents to Opik so they can read my traces and experiments and
fix code from them. Reference: https://www.comet.com/docs/opik/mcp-server

1. Detect which coding agents are installed on this machine (check for their
   binaries and config directories), and which one you are running in.
2. Ask me one question, then wait: set up only you, or every agent you found?
3. Register the Opik MCP server for the chosen agents. Name `opik-mcp`, transport
   Streamable HTTP, URL https://www.comet.com/opik/api/v1/mcp, no headers, no
   credentials; sign-in happens in my browser on first connect. Prefer each
   client's own add command (for example `claude mcp add --transport http opik-mcp <url>`
   or `codex mcp add opik-mcp --url <url>`); otherwise
   `npx add-mcp <url> --name opik-mcp -g -a <agent>` (or `--all`). User-level
   config, never this repo.
4. Install the Opik skills for the same agents:
   `npx skills add comet-ml/opik-skills -g -y -a <agent>` (or `-a '*'`).
5. Verify with a real call, not by reading config. Reload your MCP servers if
   your client can, then call the Opik `list` tool for projects and show me the
   result. If you cannot reach the new server in this session, confirm the
   endpoint answers (`curl -s -o /dev/null -w '%{http_code}' <url>` returns 401),
   then tell me to start a new session and ask you to "list my Opik projects".
6. Report: which agents you configured and where each config lives, the skills
   installed, and the verification result. Say plainly whether it is verified or
   still waiting for a restart and the browser sign-in.

Do not change anything else on my machine or in this repo. If I use a self-hosted
Opik, use my deployment's API base plus /v1/mcp; if it returns 404, fall back to
`uvx opik mcp configure`.
```

One-click for Cursor and VS Code, hosted server only, sign-in in the browser:
...
The buttons add the server only, not the skills. Another client, or a self-hosted
...
Zed, Gemini CLI, Claude Desktop, Goose, Cline, Kiro and a dozen more. On a
self-hosted deployment, replace the URL with your own API base plus `/v1/mcp`, or
...
Two checks solve most problems: run `uvx opik mcp status`, then start a new session in
your client.
...
<AccordionGroup>
  ...
  <Accordion title="Opik is not in the client, or it connected with no tools">
    ...
    `uvx opik mcp status`: it lists every client that has Opik and the file it lives
  ...
  <Accordion title='"Opik is not configured yet" or "needs either a terminal or an explicit client"'>
    ...
    Name the client, and provide credentials through `OPIK_API_KEY` and
    `OPIK_WORKSPACE` in the environment, or through a `~/.opik.config` created by
    an earlier `uvx opik configure`, rather than typing the key into the command:
  ...
  <Accordion title="The agent sees no data, or the wrong data">
    ...
    the hosted server the workspace is chosen at sign-in. Run `uvx opik configure`,
    pick the workspace, then `uvx opik mcp configure` again and start a new
    session. If `uvx opik mcp status` shows `✗ OUT OF SYNC`, the same re-run fixes it.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Hosted or local server, which do I get?">
    ...
    otherwise, or when you pass `--local-server`. `uvx opik mcp status` shows which one
  ...
  <Accordion title="Can I use several workspaces?">
    ...
    server, run `uvx opik configure` to switch, then `uvx opik mcp configure` again.
  ...
  <Accordion title="How do I update or remove it?">
    ...
    skill pack; uvx otherwise reuses its cached copy. If you also have the `opik`
    package installed, the two can be different versions; their MCP configs are
    compatible, and `uvx opik mcp status` reads either. Remove: there is no
