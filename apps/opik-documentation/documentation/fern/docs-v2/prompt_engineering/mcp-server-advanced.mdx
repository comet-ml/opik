---
headline: MCP server — advanced configuration
...
og:title: Opik MCP server — advanced configuration
title: MCP server — advanced configuration
---

Everything beyond `uvx opik mcp configure`. If you haven't set the server up yet,
start at [Opik's MCP server](/mcp-server) — this page is for checking, changing
and repairing that setup, and for clients the CLI doesn't cover.
...
results. That is why setup installs both.
