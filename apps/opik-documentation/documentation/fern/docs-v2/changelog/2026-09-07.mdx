...
A custom provider can now be configured with `auth_config` instead of a static `api_key`:
...
```json
{
  "auth_config": {
    ...
    "send_as": "form",
    "credentials": [{ "key": "client_id", "value": "..." }, { "key": "client_secret", "value": "...", "secret": true }],
...
Opik fetches and refreshes the token automatically, using `fallback_ttl_seconds` when a provider doesn't return an expiry. A new "test connection" action in the provider dialog verifies the token exchange before you save.
