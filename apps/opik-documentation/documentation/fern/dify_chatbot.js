const injectDifyChatbot = () => {
  // Add configuration script
  const configScript = document.createElement("script");
  configScript.textContent = `window.difyChatbotConfig = {
      token: 'fsuGIcZVsGayZOB2'
    }`;
  document.head.appendChild(configScript);

  // Add main script
  const dify_embed_min_js = `
!function() {
    ...
    document.addEventListener("keydown", d),
    p?.dynamicScript ? e() : document.readyState === 'loading' ? document.addEventListener('DOMContentLoaded', e) : e();
}();`;

  const mainScript = document.createElement("script");
  mainScript.textContent = dify_embed_min_js;
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", injectDifyChatbot);
} else {
  injectDifyChatbot();
}
