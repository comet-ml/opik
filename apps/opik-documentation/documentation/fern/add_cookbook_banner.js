...
const watchPage = () => {
  ...
  const observer = new MutationObserver(() => {
    ...
    // Only proceed if we're on a cookbook page and not the overview
    if (
      path.includes("docs/opik/cookbook/") &&
      !path.endsWith("docs/opik/cookbook/overview") &&
      heading &&
      !heading.nextSibling?.classList?.contains("callout-outlined-tip")
    ) {
      console.log("Injecting GitHub link...");
      injectGitHubLink();
    }
  });

  if (document.body) {
    console.log("Found body, attaching observer");
    ...
    if (
      ...
      console.log("Initial GitHub link injection...");
      injectGitHubLink();
    }
  } else {
...
console.log("Script loaded, waiting to start...");
