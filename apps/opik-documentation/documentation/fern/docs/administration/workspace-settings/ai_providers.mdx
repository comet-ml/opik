...
Some gateways (notably Azure OpenAI behind API Management) require specific query parameters — most commonly `api-version` — on every request. Use the "Query parameters" section to add key/value pairs that Opik will append to every outbound URL.
...
Parameters are URL-encoded automatically and appended with `?` or `&` as appropriate. Leave this section empty for providers that don't require extra query parameters.

##### Auth Header Name and Suppressing the Default Authorization Header

Most OpenAI-compatible providers expect an `Authorization: Bearer <key>` header — Opik sends this automatically. Some gateways (for example, Azure OpenAI behind API Management) expect the key in a differently-named header such as `api-key` or a tenant-specific name, sometimes without any `Authorization` header at all.

- **Auth header name** (optional): If set, Opik sends the API key as `{name}: <key>` *in addition to* the default `Authorization: Bearer <key>` header. Most APIM gateways ignore the extra `Authorization` header, so this alone is often enough.
- **Suppress default Authorization header** (toggle, default off): Turn this on only if your gateway explicitly rejects requests that include `Authorization: Bearer`. When on, Opik sends only the custom header configured above. Leave off for every existing provider — the default preserves today's behavior.

##### URL Templates with `{model}`

For multi-deployment gateways (for example, Azure OpenAI via APIM with many deployments), the deployment name is baked into the URL path rather than sent in the request body. Rather than creating a separate provider entry per deployment, you can write the URL once with `{model}` as a placeholder:

```
https://my-gateway.azure-api.net/openai/deployments/{model}
```

At request time Opik substitutes `{model}` with the deployment name of whichever model the end-user picked from the Models list. The Models field then contains all deployment names (e.g. `gpt-4o, gpt-4o-mini, gpt-4o-mini-ZA`), and the playground / Online Evaluation dropdown shows each one as a selectable model routed to its own upstream path.
...
- An `api-key` header (or a tenant-specific header name) rather than `Authorization: Bearer`.

Configure it as follows:

1. Select **vLLM / Custom provider**.
2. **Provider name**: something descriptive (e.g. `azure-apim-prod`).
3. **URL**: full gateway path up to and including the deployment placeholder, e.g.
   `https://my-gateway.azure-api.net/openai/deployments/{model}`.
