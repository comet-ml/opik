...
Some gateways require specific query parameters — such as a version identifier — on every request. Use the "Query parameters" section to add key/value pairs that Opik will append to every outbound URL.
...
- **Auth header name** (optional): If set, Opik sends the API key as `{name}: <key>` *in addition to* the default `Authorization: Bearer <key>` header. Most gateways ignore the extra `Authorization` header, so this alone is often enough.
- **Suppress default Authorization header** (toggle, default off): Turn this on only if your gateway explicitly rejects requests that include `Authorization: Bearer`. When on, Opik sends only the custom header configured above.

##### URL Templates with `{model}`

When a gateway expects the model or deployment name in the URL path rather than in the request body, you can write the URL once with `{model}` as a placeholder instead of creating one provider entry per model:
