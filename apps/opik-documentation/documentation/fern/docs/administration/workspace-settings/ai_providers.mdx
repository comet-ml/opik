...
Use the "Query parameters" section to define these key/value pairs, and Opik will automatically append them to every outgoing request.
...
The model name is interpolated raw (no URL encoding) so it lands as a literal path segment, matching what most gateways expect. Use alphanumeric and hyphen-only model names — values containing `/` (e.g. HuggingFace-style `mistralai/Mistral-7B-Instruct-v0.3`) will split into extra path segments and Opik will emit a warning in the backend logs at request time.
