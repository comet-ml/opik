...
The `--local-be-fe` mode uses `host.containers.internal` to let the container reach your local backend. This requires Podman 4.0+ with Netavark networking (the default network driver from Podman 4.0) or pasta networking (the default from Podman 5.0).
