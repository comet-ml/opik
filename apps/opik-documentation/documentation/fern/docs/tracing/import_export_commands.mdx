...
Every import automatically maintains a `migration_manifest.json` file in the
`--path` directory. This manifest tracks which files have been imported and the
ID mappings needed to link experiments back to their traces.
...
```
OUTPUT_DIR/
├── migration_manifest.json          # Created automatically during import to track progress
└── WORKSPACE/
    ├── datasets/
...
your destination credentials. The `migration_manifest.json` created during
...
1. **Import was interrupted and re-running creates duplicates**
   - This should not happen — the import automatically detects and resumes from the `migration_manifest.json` in your `--path` directory
   ...
   - If the manifest is corrupt, use `--force` to start fresh (this will re-import everything)
