...
|-----------|------|---------|-------------|
| `job` | `Job` | `None` | Optional Harbor Job instance. If provided, returns the same job. |
| `project_name` | `str` | `None` | Opik project name. Uses `OPIK_PROJECT_NAME` env var if not set |

**Returns:** The job instance if provided, `None` otherwise.

**Usage modes:**
- `track_harbor(job)` - Wraps a job and enables tracking (for SDK usage)
- `track_harbor()` - Enables global tracking without a job (for CLI usage)
