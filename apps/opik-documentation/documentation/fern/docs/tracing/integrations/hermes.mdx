...
<Steps>
  <Step title="1. Install the plugin">
    Install the package from PyPI into the same Python environment as Hermes. This pulls in the `opik` SDK automatically and registers the plugin via the `hermes_agent.plugins` entry point (name `opik`).
...
<Steps>
  <Step title="1. Send a message">
    ```bash
    hermes chat -q "hello"
    ```

    On startup the plugin logs `OPIK: Started logging traces to ...` once it's enabled and connected. You can also open the Hermes web UI at `http://localhost:9119` and use the Chat tab.
  </Step>
  <Step title="2. Verify traces in Opik">
