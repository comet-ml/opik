...
Opik automatically logs the call, attaches the generated audio file, adds a
**Chars** column to usage tables, and the UI shows an inline audio player.  Cost
is calculated per character using OpenAI’s published rates.
