...
<Tip>
`track_openai` can also be used with OpenAI-compatible providers (including OpenRouter) by pointing your OpenAI client at the provider endpoint. For direct OpenRouter SDK tracking, use the [OpenRouter SDK integration](/docs/opik/integrations/openrouter/).
</Tip>

<Tip>
OpenRouter routing metadata is inferred from the request model and response when available, so it is best-effort and may not expose an explicit upstream selected-route label for every call.
</Tip>
