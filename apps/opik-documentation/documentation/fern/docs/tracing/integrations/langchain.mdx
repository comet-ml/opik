...
## OpikTracer Configuration

The `OpikTracer` constructor accepts several parameters to customize its behavior:

```python
from opik.integrations.langchain import OpikTracer
from opik.types import DistributedTraceHeadersDict

# Example distributed headers (optional)
...
opik_tracer = OpikTracer(
    tags=["langchain", "production"],           # List of tags for all traces
    metadata={"version": "1.0", "env": "prod"}, # Additional metadata
    project_name="my-langchain-project",        # Project name for traces
    thread_id="conversation-123",               # Thread ID for grouping traces
    distributed_headers=distributed_headers,    # For distributed tracing
    graph=graph                               # LangGraph object for visualization
)
```

### Parameters

- **`tags`** (Optional[List[str]]): List of tags to be applied to each trace logged by the tracer
- **`metadata`** (Optional[Dict[str, Any]]): Additional metadata for each trace logged by the tracer  
- **`project_name`** (Optional[str]): The name of the project to log data to
- **`thread_id`** (Optional[str]): Thread ID for grouping related conversations or interactions
- **`distributed_headers`** (Optional[DistributedTraceHeadersDict]): Headers for connecting traces across services
- **`graph`** (Optional[Graph]): A LangGraph Graph object to track the Graph Definition in Opik

## Cost Tracking

The `OpikTracer` automatically tracks costs for supported LLM providers. Cost tracking works with Chat models from major providers including:

- **OpenAI** (ChatOpenAI)
- **Anthropic** (ChatAnthropic) 
- **Anthropic Vertex AI** (ChatAnthropicVertex)
...
The `OpikTracer` is fully compatible with the `@opik.track` decorator, allowing you to create hybrid tracing approaches:
