...
Opik provides seamless integration with the [Google Generative AI Node.js SDK](https://github.com/googleapis/generative-ai-js) through the `opik-gemini` package, allowing you to trace, monitor, and debug your Gemini API calls.
...
// Get a model and generate content
const model = trackedGenAI.getGenerativeModel({ model: "gemini-2.0-flash" });
const result = await model.generateContent("Hello, how can you help me today?");
...
async function streamingExample() {
  ...
  const result = await model.generateContentStream(
    "Write a short story about AI observability"
  );
...
const model = trackedGenAI.getGenerativeModel({ model: "gemini-2.0-flash" });
const result = await model.generateContent("Generate a creative story");
...
async function chatExample() {
  ...
  const result = await chat.sendMessage(
    "What are the key benefits of using Opik?"
  );
