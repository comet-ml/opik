...
og:description: Monitor and debug OpenAI API calls effortlessly using Opik's tracing
...
<Tip>
`trackOpenAI` also supports OpenAI-compatible providers (including OpenRouter) when your client is configured with their endpoint. For dedicated OpenRouter SDK tracing, use the [OpenRouter SDK integration](/docs/opik/integrations/openrouter/).
</Tip>

<Tip>
OpenRouter routing metadata is inferred from request/response model fields and is best-effort; a separate explicit `routing_provider` field is not guaranteed to be populated today.
</Tip>
