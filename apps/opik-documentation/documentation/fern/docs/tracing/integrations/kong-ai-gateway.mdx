...
[Kong](https://docs.konghq.com/gateway/latest/) is a popular open-source API gateway that has recently released an AI Gateway. If you are looking for an LLM gateway that is production-ready and supports many of the expected enterprise use cases (authentication mechanisms, load balancing, caching, etc.), this is the gateway we recommend.
