...
This page lists all models that Opik supports for automatic cost tracking. Cost tracking is automatically enabled when using any of the [supported integrations](/v1/tracing/cost_tracking#supported-models-providers-and-integrations) with these models.
...
Opik supports cost tracking for **359 models** across **7 providers**:
...
Opik automatically calculates costs for the models listed above when using [supported integrations](/v1/tracing/cost_tracking#supported-models-providers-and-integrations). The cost calculation is handled by backend services that:
