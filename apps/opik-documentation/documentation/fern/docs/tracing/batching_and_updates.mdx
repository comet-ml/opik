...
This only affects the low-level client API (`client.trace()`, `trace.span()`, `.end()`, `.update()`). The `@track` decorator, `start_as_current_span`, and `start_as_current_trace` context managers handle lifecycle automatically and are not affected.

## Recommended approaches

### Use decorators or context managers (recommended)

The safest way to create and manage spans and traces is through the `@track` decorator or context managers. These handle the full lifecycle — creation, data capture, and finalization — in a single operation with no separate update step.
...
with opik.start_as_current_trace(name="my_trace") as trace:
    with opik.start_as_current_span(name="my_span") as span:
        result = call_llm("Hello")
```

### Provide all data at creation time

If you need to use the low-level client API, pass all data (including `end_time`) when creating the span or trace. This avoids the need for a separate update call.
