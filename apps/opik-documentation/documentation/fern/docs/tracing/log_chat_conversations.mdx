...
### Online Scoring Cooling Period

For online evaluation rules (automatic scoring), Opik waits for a "cooling period" after the last activity
in a thread before running the rules. This gives conversations time to settle before automatic evaluation.

<Note>
  By default, the cooling period is 15 minutes. You can change this value by setting the
  `OPIK_TRACE_THREAD_TIMEOUT_TO_MARK_AS_INACTIVE` environment variable (if you are using the Opik self-hosted version).
  On cloud, you can change this setting at workspace level under "Online scoring cooling period".
</Note>
