...
Opik has integrations with many popular Agent frameworks ([LangGraph](/integrations/langgraph), [LLamaIndex](/integrations/llama_index))
...
You can track the agent execution by using either one of [Opik's integrations](/integrations/overview) or the `@track` decorator:

<Tabs>
    <Tab value="LangGraph" title="LangGraph">
    You can log the agent execution by using the [OpikTracer](/integrations/langgraph) callback:
    ...
    To log a Haystack pipeline run, you can use the [`OpikConnector`](/integrations/haystack). This connector will log the pipeline run to the Opik platform and add a `tracer` key to the pipeline run response with the trace ID:
    ...
    Opik has a built-in integration with [LLamaIndex](/integrations/llama_index) that makes it easy to track the agent execution:
...
If you are using our [LangGraph](/integrations/langgraph) integration, you can also track the agent definition by passing in the `graph` argument to the `OpikTracer` callback:
