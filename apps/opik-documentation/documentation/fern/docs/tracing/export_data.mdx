...
All search methods (`search_traces`, `search_spans`, and `search_threads`) accept a `filter_string` (Python) / `filterString` (TypeScript) parameter that uses the same Opik Query Language (OQL) syntax:
...
Here are some examples of using `filter_string` / `filterString`:
...
<Tabs>
  <Tab value="Python" title="Python">
    ...
    # Filter by input content (works with traces, spans, and threads)
    results = client.search_traces(filter_string='input contains "Opik"')

    # Filter by date range
    results = client.search_traces(filter_string='start_time >= "2024-01-01T00:00:00Z"')

    # Filter by tag
    results = client.search_traces(filter_string='tags contains "production"')

    # Filter by token usage
    results = client.search_traces(filter_string='usage.total_tokens > 1000')

    # Filter by metadata
    results = client.search_traces(filter_string='metadata.model = "gpt-4o"')

    # Filter by feedback score presence
    results = client.search_traces(filter_string='feedback_scores.user_rating is_not_empty')

    # Filter by feedback score absence
    results = client.search_traces(filter_string='feedback_scores.user_rating is_empty')

    # The same filter syntax works for threads
