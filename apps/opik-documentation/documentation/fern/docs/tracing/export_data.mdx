...
2. Using the Opik REST API: You can use the [`/traces`](/reference/rest_api/get-traces-by-project.api) and [`/spans`](/reference/rest_api/get-spans-by-project.api) endpoints to export traces and spans.
...
To export traces using the Opik REST API, you can use the [`/traces`](/reference/rest_api/get-traces-by-project.api) endpoint and the [`/spans`](/reference/rest_api/get-spans-by-project.api) endpoint. These endpoints are paginated so you will need to make multiple requests to retrieve all the traces or spans you want.
