...
When any Python SDK method accepts a `project_name` parameter, the value is resolved using the following precedence:

1. **Explicit argument** — the `project_name` passed directly to the method or decorator (e.g., `@track(project_name="...")`, `client.trace(project_name="...")`)
...
This means that if you call an Opik client method inside a `@track(project_name="my-agent")` decorated function without passing `project_name` explicitly, it will automatically use `"my-agent"` from the active context.

#### `opik.project_context()`

The `opik.project_context()` context manager sets the project name for all Opik operations within a block — traces, spans, feedback scores, and integration callbacks:

```python
import opik

with opik.project_context("customer-support"):
    # All traces and spans created here will be logged to "customer-support"
    my_agent(query)
```
