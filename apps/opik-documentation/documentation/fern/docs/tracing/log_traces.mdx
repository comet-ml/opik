---
subtitle: >-
  Describes how to log LLM calls to the Opik platform using function decorators,
  integrations or the low level client.
slug: /tracing/log_traces
---

<Tip>
  If you are just getting started with Opik, we recommend first checking out the [Quickstart](/docs/quickstart) guide
  that will walk you through the process of logging your first LLM call.
</Tip>
...
1. **Using one of our [integrations](/docs/tracing/integrations/overview):** This is the easiest way to get started.
...
<Tip>
  ...
  guide](/docs/self-host/overview) to get started. Alternatively, you can use our hosted platform by creating an account
  on [Comet](https://www.comet.com/signup?from=llm).
</Tip>
...
integrations available for [many of the most popular LLM frameworks and libraries](/docs/tracing/integrations/overview).
...
method in conjunction with one of our [integrations](/docs/tracing/integrations/overview) for the most seamless experience.
...
You can learn more about this feature in the [Online evaluation](/docs/production/rules) guide.
...
```bash
npm install --save opik
```

Once the library is installed, you can use initialized the Opik client:
...
The documentation for the Opik REST API is available [here](/docs/api-reference).
