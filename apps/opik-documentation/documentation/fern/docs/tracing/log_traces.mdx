...
<Tip>
  If you are just getting started with Opik, we recommend first checking out the [Quickstart](/v1/quickstart) guide that
...
[Python SDK](https://www.comet.com/docs/opik/python-sdk-reference/), [first class OpenTelemetry support](/v1/integrations/opentelemetry)
...
<Tip>
  ...
  integrations in the [integrations overview](/v1/integrations/overview) page.
</Tip>

We won't be covering how to track chat conversations in this guide, you can learn more about this in the
[Logging conversations](/v1/tracing/log_chat_conversations) guide.
...
<Tip>
  ...
  guide](/v1/self-host/overview) to get started. Alternatively, you can use our hosted platform by creating an account on
...
<Tabs>
  ...
  <Tab title="Other" value="other" language="other">
    ...
    a full list of integrations [here](/v1/integrations/overview). For example:
    ...
    [Log Traces](/v1/tracing/log_traces) guide for more information.
...
<Tip>
  ...
  of integrations in the integrations [overview page](/v1/integrations/overview).
</Tip>
...
[agent graph](/v1/tracing/log_agent_graphs) and review all the tool calls made by the agent.
...
<Tabs>
    <Tab title="Typescript" value="typescript" language="typescript">
        ...
        <Tip>
            ...
            you can find more information about the configuration [here](/v1/tracing/sdk_configuration).
        </Tip>
    </Tab>
    <Tab title="Python" value="python" language="python">
...
<Tabs>
    <Tab title="Typescript" value="typescript" language="typescript">
        ...
        When tracing is disabled, the `track` decorator and the integrations stop sending data, and
        direct `client.trace()` / `span` calls become no-ops as well. Non-tracing APIs (datasets,
        experiments, prompts) continue to work. With tracing disabled, the client no longer requires an
        API key or workspace, so an instrumented app can run without sending anything to Opik and
        without a local deployment.
    </Tab>
    <Tab title="Python" value="python" language="python">
        You can disable the logging process globally using the `OPIK_TRACK_DISABLE` environment variable.
...
- [Logging chat conversations](/v1/tracing/log_chat_conversations)
- [Logging user feedback](/v1/tracing/annotate_traces)
- [Setup online evaluation metrics](/v1/production/rules)
