...
When `@track(project_name="...")` is used, the decorator sets the project context for the duration of the function. Any nested `@track`-decorated functions, `Opik()` client calls, and native integrations inside it will automatically inherit that project name:
