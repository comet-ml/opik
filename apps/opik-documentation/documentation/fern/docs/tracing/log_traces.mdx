...
<Tabs>
    ...
    <Tab title="Python" value="python" language="python">
        ...
        When `@track(project_name="...")` is used, the decorator sets the project context for the duration of the function. Any nested `@track`-decorated functions or Opik client calls inside it will automatically inherit that project name:
        ...
        @track
        def retrieve(query):
            # Inherits project_name="my-agent" from the parent
            ...

        @track
        def generate(context):
            # Also inherits project_name="my-agent"
            ...
        ```

        You can also use the `opik.project_context()` context manager to set the project name for all operations within a block:

        ```python
        import opik

        with opik.project_context("my_project"):
            # All traces, spans, and client operations here use "my_project"
            my_agent(query)
        ```
...
The project name for the `Opik()` client, `@track`-decorated functions, and native integrations (e.g., `track_openai`, `OpikTracer`) is resolved using the following precedence:

1. **Explicit argument** — the `project_name` passed directly to `Opik()`, `@track(project_name="...")`, an integration constructor (e.g., `OpikTracer(project_name="...")`), or a client method (e.g., `client.trace(project_name="...")`)
2. **Active project context** — set by `@track(project_name=...)` on a parent function or by the `opik.project_context()` context manager
3. **Client configuration** — the project name from the `Opik()` constructor, `OPIK_PROJECT_NAME` environment variable, or `~/.opik.config` file
4. **Default** — falls back to `"Default Project"` (a warning is logged once to remind you to configure a project name)

This means that if you use the `Opik()` client, call a `@track`-decorated function, or initialize a native integration inside an active project context without passing `project_name` explicitly, it will automatically use the project name from that context.

#### `opik.project_context()`

The `opik.project_context()` context manager sets the project name for all Opik operations within a block — the `Opik()` client, `@track`-decorated functions, and native integrations:

```python
import opik

with opik.project_context("customer-support"):
    # The Opik client, @track-decorated functions, and native integrations
    # all use "customer-support" as the project name
    my_agent(query)
```

The first context to set a project name becomes the owner. Nested `project_context` or `@track(project_name=...)` calls with a different name are ignored (a warning is logged), and the outer project name is preserved. This prevents traces from being split across projects within a single execution.
