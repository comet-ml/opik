...
<Tabs>
    ...
    <Tab title="Python" value="python" language="python">
        ...
        <Tip>
            The Python SDK resolves project names in this order: explicit argument → active project context (`@track` or `opik.project_context()`) → client configuration (`OPIK_PROJECT_NAME` env var or config file) → `"Default Project"`. For full details, see [Project Name Resolution](/tracing/sdk_configuration#project-name-resolution).
        </Tip>
    </Tab>
