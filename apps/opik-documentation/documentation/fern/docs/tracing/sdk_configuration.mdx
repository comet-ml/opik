...
In the Python SDK, `project_name` follows a specific resolution order that extends the general configuration precedence. When any SDK method accepts a `project_name` parameter, the value is resolved as follows:
...
2. **Active project context** — set by `@track(project_name=...)` on a parent function or by the `opik.project_context()` context manager (see below)
...
#### `opik.project_context()` context manager

You can use `opik.project_context()` to set the project name for all Opik operations within a block — traces, spans, feedback scores, and integration callbacks:
...
#### `@track` decorator context propagation

When you use `@track(project_name="...")`, the decorator sets the project context for the duration of the decorated function. Any nested `@track`-decorated functions or Opik client calls inside it will inherit that project name automatically:
...
def agent(query):
    # Both retrieve() and generate() inherit "my-agent" as the project
