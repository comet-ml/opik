...
In this model, `EpisodeResult.assertions` contains structured verdict records (for example, `"assistant_replied_each_turn" => passed=True`) rather than executable validators. If you return a plain dictionary, `validation_verdicts` is also accepted as an alias for `assertions` when you prefer verdict-focused naming. `episode.is_passing()` and `episode.assert_is_passing()` evaluate those records plus budgets into the final test outcome.
