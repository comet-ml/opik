...
### Simulations and Trajectories

For multi-turn agent testing, use `llm_unit` for lightweight pass/fail tracking and combine it with simulation/evaluation APIs for deeper quality checks:

- run scenarios with `run_simulation` to produce deterministic conversation threads,
- evaluate threads with `evaluate_threads` and conversation metrics,
- evaluate tool and reasoning quality with trajectory-focused metrics.

This pattern gives you a CI-friendly gate while still catching silent regressions in agent behavior.
