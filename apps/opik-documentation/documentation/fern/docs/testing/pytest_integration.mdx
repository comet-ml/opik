...
For multi-turn agent testing, use `llm_episode` and return an `EpisodeResult`. This lets you keep native pytest assertions while also emitting structured episode artifacts (assertions, scores, budgets, trajectory summary) for CI/reporting.
