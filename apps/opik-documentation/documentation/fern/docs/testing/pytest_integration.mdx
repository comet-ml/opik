...
Pytest hooks activate automatically when Opik `llm_unit` tests are collected. You can also force enablement with `--opik` or by setting `opik_pytest_enabled = true` in your pytest configuration.
