...
Ensuring your LLM applications are working as expected is a crucial step before deploying to production. Opik provides a Pytest integration so you can track both the overall pass/fail rate of your suite and the pass/fail result of each test.
...
The `llm_unit` decorator also works well with `pytest.mark.parametrize`, which lets you run the same test with different inputs:
...
### Final Example: Episode-Based Tests with Simulated Users

Use this pattern after the previous `llm_unit` examples, when you need multi-turn agent validation in CI.

`llm_episode` lets you return an `EpisodeResult` while still using native pytest failure semantics and optional structured artifacts (assertions, scores, budgets, trajectory summary).

In this model, `EpisodeResult.assertions` stores structured verdict records (for example, `"assistant_replied_each_turn" => passed=True`), not executable validators. If you return a plain dictionary, `validation_verdicts` is accepted as an alias for `assertions` when you prefer verdict-focused naming. `episode.is_passing()` and `episode.assert_is_passing()` evaluate those verdicts plus budgets into the final outcome.

The example below uses `SimulatedUser` with fixed responses so the scenario is deterministic and CI-friendly:
...
def test_refund_episode_ci_gate():
    ...
    # Native pytest failure signal with structured output on failure.
