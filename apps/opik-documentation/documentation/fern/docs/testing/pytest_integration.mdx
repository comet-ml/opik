...
In this model, `EpisodeResult.assertions` contains structured verdict records (for example, `"assistant_replied_each_turn" => passed=True`) rather than executable validators. `episode.is_passing()` and `episode.assert_is_passing()` evaluate those records plus budgets into the final test outcome.
