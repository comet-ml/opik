...
The `commit` parameter accepts the commit ID (also called commit hash) of the specific prompt version you want to retrieve. You can find commit IDs in the prompt history in the Opik UI or by using the `get_prompt_history` method (see below).
...
## Comparing and retrieving prompt commits

To compare different versions of a prompt or analyze how your prompts have evolved over time, you can retrieve the complete version history using [`get_prompt_history`](https://www.comet.com/docs/opik/python-sdk-reference/Opik.html#opik.Opik.get_prompt_history):
