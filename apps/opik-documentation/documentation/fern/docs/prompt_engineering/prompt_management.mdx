...
<Note>
  Structured prompts that include `image_url` parts keep their metadata (for
  example the `detail` field) when saved or loaded. Opik only converts those
  parts to plain markdown placeholders when the selected model does not support
  image input.
</Note>
