...
Prompts can be used in all supported third-party integrations by attaching them to traces and spans through the [`opik_context` module](/v1/prompt_engineering/prompt_management#adding-prompts-to-traces-and-spans).

For instance, you can use prompts with the `Google ADK` integration, as shown in the [example here](/v1/integrations/adk#prompts-integration).
