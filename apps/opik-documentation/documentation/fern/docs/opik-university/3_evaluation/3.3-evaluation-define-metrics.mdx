...
This video explores Opik's comprehensive [metrics](/v1/evaluation/metrics/overview) system that transforms subjective LLM assessment into quantifiable measurements. You'll discover the different types of automated scoring methods available, see practical examples using [Answer Relevance](/v1/evaluation/metrics/answer_relevance) and [Levenshtein](/v1/evaluation/metrics/heuristic_metrics) metrics, and learn how to create [custom metrics](/v1/evaluation/metrics/custom_metric) when needed. The video also covers cost considerations and best practices for combining multiple [metrics](/v1/evaluation/metrics/overview) to capture different dimensions of quality.
...
- **Easy Implementation**: Import metrics directly from [`opik.evaluation.metrics`](/v1/evaluation/overview#running-an-evaluation) and instantiate classes - demonstrated with Answer Relevance and Levenshtein ratio
