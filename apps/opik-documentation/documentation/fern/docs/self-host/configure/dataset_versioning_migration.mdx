...
Starting with Opik version `1.9.92`, the platform includes an **automated migration process** to support enhanced dataset versioning capabilities. This migration runs automatically during application startup.
...
### 2. Startup Data Migration (Automated, Blocking)
Runs synchronously during application startup before the application begins serving requests. This migration calculates and updates the `items_total` field for dataset versions created by the Liquibase schema migration.

**Behavior:**
- If all versions are already migrated, the migration is skipped with an INFO log message
- If unmigrated versions exist, the migration runs and blocks startup until complete
- If the migration fails, the application **stops with a descriptive error message** (fail-fast)

**Escape hatch**: If the startup migration fails and you need to start the application to investigate, set `DATASET_VERSIONING_STARTUP_MIGRATION_ENABLED=false` to skip the migration temporarily.
...
3. **Startup data migration runs** (blocking, before serving requests)
   - Processes dataset versions in batches
   - Updates `items_total` field in MySQL
   - Skips automatically if already migrated
4. **Application starts serving requests** after migration completes

### Subsequent Restarts

On subsequent restarts after a successful migration, the startup migration detects that all versions are already migrated and skips immediately with a log message:

```
Dataset versioning startup migration: all dataset versions fully migrated, skipping
```
...
Controls the blocking startup migration. The migration runs automatically and is safe to leave enabled — it becomes a no-op after the first successful run.

| Environment Variable | Default | Description |
|---------------------|---------|-------------|
| `DATASET_VERSIONING_STARTUP_MIGRATION_ENABLED` | `true` | Enable the blocking startup migration. Set to `false` to skip and investigate manually if it fails. |
...
| `DATASET_VERSION_ITEMS_TOTAL_MIGRATION_JOB_TIMEOUT_SECONDS` | `3600` | Maximum time for the entire migration (1 hour) |
...
```
Dataset versioning startup migration: completed successfully, all dataset versions migrated
```

Or, if already migrated from a previous run:
...
```sql
-- Should return 0 (no unmigrated versions remain)
SELECT count(id)
...
### Scenario 1: Startup Migration Fails (Fail-Fast)

**Symptoms:**
- Application fails to start with an error containing "data migration failed"
- Error message includes: `Set DATASET_VERSIONING_STARTUP_MIGRATION_ENABLED=false to skip and investigate manually`

**Recovery Steps:**

1. **Temporarily skip the migration** to start the application:
   ```yaml
   # Docker Compose
   services:
     backend:
       environment:
         - DATASET_VERSIONING_STARTUP_MIGRATION_ENABLED=false
   ```
...
2. **Investigate the root cause** by checking the error logs:
   ```bash
   kubectl logs -n opik deployment/opik-backend | grep -i "migration"
   ```
...
4. **After resolving the issue**, re-enable the startup migration by removing the override (or setting it to `true`) and restart.

### Scenario 2: Migration Takes Longer Than Expected

**Symptoms:**
- Application startup takes more than 10 minutes
- Logs show migration batches still being processed
...
3. **Check database health** and resources as described in Scenario 1.

4. **Restart the backend service** after adjusting configuration.

### Scenario 3: Transient Timeout Errors During Migration
...
These timeout errors are **expected and transient** during the migration process. They occur because the Liquibase migration is actively writing large amounts of data to ClickHouse.
...
### Scenario 4: Data Missing After Migration
...
- Migration interrupted before completion
...
4. **After reconciliation is complete, disable lazy migration:**
   ```yaml
   DATASET_VERSIONING_MIGRATION_LAZY_ENABLED: "false"
   ```

5. **Restart the backend** to apply the change.
...
2. **Validate data integrity** using the verification queries:
...
3. **Keep lazy migration disabled** unless data reconciliation is needed
...
```
Dataset versioning startup migration: found N unmigrated dataset versions, running migration
Starting dataset version items_total migration with batch size '100'
Processing batch of 'N' dataset versions for items_total calculation
Dataset versioning startup migration: completed successfully, all dataset versions migrated
```
...
If startup migration is disabled:

```
Dataset versioning startup migration is disabled, skipping
```
