...
1. **Classpath defaults** — `llm-models-default.yaml` shipped inside the backend JAR. Always loaded.
2. **Remote CDN YAML** — optional. Fetched from a configurable URL and refreshed on a schedule. Models added here appear without a backend restart.
...
| `LLM_MODEL_REGISTRY_REMOTE_ENABLED` | `false` | Set `true` to fetch from a remote CDN. |
| `LLM_MODEL_REGISTRY_REMOTE_URL` | empty | URL (http/https) of the remote YAML. |
...
Keep `LLM_MODEL_REGISTRY_REMOTE_ENABLED=false` (the default) and rely on the classpath defaults that ship with your Opik version. Upgrade Opik to pick up new models.
...
LLM_MODEL_REGISTRY_REMOTE_URL=https://your-cdn.example.com/opik/llm-providers.yaml
LLM_MODEL_REGISTRY_REFRESH_INTERVAL_SECONDS=3600
```

The backend fetches the YAML on startup and re-fetches every `REFRESH_INTERVAL_SECONDS`. Remote fetch failures are logged and do not affect routing — the last-good registry is retained.
