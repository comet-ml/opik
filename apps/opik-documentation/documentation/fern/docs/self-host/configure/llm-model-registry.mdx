...
1. **Classpath defaults** — `llm-models-default.yaml` shipped inside the backend JAR. Always loaded. Acts as the baseline and the air-gapped fallback.
2. **Remote CDN YAML** — **enabled by default**. Fetched from `https://cdn.comet.ml/opik/llm-models-default.yaml` (regenerated daily by the Opik sync workflow) and refreshed every 5 minutes. This is how new models from providers reach running deployments between Opik releases.
...
| `LLM_MODEL_REGISTRY_REMOTE_ENABLED` | `true` | Set `false` to disable the remote CDN fetch (e.g. air-gapped deployments). |
| `LLM_MODEL_REGISTRY_REMOTE_URL` | `https://cdn.comet.ml/opik/llm-models-default.yaml` | URL (http/https) of the remote YAML. Override to point at your own mirror. |
...
Models are keyed by `id` (or `qualifiedName` for Vertex AI). When a merge happens:
...
Leave the defaults in place. The backend fetches `https://cdn.comet.ml/opik/llm-models-default.yaml` at startup and every 5 minutes, layering it on top of the classpath YAML shipped in the release. New models added by the Opik sync workflow reach your deployment within minutes, without a redeploy.

Remote fetch failures are logged but non-fatal: the backend keeps serving the last successful registry (or the classpath defaults if the first fetch fails).

### Air-gapped deployment

Set `LLM_MODEL_REGISTRY_REMOTE_ENABLED=false` to disable the outbound CDN fetch. The backend will serve only the classpath defaults that ship with your Opik version. Upgrade Opik to pick up new models.

### Point at your own CDN mirror

If corporate policy requires hosting the YAML yourself (e.g. behind an internal proxy), override the URL:
...
Mirror the Opik CDN content (or your own curated variant) at that URL. The backend fetches on startup and re-fetches every `REFRESH_INTERVAL_SECONDS`.
