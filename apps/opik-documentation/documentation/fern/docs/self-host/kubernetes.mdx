...
Example of how you can create read-only ClickHouse users with custom settings profiles:
...
When using Kubernetes secrets for passwords (`adminUser.useSecret.enabled: true`), the password is read from the admin secret using the key `<username>_pass` by default.
