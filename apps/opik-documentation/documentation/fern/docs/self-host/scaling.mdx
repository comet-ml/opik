...
**Opik Backend** — 8–10 pods, autoscaling on memory (80% threshold)
- 3 CPU cores, 5 GB memory per pod
- Horizontal Pod Autoscaler: min 8, max 10

**Opik Python Backend** — 9–12 pods, autoscaling on CPU (85% threshold)
- 1.5 CPU cores, 2 GB memory per pod
- Horizontal Pod Autoscaler: min 9, max 12

**Opik Frontend** — 2–5 pods (minimal resources)
- 50 millicores CPU, 32 MB memory per pod
