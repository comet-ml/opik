...
Opik provides a Kubernetes Job for automated backup restoration. This job handles the complete restore process, including downloading backups from S3 (if needed) and restoring them to ClickHouse.
...
clickhouse:
  backupServer:
    enabled: true  # Required: backup server must be enabled
  backup:
    restore:
      createJob: true
      backupName: "backup-20240101-120000"  # Name of the backup to restore
