...
- On a multi-replica cluster, check **every** replica, not just the one you are connected to. A partially restored replica can look complete at the table level while its replicated tables are stale. `SELECT database, table, is_readonly, absolute_delay FROM system.replicas WHERE database = 'opik'` should show `is_readonly = 0` everywhere; anything read-only or lagging means the schema is not settled and re-baselining would freeze that gap in place.
...
<Steps>
  <Step>
    ...
    ```bash
    kubectl run opik-rebaseline --rm -it --restart=Never \
      --image=<the image your opik-backend deployment uses> \
      --namespace=<your namespace> \
      --overrides='{"spec":{"containers":[{"name":"opik-rebaseline","image":"<same image>","command":["bash"],"stdin":true,"tty":true,"envFrom":[{"secretRef":{"name":"<your opik-backend secret>"}},{"configMapRef":{"name":"<your opik-backend configmap>"}}]}]}}'
    ```
    ...
    Copy the image reference from the failing deployment (`kubectl get deploy opik-backend -o jsonpath='{.spec.template.spec.containers[0].image}'`) rather than using `latest` — the recovery must run against the version the schema was migrated to.
  </Step>
  <Step>
...
Pass `--database db` to re-baseline the MySQL (state) changelog instead of the ClickHouse (analytics) one, and `--yes` to skip the confirmation prompt in an automated runbook.

If you prefer to run the underlying commands directly, the script is a thin wrapper around Dropwizard's migration commands:
...
6. **Back Up the Changelog**: Include ClickHouse's `default` database in backups alongside `opik`. It holds `DATABASECHANGELOG`; restoring data without it leaves the deployment unable to start new replicas. See [Lost Liquibase Changelog with an Intact Schema](#lost-liquibase-changelog-with-an-intact-schema).
