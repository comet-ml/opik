...
<Callout intent="warning">
Only do this when the schema genuinely matches the changelog. Re-baselining asserts "everything up to here is already applied" — if a migration truly has not run, it is marked as done and never will, leaving the schema permanently behind a ledger that claims otherwise. Always review the pending list first.
</Callout>

<Steps>
  <Step>
    Open a shell in a running `opik-backend` pod or container, in its working directory (`/opt/opik`).
  </Step>
  <Step>
