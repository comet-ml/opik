...
-- Restore each Opik table (schema name may vary per installation)
...
<Callout intent="info">
  **Note**: The exact list of tables may vary depending on your Opik version, and the schema name is typically `opik` but may differ in your installation. Use the `\d` command to list all tables in your database and restore each one.
</Callout>
