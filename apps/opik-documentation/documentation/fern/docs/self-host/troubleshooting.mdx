...
- For ClickHouse (`dbAnalytics`, the default) on a multi-replica cluster, check **every** replica, not just the one you are connected to. A partially restored replica can look complete at the table level while its replicated tables are stale:

  ```sql
  SELECT database, table, is_readonly, absolute_delay
  FROM system.replicas
  WHERE database = '<your analytics database>'
  ```

  Substitute your `ANALYTICS_DB_DATABASE_NAME` (`opik` unless you overrode it). Every row should show `is_readonly = 0` and no meaningful `absolute_delay`; anything read-only or lagging means the schema is not settled, and re-baselining would freeze that gap in place.
