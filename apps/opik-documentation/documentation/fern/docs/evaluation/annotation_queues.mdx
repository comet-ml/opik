...
## Managing Queues with Python SDK

You can create and manage annotation queues programmatically using the Python SDK. This is useful for automating the process of adding items to queues based on specific criteria.
...
thread_queue = client.create_annotation_queue(
...
threads = threads_client.search_threads(project_name="my-project")
thread_queue.add_threads(threads[:10])
```
...
queues = client.get_annotation_queues(project_name="my-project")

for queue in queues:
    print(f"Queue: {queue.name}, Scope: {queue.scope}, Items: {queue.items_count}")
```
