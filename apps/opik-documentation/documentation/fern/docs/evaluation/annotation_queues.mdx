...
// Create a thread annotation queue
const threadQueue = await client.createAnnotationQueue({
  name: "Thread Review Queue",
  scope: "thread",
  description: "Threads needing review",
});

// Add threads to the queue using TraceThread objects
// Note: You'll need to obtain TraceThread objects from your backend
...
for (const queue of queues) {
  console.log(
    `Queue: ${queue.name}, Scope: ${queue.scope}, Items: ${queue.itemsCount}`
  );
}
```
