...
- **Method usage**: Both methods of the class must be defined and in order for `generate_provider_response()` to be used in the scoring process, it must be called in `generate_string()`.

### Using the Custom Model with the `Hallucination` Metric

In order to run an evaluation using your Custom Model with the `Hallucination` metric, 
you will first need to instantiate our `CustomOpenAICompatibleModel` class and pass it to the `Hallucination` metric.
...
**Key considerations for the implementation:**
- **ScoreResult Output**: Returns a ScoreResult object containing the metric name (`name`), score value (`value`), optional explanation (`reason`), metadata (`metadata`), and a failure flag (`scoring_failed`).