...
If your model provider exposes an OpenAI-compatible API (e.g. Silicon Flow, Together AI, Anyscale, or a local server like Ollama), you can use it without writing a custom model class. Set two environment variables and prefix the model name with `openai/`:
...
metric = Hallucination(model="openai/deepseek-ai/DeepSeek-V3")
...
The `openai/` prefix tells LiteLLM to route the request through the OpenAI-compatible API at `OPENAI_BASE_URL`. Replace the model name after the prefix with whatever model identifier your provider expects.
