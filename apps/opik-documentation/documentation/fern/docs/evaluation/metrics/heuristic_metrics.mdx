...
| ReadabilityGuard | Deterministic readability guard using Flesch–Kincaid bounds. |
| RegexMatch | Validates the output against a regular expression pattern. |
| ROUGE | Calculates ROUGE variants (rouge1, rouge2, rougeL, rougeLsum, rougeW). |
...
| ToneGuard | Flags tone issues such as negativity, shouting, or forbidden phrases. |

### Conversation heuristics

| Metric | Description |
| --- | --- |
| BLEUC | Aggregates sentence-level BLEU across assistant turns (implemented by `BleuConversationMetric`). |
| DegenerationC | Detects repetition and low-entropy responses over a conversation (implemented by `ConversationDegenerationMetric`). |
| Knowledge Retention | Checks whether the last assistant reply preserves user-provided facts from earlier turns. |
| METEORC | Aggregates METEOR scores across assistant turns (implemented by `MeteorConversationMetric`). |
| ROUGEC | Aggregates ROUGE scores across assistant turns (implemented by `RougeConversationMetric`). |
...
> The `*C` names map to classes such as `RougeConversationMetric`, `BleuConversationMetric`, and `MeteorConversationMetric`.
...
`ROUGE` supports multiple variants out of the box: `rouge1`, `rouge2`, `rougeL`, `rougeLsum`, and `rougeW`. You can switch variants via the `rouge_type` argument and optionally enable stemming or sentence splitting.
...
`ReadabilityGuard` enforces Flesch–Kincaid grade boundaries. Set `min_grade` and/or `max_grade` to constrain the acceptable reading level.

```python
from opik.evaluation.metrics import ReadabilityGuard

metric = ReadabilityGuard(min_grade=6, max_grade=10)
...
`ToneGuard` flags outputs that sound aggressive, negative, or violate a list of forbidden phrases. You can tweak sentiment thresholds, uppercase ratios, and exclamation limits.

```python
from opik.evaluation.metrics import ToneGuard

metric = ToneGuard(max_exclamations=1)
