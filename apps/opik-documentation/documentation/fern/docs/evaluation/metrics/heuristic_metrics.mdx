...
| ROUGEC | Aggregates ROUGE scores across assistant turns (implemented by `RougeCMetric`). |
| BLEUC | Aggregates sentence-level BLEU across assistant turns (implemented by `BleuCMetric`). |
| METEORC | Aggregates METEOR scores across assistant turns (implemented by `MeteorCMetric`). |
...
> The `*C` names map to classes such as `RougeCMetric`, `BleuCMetric`, and `MeteorCMetric`.
