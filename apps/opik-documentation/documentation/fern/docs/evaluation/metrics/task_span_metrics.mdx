...
Task span metrics are evaluation metrics that include a `task_span` parameter in their `score` method. The Opik evaluation engine automatically detects these metrics by inspecting the `score` method signature and handles them differently from regular metrics.
