...
| `ConversationPromptPerplexityMetric` | Rates how difficult the prompt is for downstream models. | `PromptPerplexityJudge` |
...
### ConversationPromptPerplexityMetric {#conversationpromptperplexitymetric}
Estimates how challenging the conversation’s latest user request will be for your model family. Higher scores correlate with vague or technically complex questions that historically yield weaker answers. Use this signal to decide when to route conversations to higher-capability models (or to human agents) before investing compute on elaborate reasoning chains.
