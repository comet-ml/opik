...
### RevisEval Judge
Runs a retrieval-augmented verification pass before scoring. The judge may suggest revisions based on the fetched evidence, making it handy when you need self-correction feedback alongside a score.

```python title="RevisEval"
from opik.evaluation.metrics import RevisEvalJudge

metric = RevisEvalJudge()
score = metric.score(
    input="Document: ...",
    output="Summary: ...",
)
...
Because RevisEval performs retrieval internally, ensure you configure API credentials for the underlying search provider.
