...
perplexity = PromptPerplexityJudge().score(prompt=prompt)
uncertainty = PromptUncertaintyJudge().score(prompt=prompt)
print(perplexity.value, uncertainty.value)
```

Combining both metrics helps identify prompts that are simultaneously complex and underspecified, alerting product flows before the final model produces a disappointing answer. Each judge emits an integer from 0 (best) to 10 (worst) prior to normalisation.
