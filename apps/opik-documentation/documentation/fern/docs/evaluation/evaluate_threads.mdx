...
<Note>
  You can evaluate and score threads at any time, regardless of whether new traces are still being added.
  For online evaluation rules (automatic scoring), Opik waits for a configurable "cooling period" after
  the last activity before running the rules.
</Note>
