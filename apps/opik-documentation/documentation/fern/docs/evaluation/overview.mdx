...
```python
import opik
from opik.evaluation import evaluate_prompt, report
from opik.evaluation.metrics import Hallucination
# Create a dataset that contains the samples you want to evaluate
opik_client = opik.Opik()
dataset = opik_client.get_or_create_dataset("Evaluation test dataset")
...
# Or print the formatted report into the console
report.display_evaluation_scores_statistics(dataset.name, evaluation_results=result)
```
