...
You can access version metadata and retrieve items from specific versions using the SDK:
...
print(f"Current version: {version_info.version_name}")

# Access a specific version
v1 = dataset.get_version_view("v1")
print(f"Version: {v1.version_name}, Items: {v1.items_total}")

# Retrieve items from a specific version
items = v1.get_items()
```
...
console.log(`Current version: ${versionInfo?.versionName}`);

// Access a specific version
const v1 = await dataset.getVersionView("v1");
console.log(`Version: ${v1.versionName}, Items: ${v1.itemsTotal}`);

// Retrieve items from a specific version
const items = await v1.getItems();
```
</CodeBlocks>

### Working with dataset versions programmatically

The Python SDK provides methods for inspecting and working with dataset versions:
