...
<CodeBlocks>
  ...
  # Use filtered items in evaluation
  from opik.evaluation import evaluate_prompt
  results = evaluate_prompt(
      dataset=dataset,
      dataset_filter_string='tags contains "failed"',
...
You can pass `dataset_filter_string` to `evaluate_prompt()` to evaluate only a subset of dataset items:
