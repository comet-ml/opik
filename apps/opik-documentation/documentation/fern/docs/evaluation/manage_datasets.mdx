...
<Note>
  Image fields are automatically detected: when a column contains base64 data or an image URL, the dataset table renders
  a thumbnail while keeping the raw value available for copy/download.
</Note>
