...
You can retrieve a filtered view of a dataset by passing a `filter_string` parameter to the `get_dataset` method.
When a filter is specified, a `DatasetView` object is returned instead of a regular `Dataset`. The `DatasetView`
is a read-only view that only includes items matching your filter criteria.
...
<CodeBlocks>
  ...
  # Use the filtered view in evaluation
...
<CodeBlocks>
  ...
  specific_category = client.get_dataset(
      name="my_dataset",
      filter_string='data.category = "finance"'
  )

  # Filter by date
  recent_items = client.get_dataset(
      name="my_dataset",
      filter_string='created_at >= "2024-06-01T00:00:00Z"'
  )

  # Multiple conditions
  complex_filter = client.get_dataset(
      name="my_dataset",
...
### DatasetView behavior

The `DatasetView` object is read-only to ensure data integrity. Attempting to modify
a `DatasetView` will raise a `DatasetViewImmutableError`:
...
<CodeBlocks>
  ...
  client = Opik()
  filtered_view = client.get_dataset(
      name="my_dataset",
      filter_string='tags contains "test"'
  )

  # ❌ This will raise DatasetViewImmutableError
  filtered_view.insert([{"input": "new item"}])

  # ❌ This will also raise DatasetViewImmutableError
  filtered_view.delete(["item-1"])
  ```
</CodeBlocks>

### Converting a DatasetView to a Dataset

If you need to create a mutable dataset from a filtered view, use the `to_dataset()` method:
...
<CodeBlocks>
  ...
  # Convert to a new mutable dataset
  new_dataset = failed_tests.to_dataset(
      name="failed_tests_copy",
      description="Copy of failed test cases for investigation"
  )

  # Now you can modify the new dataset
  new_dataset.insert([{"input": "new test", "expected_output": "result"}])
  ```
</CodeBlocks>

<Tip>
  The `to_dataset()` method creates a new dataset on the backend and copies all items from
  the filtered view. This is useful when you want to create a subset of your data for
  focused work or archival purposes.
</Tip>
