...
1. **Plain Scoring Functions**: Use `scoring_inputs` and `task_outputs` parameters
...
def custom_equals_scorer(
    ...
    Args:
        scoring_inputs: Data from the dataset item (includes expected outputs)
        task_outputs: Outputs from the evaluation task
    """
    expected = scoring_inputs.get("expected_output")
...
def comprehensive_scorer(
    ...
    Args:
        scoring_inputs: Dataset item data
