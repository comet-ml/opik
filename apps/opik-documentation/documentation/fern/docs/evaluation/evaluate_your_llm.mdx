...
Opik supports advanced evaluation metrics that can analyze the detailed execution information of your tasks. These metrics receive a `task_span` parameter containing structured data about the task execution, including input, output, metadata, and nested operations.
...
class TaskSpanQualityMetric(BaseMetric):
    def __init__(
        self,
        name: str = "task_span_quality",
        track: bool = True,
    ...
    def score(self, task_span: SpanModel) -> score_result.ScoreResult:
        # Access detailed span information
        span_name = task_span.name
        input_data = task_span.input
        output_data = task_span.output
        metadata = task_span.metadata

        # Example: Check if the task executed successfully
        has_error = task_span.error_info is not None

        # Example: Analyze input complexity
        input_complexity = len(str(input_data)) if input_data else 0

        # Custom scoring logic
        if has_error:
            score_value = 0.0
            reason = "Task execution failed"
        elif input_complexity > 1000:
            score_value = 0.8
            reason = "High complexity input handled well"
        else:
            score_value = 1.0
...
# Another example: Check specific task characteristics
...
quality_metric = TaskSpanQualityMetric()
...
evaluation = evaluate(
    ...
    scoring_metrics=[
        ...
        quality_metric,       # Task span metric
...
class HierarchyAnalysisMetric(BaseMetric):
    def score(self, task_span: SpanModel) -> score_result.ScoreResult:
        ...
        nested_span_count = len(task_span.spans)

        # Count different types of operations
        llm_spans = [s for s in task_span.spans if s.type == "llm"]
        tool_spans = [s for s in task_span.spans if s.type == "tool"]

        # Analyze the complexity of the operation
        total_operations = nested_span_count
        llm_operations = len(llm_spans)

        if total_operations == 0:
            score = 0.5
            reason = "Simple operation with no nested steps"
        elif llm_operations > 3:
            score = 0.7
            reason = f"Complex operation with {llm_operations} LLM calls"
        else:
            score = 1.0
            reason = f"Well-structured operation with {total_operations} steps"
