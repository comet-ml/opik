...
class MyDatasetSampler(samplers.BaseDatasetSampler):
    ...
    def sample(self, dataset: List[dataset_item.DatasetItem]) -> List[dataset_item.DatasetItem]:
        # Sample max_samples items from the dataset
        return dataset[:self.max_samples]
```

Implementing your own dataset sampler is useful if you want to implement a custom sampling strategy. For example,
you can implement a dataset sampler that samples the dataset using some filtering criteria.
