...
You can evaluate only a subset of your dataset items by using the `filter_string` parameter. This is useful when you want to run experiments on specific categories of data:
...
<CodeBlocks>
    ...
    evaluate_prompt(
        ...
        model="gpt-4",
        filter_string='data.category = "finance" AND data.difficulty = "hard"',
    )
    ```

</CodeBlocks>

The filter uses Opik Query Language (OQL) syntax. For more details on filter syntax and supported columns, see [Filtering datasets programmatically](/evaluation/manage_datasets#filtering-datasets-programmatically).
