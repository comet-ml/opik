...
It’s important to note that otel auto-instrumentations are not familiar with Opik, so you may need to set attributes manually, like the `output` attribute in the example above.
