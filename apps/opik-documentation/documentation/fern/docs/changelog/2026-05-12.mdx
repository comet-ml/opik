...
## 🏠 Recent Activity Feed on Project Home Page

The project home page now shows a **Recent Activity** feed, surfacing the latest experiments, optimization runs, dataset versions, agent config blueprints, and alert events across your project in a single, time-sorted list.

**Why it matters:** Instead of clicking into each sub-section to find out what happened, you get a quick overview of the project's state the moment you land on the home page.
...
## 🐍 Python SDK Reliability Improvements

We fixed two separate bugs that could silently drop traces under load:

- **Streamer race condition** — A lock gap between the flushing thread and the message batcher meant 1–22% of traces could be silently dropped under bursty traffic. Fixed by serializing flushes with the batch lock.
- **Drain-time attachment loss** — When `client.end()` was called, attachment re-enqueue messages were hitting a closed drain gate and being dropped, losing ~5–6% of data. Fixed by letting internal re-enqueues bypass the drain guard.

Additionally:
- **Auto-retry on rate limits** — `search_traces()` and `search_spans()` now automatically wait and retry on 429 responses instead of raising an error, so large bulk searches complete reliably even under API rate limits
