...
For online scoring rules on threads, Opik waits for a "cooling period" after the last activity in a thread
before running the evaluation. This ensures the scoring is done on the full context of the conversation.

<Note>
  The default cooling period is 15 minutes but can be configured at the workspace level under "Online scoring cooling period".
