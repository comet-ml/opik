...
<Tip>
  Online evaluation metrics allow you to score all (or a sample) your production traces and easily identify any issues with your
  production LLM application.
</Tip>

When working with LLMs in production, the sheer number of traces means that it isn't possible to manually review each trace. 
Opik allows you to define rules to automatically score the LLM calls logged to the platform in two powerful ways:
1. **LLM as a Judge**: Use language models to evaluate outputs for qualities like relevance, hallucination, and moderation
2. **Python Metrics**: Create custom scoring logic using Python for precise, deterministic evaluation

![Opik Online Evaluation](/img/production/online_evaluation.gif)
...
## Creating a LLM as a Judge Scoring Rule
...
## Creating a Python Code Scorer 

<Frame>
  <img src="/img/production/online_evaluation_python_code.png" />
</Frame>

When creating a new Python metrics rule, you will be presented with the following options:

1. **Name:** The name of the rule
2. **Sampling rate:** The percentage of traces to score. When set to `1`, all traces will be scored.
3. **Code:** Your code needs to extend our [BaseMetric](https://github.com/comet-ml/opik/blob/main/sdks/python/src/opik/evaluation/metrics/base_metric.py) class.
4. **Variable mapping:** This is the mapping of the variables in the prompt to the values from the trace.

On the example above, we're creating a rule to run over 25% of the traces in our project, checking the similarity between the input query and the output.

### Running Python Metrics in Comet Cloud

When using Comet Cloud, the code scorer will be executed in a sandbox environment with [selected set of Python libraries](https://github.com/comet-ml/opik/blob/main/apps/opik-sandbox-executor-python/requirements.txt) and a 3 second timeout. Network and file system access is disabled.

### Running Python Metrics in Self-hosted

You can decide using either a sandbox environment (via Docker containers) or spawning a new process to execute the code scorer. Both allows customised timeout, and the latter has not libraries or network access restrictions. The most important parameters are:
```
PYTHON_CODE_EXECUTOR_STRATEGY: docker | process
...
The scores returned by the online evaluation rules will be stored as feedback scores for each trace. 

<Frame>
  <img src="/img/production/online_evaluation_python_score.png" />
</Frame>

This will allow you to review these scores in the traces sidebar and track their changes over time in the Opik dashboard.
