...
Navigate to Configuration → Alerts to see all configured alerts. The table displays:

- **Name**: Alert name
- **Endpoint**: Webhook URL (truncated)
- **Events**: Number of trigger types configured
- **Created by**: User who created the alert
- **Status**: Enabled or disabled

<Frame>
  <img src="/img/production/alerts_list.png" alt="Alerts list view" />
</Frame>

### Editing alerts

1. Click on an alert row or the edit icon
2. Modify any configuration settings
3. Click "Save" to update the alert

**Note**: Updating an alert replaces all its triggers and configurations. Make sure to preserve any settings you want to keep.
...
1. Edit the alert
2. Toggle off the "Enable alert" switch
3. Save the alert

Disabled alerts will not send webhooks but remain in your configuration for future use.
...
Deleted alerts cannot be recovered. Consider disabling instead if you might need them later.
