...
2. **In Opik**:
   ...
   - Enter the Feishu bot webhook URL: `https://open.feishu.cn/open-apis/bot/v2/hook/...`
   - Add triggers for the events you want to monitor
   - Click "Test connection" to verify
   - Click "Create alert"

Opik will automatically format all alert payloads into Feishu interactive cards with:
- Color-coded headers by severity (red for errors, orange for metric-threshold and guardrail events, blue for informational events)
