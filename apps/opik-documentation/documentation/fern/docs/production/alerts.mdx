...
Opik provides four destination types for alerts:
...
- **Feishu**: Native integration with automatic interactive-card formatting for Feishu (Lark)
...
3. **Configure webhook settings**
   - **Destination**: Select the alert destination type:
     ...
     - **Feishu**: For native Feishu (Lark) integration (automatically formats events as interactive cards)
   - **Endpoint URL**: Enter your webhook URL (must start with `http://` or `https://`)
     ...
     - For Feishu: Use your Feishu group bot webhook URL (e.g., `https://open.feishu.cn/open-apis/bot/v2/hook/...`)
...
1. **Native integrations** (Slack, PagerDuty, Feishu): Use built-in formatting for popular services - no middleware required
...
### Feishu integration (Native)

Opik provides native Feishu (Lark) integration that automatically formats alert events as [interactive cards](https://open.feishu.cn/document/common-capabilities/message-card/message-cards-content) delivered to a group chat via a custom bot webhook.

#### Prerequisites
- A Feishu (Lark) group chat where you can add a custom bot
- Permission to create custom bots in the group

#### Setup steps

1. **In Feishu (Lark)**:
   - Open the target group chat → Settings → Bots
   - Click "Add Bot" and choose "Custom Bot"
   - Give the bot a name (e.g., "Opik Alerts") and copy the generated Webhook URL

2. **In Opik**:
   - Go to Configuration → Alerts tab
   - Click "Create new alert"
   - Give your alert a descriptive name
   - Select **Feishu** as the destination type
   - Enter the custom bot webhook URL:
     - Feishu (China): `https://open.feishu.cn/open-apis/bot/v2/hook/...`
     - Lark (international): `https://open.larksuite.com/open-apis/bot/v2/hook/...`
   - Add triggers for the events you want to monitor
   - Click "Test connection" to verify
   - Click "Create alert"

Opik will automatically format all alert payloads into Feishu interactive cards with:
- Color-coded headers by severity (red for trace errors, orange for other threshold and guardrail events such as cost, latency, and feedback score alerts, blue for informational events)
- Human-readable event summaries and details
- "View in Opik" action buttons linking back to the relevant page

### Custom integration with middleware service (Optional)

For more complex integrations or custom formatting requirements, you can use a middleware service to transform Opik's payload before sending it to your destination. This approach works with any destination type (General, Slack, PagerDuty, or Feishu).
...
4. Configure your middleware to forward to the final destination (Slack, PagerDuty, Feishu, etc.)
...
- Explore [Online Evaluation Rules](/v1/production/rules) for automated model monitoring
- Learn about [Guardrails](/v1/production/guardrails) for proactive risk detection
- Review [Production Monitoring](/v1/production/production_monitoring) best practices
