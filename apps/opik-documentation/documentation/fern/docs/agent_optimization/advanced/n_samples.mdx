...
When you set `n > 1` in your prompt's `model_kwargs`, the optimizer:

1. Requests N completions from the LLM in a single API call
...
Set the `n` parameter in your `ChatPrompt.model_kwargs`:
...
<AccordionGroup>
  <Accordion title="Reducing Evaluation Variance">
    Single-sample evaluation is noisy. With `n=3`, if even one of three candidates passes the metric, the prompt gets credit. This makes optimization more robust to stochastic failures.
    ...
    prompt = ChatPrompt(
        model="gpt-4o-mini",
        messages=[...],
        model_kwargs={"n": 3, "temperature": 0.8},
    )
  ...
  <Accordion title="Handling Stochastic Tasks">
    ...
    prompt = ChatPrompt(
        ...
        model_kwargs={"n": 3, "temperature": 1.0},
    )
    ```
  </Accordion>
</AccordionGroup>
...
Currently, the optimizer uses `best_by_metric` selection:
...
- `candidate_scores`: List of all scores
...
<AccordionGroup>
  <Accordion title="Tool-calling forces n=1">
    ...
    prompt = ChatPrompt(
        ...
        messages=[...],
        tools=[...],
        model_kwargs={"n": 3},  # Ignored when tools are used
    )
    ```
  </Accordion>
