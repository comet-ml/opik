...
- **Deterministic** – cache external model calls, set temperature to 0 and a seed value (in supported models) so repeated runs match.
...
- **LLM-as-a-judge** – call out to an evaluation model (OpenAI, Anthropic, etc.) inside the metric. Always  include detailed prompts so results stay stable.
