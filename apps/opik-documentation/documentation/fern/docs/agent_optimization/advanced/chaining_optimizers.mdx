...
| --- | --- | --- | --- | --- |
| Hierarchical Reflective → Parameter | Existing long/complex prompt senario: Reflective analysis finds failure modes; Parameter optimizer then tightens parameters on the improved prompt. | Excellent for legacy prompts with lots of existing complexity. Helps produces explainable changelog. | Requires metrics with rich `reason` strings; two stages increase cost. | Medium |
...
- **Reuse OptimizationResult** – every optimizer returns an `OptimizationResult`, so you can pass `result.prompt` directly into the next stage without rewriting glue code.
