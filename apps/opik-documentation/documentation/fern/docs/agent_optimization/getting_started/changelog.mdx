...
| --- | --- | --- |
| **2026-08-04** | `3.2.0` | Optimizer-internal LLM calls are now always traced and attributable: a call made with no span open opens its own trace and carries the optimization id as a tag, so optimizer spend that belongs to no trial (notably GEPA reflection) is priced and counted against the run. Replaces the `opik_monitor` logger that double-logged alongside `track_completion`. Adds GEPA reflection budget controls, a typed optimizer exception hierarchy, and thread-safe usage/cost accumulators for concurrent runs. **Dependency change — upgrade both together:** `pip install -U "opik-optimizer>=3.2.0" "gepa>=0.1.0"`. GEPA 0.1.0 renamed the reflection-template markers to `<curr_param>`/`<side_info>` and added the `reflection_prompt_template` kwarg, which this release requires. Do not raise `gepa` on its own: `opik-optimizer` 3.1.1 and earlier declare `gepa>=0.0.7` with no upper bound, so `pip install -U gepa` is accepted and then fails at runtime on the old `<curr_instructions>`/`<inputs_outputs_feedback>` dialect. Optimization Studio pins its own optimizer version and picks this up when that pin is raised, not when you upgrade locally. |
| **2026-04-16** | `3.1.1` | GEPA compatibility fix ([#6328](https://github.com/comet-ml/opik/pull/6328)). Backfilled: this release shipped without a changelog row. |
...
<Info>
  Looking for older updates? Browse the [full Opik changelog](/v1/changelog) and filter by the **Agent Optimization** tag.
</Info>
