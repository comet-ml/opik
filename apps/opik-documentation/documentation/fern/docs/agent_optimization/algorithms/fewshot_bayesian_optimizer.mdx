---
title: "Few-Shot Bayesian Optimizer: Example-Based Chat Prompt Tuning"
---
...
<Tip>
  The core of this optimizer relies on robust evaluation (Step 4) where each combination of few-shot examples is assessed using your `MetricConfig` against the `dataset`. Understanding Opik's evaluation framework is key to effective use:
...
```python
import sys; print(f"sys.path inside pytest code block: {sys.path}")
