...
# Cost = 12 trials × 5 samples = 60 metric calls
...
# Cost = 60 trials × 12 samples = 720 metric calls
...
- Budget: `max_trials` determines how many different prompts to test. The total metric calls = `max_trials × n_samples`.
...
opt_result = optimizer.optimize_prompt(
    ...
    max_trials: int = 10,  # Number of different prompts to test
...
- Low improvements: Increase `max_trials` and/or `n_samples`, try a stronger `reflection_model`, or adjust the starting system prompt.
