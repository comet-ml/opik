...
Tool optimization is a specialized feature that allows you to optimize prompts that use external tools and the Model Context Protocol (MCP). This capability is currently **exclusively supported by the MetaPrompt Optimizer**.
...
<Note>
  **Important Distinction:** Many optimizers (including GEPA, MetaPrompt, etc.) can optimize **agents that use tools** -
  ...
  through the MetaPrompt Optimizer.
</Note>
...
**True tool optimization** is currently only available for MCP tools through the MetaPrompt Optimizer. MCP tools provide standardized interfaces for external integrations:

```python
# MCP tool optimization example
# See scripts/archive/litellm_metaprompt_context7_mcp_example.py for working examples
...
MCP Tool Optimization:
    ...
    - See [PR#3341](https://github.com/comet-ml/opik/pull/3341) for implementation details
    - Working examples available in `scripts/archive/litellm_metaprompt_context7_mcp_example.py`
