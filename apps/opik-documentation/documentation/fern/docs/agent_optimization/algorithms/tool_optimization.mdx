...
description: Learn how to optimize prompts that leverage MCP (Model Context Protocol)
  tools and function calling capabilities across supported optimizers.
...
Tool optimization is a specialized feature that allows you to optimize prompts that use external tools and the Model Context Protocol (MCP). This capability is supported by all optimizers **except** `FewShotBayesianOptimizer` and `ParameterOptimizer`.
...
# Any supported optimizer works here (e.g., HRPO, Evolutionary, GEPA).
```

<Note>
MCP Tool Optimization:

    - Supported by all optimizers **except** FewShotBayesianOptimizer and ParameterOptimizer
