...
- **`n_samples`** (int, float, percent string, or "all"/"full"/`None`, optional): Number of dataset items to use per evaluation (uses the validation dataset when provided). Use counts (e.g., `50`), fractions (e.g., `0.1`), percentages (e.g., `"10%"`), or `"all"`/`"full"`/`None` for the full dataset.
- **`n_samples_minibatch`** (int, optional): Optional number of samples for inner-loop minibatches (defaults to `n_samples`).
- **`n_samples_strategy`** (str, optional): Sampling strategy for subsampling (default: `"random_sorted"`).
