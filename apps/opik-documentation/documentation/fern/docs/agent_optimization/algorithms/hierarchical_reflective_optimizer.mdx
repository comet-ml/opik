...
<ParamField path="agent" type="OptimizableAgent | None" optional={true}>Optional custom agent instance for executing prompts. If not provided, a default LiteLLM-based agent is used. Pass an **instance** of your agent class (e.g., `agent=MyAgent()`), not the class itself.</ParamField>
