---
description: Learn how to use HAPO (Hierarchical Adaptive Prompt Optimizer) to improve prompts
  through systematic root cause analysis of failure modes and targeted refinement.
headline: HAPO | Opik Documentation
og:description: Learn how HAPO (Hierarchical Adaptive Prompt Optimizer) enhances prompt refinement
  by addressing specific failure modes through systematic analysis.
og:site_name: Opik Documentation
og:title: HAPO (Hierarchical Adaptive Prompt Optimizer) - Opik
subtitle: Hierarchical root cause analysis for targeted prompt improvement
title: HAPO (Hierarchical Adaptive Prompt Optimizer)
---

`HAPO` (Hierarchical Adaptive Prompt Optimizer) uses hierarchical root cause analysis to identify and address
...
<Note>
  `HAPO` is ideal when you have a complex prompt that you want to refine
...
HAPO (Hierarchical Adaptive Prompt Optimizer) has been developed by the Opik team to improve prompts that
...
<Frame>
  <img src="/img/agent_optimization/hierarchical_reflective_optimizer.png" alt="HAPO (Hierarchical Adaptive Prompt Optimizer)" />
</Frame>
...
You can use `HAPO` to optimize a prompt:

```python maxLines=1000
from opik_optimizer import HAPO, ChatPrompt, datasets
...
There are two models to consider when using `HAPO`:
- `HAPO.model`: The model used for the root cause analysis and failure mode synthesis.
