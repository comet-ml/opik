...
optimizer = HierarchicalReflectiveOptimizer(
    ...
    n_threads=12,                         # Parallel threads for evaluation
...
- `n_threads`: Number of parallel threads used for evaluating prompts against the dataset. Higher values speed up evaluation but increase concurrent API calls.
...
- **Solution:** Reduce `n_samples`, increase `n_threads`, or decrease `max_parallel_batches` to balance speed vs. thoroughness.

**Issue:** Rate limiting errors

- **Solution:** Decrease `max_parallel_batches` and `n_threads` to reduce concurrent API calls.
