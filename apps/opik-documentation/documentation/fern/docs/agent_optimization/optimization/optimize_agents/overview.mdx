...
class ADKAgent(OptimizableAgent):
    """Custom agent that integrates with Google's Agent Development Kit (ADK)."""

    def __init__(self, project_name: str = "adk-agent"):
        super().__init__()
        self.project_name = project_name
        # Initialize your ADK client or any framework-specific setup here
...
To use your custom agent with the optimizer, you need to **create an instance** of your agent class and pass it using the `agent` parameter:
...
custom_agent = ADKAgent(project_name="my-adk-project")

optimizer = MetaPromptOptimizer(model="openai/gpt-4o")
result = optimizer.optimize_prompt(
    prompt=prompt,
    agent=custom_agent,  # Pass the agent instance (not the class)
    dataset=dataset,
    metric=levenshtein_ratio,
    max_trials=5,
    n_samples=50
...
**Key points:**
- Create an instance of your agent: `custom_agent = ADKAgent()`
- Pass the instance using `agent=custom_agent` (not `agent_class=`)
...
When optimizing a multi-prompt workflow, you need to:
1. Define a dictionary of prompts (each prompt handles one step)
2. Create an instance of your custom agent
3. Pass both to the optimizer
...
# Create an instance of your custom agent
agent = AnalyzeRespondAgent(model="gpt-4o-mini")

optimizer = MetaPromptOptimizer(model="openai/gpt-4o")
result = optimizer.optimize_prompt(
    prompt=prompts,  # Pass dictionary of prompts
    agent=agent,  # Pass your agent instance (not the class)
    dataset=dataset,
    metric=levenshtein_ratio,
    max_trials=5,
    n_samples=50
...
The optimizer will test different versions of both prompts, trying various combinations to find what works best together.
...
class TwoStepAgent(OptimizableAgent):
    """A two-step agent that first analyzes, then responds."""
    ...
    def invoke_agent(
        ...
        # Second step: Generate answer using the analysis
        step2_prompt = prompts["step2"]
        # Pass the analysis result to the second prompt
...
# Define the two prompts in your workflow
prompts = {
    "step1": ChatPrompt(
...
agent = TwoStepAgent(model="gpt-4o-mini")
