...
## Existing Examples
Use the sub-guides for framework-specific guides:
...
Every optimizer ships with a default base LiteLLM agent under the hood, so even a basic prompt optimization is setup like an agent in the SDK. You are able to modify this default behaviour, you can plug in your own by subclassing `opik_optimizer.optimizable_agent.OptimizableAgent`:
