...
Datsets are a crucial component of the optimizer SDK as its key component to run and evaluate (score) each dataset item using optimizers to develop a better outcome. Without datasets its not possible to steer the optimizer on what is good and bad.
