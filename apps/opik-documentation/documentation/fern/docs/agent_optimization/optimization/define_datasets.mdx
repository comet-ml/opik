...
Datasets are a crucial component of the optimizer SDK as it's a key component to run and evaluate (score) each dataset item using optimizers to develop a better outcome. Without datasets it's not possible to steer the optimizer on what is good and bad.
...
- If rows include multimodal data, confirm they appear in the trace tree once you run an optimization.
- Run an initial small batch optimization with a few rows of data to validate.
