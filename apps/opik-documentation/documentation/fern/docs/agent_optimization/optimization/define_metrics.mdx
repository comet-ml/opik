...
<Note>
  Span-based metrics like `TotalSpanCost` and `TotalSpanDuration` automatically receive the `task_span` parameter during evaluation, which contains execution information about the agent's run. Use negative weights to minimize metrics (cost, duration) rather than maximize them.
</Note>
