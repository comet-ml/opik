...
cost = TotalSpanCost(target=0.01, name="cost")
duration = SpanDuration(target=6.0, name="duration")
...
| `TotalSpanCost` | `from opik_optimizer.metrics import TotalSpanCost` | Minimize token/call cost from the task span. |
| `SpanDuration` | `from opik_optimizer.metrics import SpanDuration` | Minimize total task runtime from the task span. |
