...
- Working examples to reference:
  - `sdks/opik_optimizer/scripts/llm_frameworks/mcp` – current MetaPrompt tool-calling example that powers docs/tests.
  - `sdks/opik_optimizer/scripts/archive/litellm_evolutionary_context7_mcp_example.py` – (historical example, for reference only; not recommended for new projects) evolutionary workflow that shows raw LiteLLM integration.
...
- Run the scripts under `sdks/opik_optimizer/scripts/llm_frameworks/mcp` whenever you upgrade the SDK—they double as regression tests to confirm MCP optimizations still work.
