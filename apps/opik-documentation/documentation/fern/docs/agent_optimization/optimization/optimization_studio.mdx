...
Choose how Opik should search for better prompts. GEPA works well for single-turn prompts and quick improvements, while HRPO is better when you need deeper analysis of why a prompt fails. If you are new, start with GEPA to get a quick baseline, then switch to HRPO if you need deeper insight. For technical details, see [Optimization algorithms](/v1/agent_optimization/algorithms/overview).

### Choose a dataset

Pick an existing dataset to supply examples. Aim for diverse, real-world cases rather than edge cases only, and keep the first run small so you can iterate quickly. If you need to create or upload data first, see [Manage datasets](/v1/evaluation/manage_datasets).
### Define a metric
...
If you want to automate optimizations in code later, follow [Optimize prompts](/v1/agent_optimization/optimization/optimize_prompts) and use the same dataset and metric from this run.

## Next steps

For a deeper breakdown of trials and traces, visit [Dashboard results](/v1/agent_optimization/optimization/dashboard_results). If you want to automate this workflow, use [Optimize prompts](/v1/agent_optimization/optimization/optimize_prompts). To fine-tune your strategy, explore [Optimization algorithms](/v1/agent_optimization/algorithms/overview).
