...
<AccordionGroup>
  ...
  <Accordion title="litellm → OpenAI connection errors (1.81.x)">
    `litellm.InternalServerError: OpenAIException - Connection error.` has been reproducible against LiteLLM `1.81.1`. These releases can break the OpenAI evaluation flow inside Opik Optimizer.
