---
title: Opik Optimizer FAQ: Your Prompt & Agent Optimization Questions Answered
---
...
<AccordionGroup>
  <Accordion title="General Optimizer & SDK Questions">
    <Tip>
    ...
    **Q: What are `TaskConfig` and `MetricConfig`? Why are they important?**
    ...
    **Q: How does `project_name` in the optimizer constructor work?**
    ...
    **Q: What does `n_samples` in `optimizer.optimize_prompt(...)` do?**
    ...
    **Q: How can I view the results of an optimization?**
    ...
    - All optimization runs are also logged to the Opik platform, where you can find detailed dashboards, compare trials, and view the evolution of prompts. (Link to relevant UI page if available, e.g., by referencing the image in quickstart `trial_dashboard.png`).
  ...
  <Accordion title="MetaPromptOptimizer">
    **Q: When is `MetaPromptOptimizer` a good choice?**
    ...
    **Q: What's the difference between `model` and `reasoning_model`?**
    ...
    **Q: How do `max_rounds` and `num_prompts_per_round` affect optimization?**
    ...
    **Q: How does `adaptive_trial_threshold` work?**
  ...
  <Accordion title="FewShotBayesianOptimizer">
    ...
    **Q: How do `min_examples`, `max_examples`, and `n_iterations` influence the search?**
    ...
    **Q: Does it optimize the `instruction_prompt` itself?**
  ...
  <Accordion title="MiproOptimizer">
    **Q: When is `MiproOptimizer` suitable?**
    ...
    **Q: How does `MiproOptimizer` relate to DSPy?**
    ...
    **Q: What does `num_candidates` in `optimize_prompt` control?**
    ...
    **Q: Can I use `MiproOptimizer` if I'm not familiar with DSPy?**
  ...
  <Accordion title="EvolutionaryOptimizer">
    ...
    **Q: What does `enable_moo` (multi-objective optimization) do?**
    ...
    **Q: How do `population_size` and `num_generations` impact the process?**
    ...
    **Q: What is `output_style_guidance` and when should I use it?**
    ...
    **Q: What's the difference between standard genetic operators and LLM-driven ones (`enable_llm_crossover`)?**
...
## Model Configuration (Legacy - see LiteLLM Support)

<Accordion title="Q: What should I pass in the model parameter when using Azure OpenAI? (Legacy)">
  A: **This information is for context. For current best practices, please see our [LiteLLM Support for Optimizers guide](/agent_optimization/opik_optimizer/litellm_support).**

  Previously, for Azure OpenAI, you would use a format like:
  ```python {pytest_codeblocks_skip=true}
  optimizer = FewShotBayesianOptimizer( # Corrected class name
      model="azure/gpt-4",  # For Azure GPT-4
      # ...
  )
  ```
  Supported Azure OpenAI models were `azure/gpt-4` and `azure/gpt-3.5-turbo`. For regular OpenAI: `openai/gpt-4`, `openai/gpt-3.5-turbo`. Now, LiteLLM handles all such configurations.
</Accordion>

## Algorithm Understanding (Links)
...
## General Questions (Legacy - see LiteLLM Support)
<Accordion title="Q: What models are supported? (Legacy)">
  A: **This information is for context. For current best practices, please see our [LiteLLM Support for Optimizers guide](/agent_optimization/opik_optimizer/litellm_support).**

  Previously, direct support focused on OpenAI and Azure OpenAI models. Opik Optimizer now supports all models available through LiteLLM, greatly expanding compatibility.
</Accordion>
