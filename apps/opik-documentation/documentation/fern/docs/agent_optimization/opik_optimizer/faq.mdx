...
<AccordionGroup>
  <Accordion title="Q: What models are supported by Opik Optimizers?">
    A: Opik Optimizer supports a wide array of Large Language Models through its integration with **LiteLLM**. This includes models from:
...
<AccordionGroup>
  <Accordion title="Q: What's the difference between standard genetic operators and LLM-driven ones (`enable_llm_crossover`) in `EvolutionaryOptimizer`?">
    A:
