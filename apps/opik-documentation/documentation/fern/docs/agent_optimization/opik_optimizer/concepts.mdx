...
<AccordionGroup>
  ...
  <Accordion title="Trial">
    Often used interchangeably with "Run" in the context of Opik Optimizer documentation referring to a single
    optimization execution. More broadly in experimental design, a trial might involve multiple runs to ensure
    statistical significance, but for Opik Optimizer, focus on it as one complete `optimize_prompt` cycle. Within some
  ...
  <Accordion title="Optimizer">
    A specialized algorithm within the Opik Optimizer toolkit designed to enhance prompt effectiveness. Each optimizer
