...
**Opik Agent Optimizer** is a turnkey, open-source agent and prompt optimization SDK. It automatically tunes prompts and agent workflows (including tool-using agents) using the datasets, metrics, and traces you already log to Opik. MetaPrompt also supports MCP tool schema optimization in beta. Instead of hand-editing instructions and re-running evaluations, pick an optimizer (MetaPrompt, HRPO, Evolutionary, GEPA, etc.) and let it iterate for you online or fully offline inside Docker and Kubernetes.
