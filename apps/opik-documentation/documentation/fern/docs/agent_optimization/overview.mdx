...
What set's the Opik optimizer apart from other optimization libraries is that you can optimize your
existing agent, you do not need to rewrite your application using our framework ! Simply pass in
your existing agent to get started:

<Tabs>
  <Tab title="Agent Optimization">
    ```python
        import opik
    ...
    prompt = opik_optimizer.ChatPrompt(
      ...
      function_map={
        "get_answer": opik.track(type="tool")(get_answer)
      },
      model="gpt-4o"
    )

    # Define the dataset to optimizer
    client = opik.Opik()
    ...
    def short_response(dataset_item, llm_output):
      if len(llm_output) < 100:
        return score_result.ScoreResult(
          name="short_response",
          value=1,
          reason="Response is short as expected."
        )
      else:
        return score_result.ScoreResult(
          name="short_response",
          value=0,
          reason="Response is too long, should be less than 100 characters."
        )
  ...
  <Tab title="Prompt Optimization">
    Let's optimize a system prompt to make sure the response is not too long.
...
- **Multiple Optimization Algorithms**: Choose from evolutionary, few-shot, meta-prompt, and GEPA optimizers
