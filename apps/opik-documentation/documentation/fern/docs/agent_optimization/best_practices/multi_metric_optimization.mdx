...
You can always do it yourself inside your custom metric function, however, if you do it via the Opik Optimizer special API, you
will be able get all of the components of your composite metric logged to Opik platform. As a result, you will be able
to review not only the final score, but also all of the sub-scores for your optimization trials and specific trial items (traces).
...
3. Pass it to your optimizer as a metric to optimize for
...
The weights determine the relative importance of each metric:
- `weights=[0.4, 0.6]` means the first metric contributes 40% and the second 60% to the final score
- Higher weights emphasize those metrics during optimization
- Weights are not required to sum up to 1, you can set any values.
...
![Multi-metric optimization progress chart](/img/agent_optimization/multi_metric_optimization/progress_chart.png)

The chart displays:
- **Composite metric** (my_composite_metric): The weighted combination of all metrics
- **Individual metrics** (levenshtein_ratio, answer_relevance_score): Each component metric tracked separately
- **Trial progression**: How metrics improve over time

This allows you to see not just the overall optimization progress, but also how each individual metric contributes to the final score.

### Trial Items View

You can also view individual trial items with their metric breakdowns:

![Trial items with multi-metric scores](/img/agent_optimization/multi_metric_optimization/trial_items.png)

For each trial item, you can see:
- The composite metric score
- Individual scores for each component metric
- How different prompts perform across all metrics
...
- Whether the weight balance is appropriate for your use case
