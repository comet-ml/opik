---
title: Advanced Prompt Engineering Best Practices with Opik Optimizer
---
Improving the performance of your LLM application and agent often requires improving the quality
of the prompts you are using. While manual iteration is common, a structured approach combined with automated tools can yield significantly better and more consistent results.

This guide covers foundational prompt engineering techniques and how Opik Optimizer can help you apply and automate them.
...
<Info>
We strongly recommend using an [LLM evaluation framework](/evaluation/evaluate_prompt). By defining:
...
*Opik Connection*: All Opik optimizers work by refining your `instruction_prompt` (defined in `TaskConfig`). A clear and specific starting prompt gives the optimizers a better foundation to build upon.
...
Refer to each optimizer's specific documentation to understand its strengths and how it aligns with different prompt engineering challenges. For general guidance on getting started, see the [Opik Optimizer Quickstart](/agent_optimization/opik_optimizer/quickstart).
