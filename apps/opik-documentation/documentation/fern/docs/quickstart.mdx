...
<Tabs>
  ...
  <Tab title="Opik Configure (TS)" value="opik-configure-ts">
    The Opik Configure is an interactive CLI tool that helps you quickly add Opik SDK to your Node.js/TypeScript project with guided setup:

    <Steps>
      <Step>
        Run the configure tool in your project directory:
        ...
        - Set up Opik client for your LLM integrations (OpenAI, LangChain, etc.)
      </Step>
    </Steps>

    <Tip>
      The tool supports custom options like `--debug` for verbose logging, `--force-install` to skip peer dependency checks, and `--install-dir` to specify a custom directory. Run `npx opik-configure --help` to see all available options.
    </Tip>
