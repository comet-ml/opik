---
subtitle: >-
  This guide helps you integrate the Opik platform with your existing LLM
  application
slug: /quickstart
---
...
Getting started is as simple as creating an [account on Comet](https://www.comet.com/signup?from=llm) or [self-hosting the platform](/docs/self-host/overview).
...
can use one of our [integrations](/docs/tracing/integrations/overview):
...
as described in the [Traces / Logging Additional Data section](/docs/tracing/log_traces#logging-additional-data).
...
like [LangChain](/docs/tracing/integrations/langchain), [LangGraph](/docs/tracing/integrations/langgraph) or [LLamaIndex](/docs/tracing/integrations/llama_index),
...
You LlamaIndex calls from that point forward will be logged to Opik. You can learn more about the LlamaIndex integration in the [LLamaIndex integration docs](/docs/tracing/integrations/llama_index).
...
<Info>
  ...
  logged data, see the [Log Traces](/docs/tracing/log_traces) guide.
</Info>
...
1. [Opik's evaluation metrics](/docs/evaluation/metrics/overview): Opik provides a suite of evaluation metrics (Hallucination, Answer Relevance, Context Recall, etc.) that you can use to score your LLM responses.
2. [Opik Experiments](/docs/evaluation/concepts): Opik allows you to automated the evaluation process of your LLM application so that you no longer need to manually review every LLM response.
