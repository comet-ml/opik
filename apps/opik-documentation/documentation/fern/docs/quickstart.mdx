...
<Tabs>
  ...
  <Tab title="Opik Wizard (TS)" value="opik-wizard-ts">
    The Opik Wizard is an interactive CLI tool that helps you quickly add Opik SDK to your Node.js/TypeScript project with guided setup:
    ...
    <Steps>
      <Step>
        ...
        The wizard will:
        - Detect your project setup (package manager, TypeScript config)
        - Install the Opik SDK and required dependencies
        - Help you configure environment variables
        - Set up tracing for your LLM integrations (OpenAI, LangChain, etc.)
      </Step>
      <Step>
        Follow the interactive prompts to:
        - Choose your package manager (npm, yarn, pnpm, bun)
        - Select which integrations to set up
        - Configure your API keys and environment variables
        - Apply formatting with Prettier (optional)
      </Step>
      <Step>
        Once complete, the wizard will have:
        - Installed all necessary packages
        - Created or updated your `.env` file with Opik configuration
        - Set up tracing in your application code

        Your project is now ready to log traces to Opik!
      </Step>
    </Steps>

    <Tip>
      The wizard supports custom options like `--debug` for verbose logging, `--force-install` to skip peer dependency checks, and `--install-dir` to specify a custom directory. Run `npx @opik/wizard --help` to see all available options.
    </Tip>
...
2. [Opik's evaluation metrics](/evaluation/metrics/overview): Opik provides a suite of evaluation
   metrics (Hallucination, Answer Relevance, Context Recall, etc.) that you can use to score your
   LLM responses.
3. [Opik Experiments](/evaluation/concepts): Opik allows you to automated the evaluation process of
