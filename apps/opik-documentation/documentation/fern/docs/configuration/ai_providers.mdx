...
For the URL section, enter your model URL, for example: `http://host.docker.internal:8000/v1`.
...
If your model access is authenticated by an api key, fill in the Api Key field with it. Otherwise, leave it blank.

In the Models section, provide all the models that are available in your server. You can select one of them later on.
