...
(function () {
  var DOCS_URL = "/docs/opik/mcp-server";
  ...
  var TABS = [
    ...
    },
    {
      ...
      label: "Claude Code",
      text: "Sets up Claude Code. Start a new session, then sign in through the browser if prompted.",
    ...
    },
    {
      ...
      label: "Codex",
      text: "Sets up Codex. Start a new session afterwards.",
    ...
    },
    {
      ...
      label: "Cursor",
      text: "Sets up Cursor. MCP needs a Cursor Pro plan or higher.",
    ...
    },
    {
      ...
      label: "VS Code",
      text: "Sets up VS Code Copilot. Reload the window afterwards.",
    ...
    },
    {
      ...
      label: "Skills only",
      text: "Installs the skill pack without the MCP server, for every agent found. Needs Node.",
  ...
  var CSS =
    ...
    ".opik-it-cmd.opik-it-cmd-alt{margin-bottom:2px}";
  ...
  function flashIcon(button, icon) {
  ...
  // Without a clipboard the command still has to be gettable, so select it and
  // say the copy did not happen. `code` is passed in rather than looked up from
  // the DOM, so the wrong block can never be selected.
  function selectCommand(code, button) {
    try {
      ...
      /* selection is a convenience; the text is on screen either way */
    }
    button.title = "Copy blocked by the browser. The command is selected, copy it by hand.";
    flashIcon(button, ICON_XMARK);
  }

  function copyText(code, button) {
    var text = code.textContent;
    if (!navigator.clipboard || !navigator.clipboard.writeText) {
      selectCommand(code, button);
      return;
    }
    navigator.clipboard.writeText(text).then(
      function () { flashIcon(button, ICON_CHECK); },
      function () { selectCommand(code, button); }
    );
  }
  ...
  function closeModal(overlay) {
    if (!overlay) return;
    if (overlay.opikOnKey) document.removeEventListener("keydown", overlay.opikOnKey);
    overlay.remove();
  }
  ...
  function openModal() {
    ...
    overlay.innerHTML =
      '<div class="opik-it-dialog" role="dialog" aria-modal="true" aria-labelledby="opik-it-title">' +
        ...
        '<p class="opik-it-alt">Another MCP client? Add the Opik Cloud server to it directly:</p>' +
    ...
    overlay.dataset.opikPath = location.pathname;
    var onKey = function (e) { if (e.key === "Escape") closeModal(overlay); };
    overlay.opikOnKey = onKey;

    overlay.addEventListener("click", function (e) { if (e.target === overlay) closeModal(overlay); });
    overlay.querySelector(".opik-it-close").addEventListener("click", function () { closeModal(overlay); });
    copyButtons.forEach(function (btn) {
      btn.addEventListener("click", function () {
        copyText(btn.previousElementSibling, btn);
      });
    });
  ...
  function scan() {
    // A client-side navigation re-renders the toolbar but leaves an open modal
    // (and its keydown listener) behind, so drop it when the path changes.
    var overlay = document.querySelector(".opik-it-overlay");
    if (overlay && overlay.dataset.opikPath !== location.pathname) {
      closeModal(overlay);
    }
