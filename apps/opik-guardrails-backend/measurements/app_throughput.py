...
def send_classification_request(text, topics):
    ...
    try:
        ...
        end_time = time.time()
        return {"error": f"HTTP error occurred: {e.response.status_code} {e.response.text}", "latency": end_time - start_time}
    except httpx.RequestError as e:
        end_time = time.time()
        return {"error": f"Request error occurred: {str(e)}", "latency": end_time - start_time}

def measure_performance(text: str, topics: List[str], num_requests: int, max_workers: int):
    start_time = time.time()
    results = []
    latencies = []
    errors = 0
    ...
    with futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        task_futures = [executor.submit(send_classification_request, text, topics) for _ in range(num_requests)]
        
        for task in tqdm(futures.as_completed(task_futures), total=num_requests, desc="Sending requests"):
            result = task.result()
...
def compare_concurrency_levels(text: str, topics: List[str], num_requests: int, concurrency_levels: List[int]):
