...
public class AiSpendResourceClient {
    ...
    public SpendUserPage getUsers(SpendMetricRequest request, int page, int size, List<SortingField> sortingFields,
        ...
        try (var response = callUsers(request, page, size, sortingFields, null, apiKey, workspaceName)) {
            assertThat(response.getStatus()).isEqualTo(expectedStatus);
    ...
    public SpendUserPage getUsers(SpendMetricRequest request, int page, int size, List<SortingField> sortingFields,
            String name, String apiKey, String workspaceName) {
        try (var response = callUsers(request, page, size, sortingFields, name, apiKey, workspaceName)) {
            assertThat(response.getStatus()).isEqualTo(HttpStatus.SC_OK);
