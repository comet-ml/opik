...
class AnthropicClientGeneratorTest {
    ...
    class SamplingParamsCapability {
         ...
         * reopened this bug). This asserts each adaptive model id (a) still maps to a real enum
         * constant, so the standalone opt-out set can't drift when a sync renames/removes a
         * constant, and (b) reports no sampling-param support.
         */
        @ParameterizedTest
        @ValueSource(strings = {"claude-sonnet-5", "claude-opus-4-7", "claude-opus-4-8"})
        void adaptiveThinkingModelsRemainEnumeratedAndRejectSamplingParams(String modelName) {
            var enumeratedValues = Arrays.stream(AnthropicModelName.values())
                    .map(AnthropicModelName::toString)
                    .collect(Collectors.toSet());
            assertThat(enumeratedValues).contains(modelName);
            assertThat(AnthropicModelName.supportsSamplingParams(modelName)).isFalse();
        }
    }
