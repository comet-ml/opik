...
class EventInterceptor implements MethodInterceptor {
    ...
    public Object invoke(MethodInvocation methodInvocation) {
        ...
        Span span = tracer
                .spanBuilder("%s.%s.%s".formatted(eventClass, methodInvocation.getMethod().getName(), event))
                .startSpan();

        if (methodInvocation.getArguments().length > 0 && methodInvocation.getArguments()[0] instanceof BaseEvent baseEvent) {
