...
public record DatasetItemBatch(
        ...
        @JsonView({
                DatasetItem.View.Write.class}) @Pattern(regexp = NULL_OR_NOT_BLANK, message = "must not be blank") @Schema(description = "Optional batch group ID to group multiple batches into a single dataset version") String batchGroupId)
        implements
            RateEventContainer{
