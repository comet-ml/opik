...
/**
 * Maps webhook events to Feishu Interactive Card payload format.
 *
...
public class FeishuWebhookPayloadMapper {
    ...
    private static String buildContent(@NonNull WebhookEvent<Map<String, Object>> event) {
        ...
        String details = switch (event.getEventType()) {
            ...
            case TRACE_ERRORS -> buildMetricsAlertDetails(metadata, baseUrl, "Trace Errors",
                    "No trace error alerts triggered");
            case TRACE_FEEDBACK_SCORE -> buildMetricsAlertDetails(metadata, baseUrl, "Trace Feedback Score",
                    "No trace feedback score alerts triggered");
            case TRACE_THREAD_FEEDBACK_SCORE -> buildMetricsAlertDetails(metadata, baseUrl, "Thread Feedback Score",
            ...
            case TRACE_COST -> buildMetricsAlertDetails(metadata, baseUrl, "Cost",
                    "No cost alerts triggered");
            case TRACE_LATENCY -> buildMetricsAlertDetails(metadata, baseUrl, "Latency",
    ...
    private static String buildMetricsAlertDetails(List<?> metadata, String baseUrl,
            String metricType, String emptyMessage) {
        if (metadata.isEmpty()) {
            return emptyMessage;
        }
