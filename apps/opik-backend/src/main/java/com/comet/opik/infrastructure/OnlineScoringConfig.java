...
public class OnlineScoringConfig {
    ...
    /**
     * Hard heap cap (in MB) on the trace-thread span preload used to size the inline-vs-agentic-tools
     * routing decision. Spans are streamed under this cap; once it is crossed the thread routes to the
     * agentic-tools path (which drills per-trace on demand), so a very large thread is never buffered in
     * full. Below the cap the bounded span list is kept for the inline/enriched path. See OPIK-7454.
     *
     * <p>Bounded to keep the estimate meaningful and heap use predictable: at least 1 MB (below that
     * almost any thread would be treated as oversized) and at most 512 MB.
     */
    @JsonProperty
