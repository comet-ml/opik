...
public class ExperimentItemPublisher {
     ...
     */
    public void publish(@NonNull UUID batchId, @NonNull List<ExperimentItemToProcess> messages) {
        if (messages.isEmpty()) {
            return;
        }
        ...
        Flux.fromIterable(messages)
                .flatMap(message -> stream.add(RedisStreamUtils.buildAddArgs(
                        ...
                        .doOnError(throwable -> log.error("Error publishing experiment item message", throwable)))
                .collectList()
                .block();

        log.info("Published '{}' experiment item messages for batch '{}'", messages.size(), batchId);
    }
}
