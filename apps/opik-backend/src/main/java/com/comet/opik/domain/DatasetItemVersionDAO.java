...
class DatasetItemVersionDAOImpl implements DatasetItemVersionDAO {
    ...
    private static final String SELECT_DATASET_ITEM_VERSIONS_WITH_EXPERIMENT_ITEMS_COUNT = """
            ...
            , experiment_items_final AS (
                ...
                -- OPIK-6177: experiment_item_aggregates.dataset_item_id IS the stable id post-migration
                -- (000084), so we reference it directly. No more lookup_div LEFT JOIN — keeps the
                -- minmax skip index on idx_experiment_item_aggregates_dataset_item_id usable.
                SELECT
                    eia.id AS id,
                    eia.experiment_id AS experiment_id,
                    eia.dataset_item_id AS stable_dataset_item_id,
    ...
    private static final String SELECT_DATASET_ITEM_VERSIONS_WITH_EXPERIMENT_ITEMS = """
            ...
            , top_dataset_items AS (
                -- OPIK-6177: post-migration, eia_t.dataset_item_id IS the stable id;
                -- direct column reference keeps idx_experiment_item_aggregates_dataset_item_id
                -- minmax skip index applicable.
                SELECT eia_t.dataset_item_id AS dataset_item_id
            ...
            , top_dataset_items AS (
                ...
                LEFT JOIN dataset_items_aggr_resolved AS di_t
                    ON di_t.id = eia_t.dataset_item_id
                WHERE eia_t.workspace_id = :workspace_id
                AND eia_t.experiment_id IN (SELECT id FROM experiment_aggregated_scope_ids)
                GROUP BY eia_t.dataset_item_id
            ...
            -- branches and the branches resolved different dataset_version_ids, we pick
            -- the highest dataset_version_id (argMax). This matches single-branch dedup
            -- semantics (dataset_items_(aggr_)resolved orders by dataset_version_id DESC,
            -- LIMIT 1 BY dataset_item_id), so cross-branch and single-branch agree on
            -- which version's metadata wins.
            SELECT
                u.id AS id,
            ...
            FROM (
                ...
                FROM (
                    -- OPIK-6177: post-migration eia.dataset_item_id IS the stable id;
                    -- direct column reference preserves skip-index pushdown.
                    SELECT
                        eia.id AS id,
                        eia.trace_id AS trace_id,
                        eia.dataset_item_id AS stable_dataset_item_id,
