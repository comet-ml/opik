...
class DatasetItemDAOImpl implements DatasetItemDAO {
    ...
    public Mono<DatasetItemPage> getItems(@NonNull UUID datasetId, int page, int size, String search,
        ...
        return makeMonoContextAware((userName, workspaceId) -> asyncTemplate.nonTransaction(connection -> {
            ...
            if (hasSearch) {
                countTemplate = countTemplate.add("search", true);
            } else {
                countTemplate = countTemplate.add("search", false);
            }
            ...
            if (hasSearch) {
                countStatement.bind("search", searchTerm);
            }
            ...
            return countAndColumnsMono.flatMap(countAndColumns -> {
                ...
                if (hasSearch) {
                    template = template.add("search", true);
                } else {
                    template = template.add("search", false);
                }
                ...
                // Bind search parameter if needed
                if (hasSearch) {
                    itemsStatement.bind("search", searchTerm);
                }
