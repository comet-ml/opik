...
public class DashboardResource {
    ...
    @GET
    @Operation(operationId = "getDashboards", summary = "Get dashboards", description = "Get dashboards", responses = {
            @ApiResponse(responseCode = "200", description = "Dashboards resource", content = @Content(schema = @Schema(implementation = Dashboard.DashboardPage.class)))
    })
    @JsonView(Dashboard.View.Public.class)
    public Response getDashboards(
        ...
        log.info("Finding dashboards, page '{}', size '{}' on workspaceId '{}'", page, size, workspaceId);
        Dashboard.DashboardPage dashboardPage = service.find(page, size);
        log.info("Found dashboards, count '{}', page '{}', size '{}' on workspaceId '{}'",
                dashboardPage.content().size(), page, size, workspaceId);
    ...
    @GET
    @Path("/by-name/{name}")
    @Operation(operationId = "getDashboardByName", summary = "Get dashboard by name", description = "Get dashboard by name", responses = {
            @ApiResponse(responseCode = "200", description = "Dashboard resource", content = @Content(schema = @Schema(implementation = Dashboard.class))),
            @ApiResponse(responseCode = "404", description = "Not found", content = @Content(schema = @Schema(implementation = ErrorMessage.class)))
    })
    @JsonView(Dashboard.View.Public.class)
    public Response getDashboardByName(@PathParam("name") String name) {

        String workspaceId = requestContext.get().getWorkspaceId();

        log.info("Finding dashboard by name '{}' on workspace_id '{}'", name, workspaceId);
        Optional<Dashboard> dashboard = service.findByName(name);

        if (dashboard.isEmpty()) {
            log.info("Dashboard not found by name '{}' on workspace_id '{}'", name, workspaceId);
            return Response.status(Response.Status.NOT_FOUND).build();
        }

        log.info("Found dashboard by name '{}', id '{}' on workspace_id '{}'", name, dashboard.get().id(), workspaceId);
        return Response.ok(dashboard.get()).build();
    }
