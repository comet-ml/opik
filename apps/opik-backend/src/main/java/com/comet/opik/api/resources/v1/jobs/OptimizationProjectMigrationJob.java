...
import static com.comet.opik.domain.OptimizationProjectMigrationService.RESULT_ERROR;
import static com.comet.opik.domain.OptimizationProjectMigrationService.RESULT_KEY;
import static com.comet.opik.infrastructure.lock.LockService.Lock;

@Singleton
@Slf4j
@DisallowConcurrentExecution
public class OptimizationProjectMigrationJob extends Job implements InterruptableJob {

    private static final Lock JOB_LOCK = new Lock("opik_job", OptimizationProjectMigrationJob.class.getSimpleName());
    ...
    public OptimizationProjectMigrationJob(
        ...
        this.cycleDuration = meter
                ...
                .setDescription(
                        "Duration of an optimization project migration cycle, tagged by result")
                .setUnit("ms")
                .ofLongs()
                .build();
    }
    ...
    public void doJob(JobExecutionContext context) {
        if (!config.enabled()) {
            log.debug("Optimization project migration job is disabled, skipping");
            return;
        }
        if (interrupted.get()) {
            log.info("Optimization project migration job was interrupted before execution, skipping");
            return;
        }
        log.info("Starting optimization project migration job");
        var startMillis = System.currentTimeMillis();
        var subscription = lockService.bestEffortLock(
                JOB_LOCK,
                Mono.defer(() -> {
                    if (interrupted.get()) {
                        log.info("Optimization project migration was interrupted before processing, skipping");
                        return Mono.empty();
                    }
                    return migrationService.runMigrationCycle()
                            ...
                            .doOnSuccess(unused -> {
                                ...
                                log.info("Optimization project migration cycle finished, duration='{}'", duration);
                            });
                }),
                Mono.defer(() -> {
                    long durationMillis = System.currentTimeMillis() - startMillis;
                ...
                .onErrorResume(throwable -> {
                    ...
                    if (interrupted.get()) {
                        log.warn("Optimization project migration was interrupted, duration='{}'", duration, throwable);
                    } else {
                        log.error("Optimization project migration failed, duration='{}'", duration, throwable);
                    }
                    return Mono.empty();
                })
    ...
    public void interrupt() {
        log.info("Interrupting optimization project migration job");
        interrupted.set(true);
        var execution = currentExecution.get();
        if (execution != null && !execution.isDisposed()) {
            execution.dispose();
            log.info("Optimization project migration job interrupted successfully");
        }
    }
}
