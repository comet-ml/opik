...
public record AnnotationQueueAutomation(
    ...
    /**
     * A single threshold on a named feedback score. The score name is deliberately not validated against
     * the workspace's feedback definitions — configuring an automation before the score exists is a
    ...
    public record ScoreCondition(
            @JsonView({
                    AnnotationQueue.View.Public.class,
                    AnnotationQueue.View.Write.class}) @NotBlank @Size(max = 255) String score,
