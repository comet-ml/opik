...
public class StatsMapper {
    ...
    public static Map<String, Double> getStatsUsage(Map<String, Object> stats) {
        return Optional.ofNullable(stats)
                .map(map -> map.keySet()
                        .stream()
                        .filter(k -> k.startsWith(USAGE) && !k.startsWith(USAGE_SUM))
                        .map(
    ...
    public static Map<String, Double> getStatsUsageSum(Map<String, Object> stats) {
        return Optional.ofNullable(stats)
                .map(map -> map.keySet()
                        .stream()
                        .filter(k -> k.startsWith(USAGE_SUM))
                        .map(
                                k -> Map.entry(k.substring("%s.".formatted(USAGE_SUM).length()), (Double) map.get(k)))
                        .collect(toMap(Map.Entry::getKey, Map.Entry::getValue)))
                .orElse(null);
    }
