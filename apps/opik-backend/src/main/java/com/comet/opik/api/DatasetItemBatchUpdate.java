...
public record DatasetItemBatchUpdate(
        @NotNull @NotEmpty @Size(min = 1, max = 1000) @Schema(description = "List of dataset item IDs to update (max 1000)") Set<UUID> ids,
        @NotNull @Valid @Schema(description = "Update to apply to all dataset items") DatasetItemUpdate update,
