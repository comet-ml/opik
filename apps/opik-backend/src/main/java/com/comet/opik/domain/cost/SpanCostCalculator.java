...
class SpanCostCalculator {
        public static BigDecimal textGenerationCost(ModelPrice modelPrice, Map<String, Integer> usage) {
                return modelPrice.inputPrice().multiply(BigDecimal.valueOf(usage.getOrDefault("prompt_tokens", 0)))
                                .add(modelPrice.outputPrice()
                                                .multiply(BigDecimal
                                                                .valueOf(usage.getOrDefault("completion_tokens", 0))));
        }

        public static BigDecimal textGenerationWithCacheCostOpenAI(ModelPrice modelPrice, Map<String, Integer> usage) {

                // In OpenAI usage format, input tokens includes the cached input tokens, so we
                // need to substract them to compute the correct input token count
                // Don't generalize yet as other providers seems to separate the cached tokens
                // from non-cached tokens

                // Get the input tokens (SDK version below 1.6.0 logged prompt_tokens, while
                // 1.6.0+ logged original_usage.prompt_tokens)
                ...
                // Get the output tokens (SDK version below 1.6.0 logged completion_tokens,
                // while 1.6.0+ logged original_usage.completion_tokens)
        ...
        public static BigDecimal textGenerationWithCacheCostAnthropic(ModelPrice modelPrice,
        ...
        /**
         * Calculates the cost of text generation where cached tokens are treated
         * separately from input/output tokens.
         * In this case, cached tokens (both read and creation) are not included in the
         * input or output token counts,
        ...
        private static BigDecimal textGenerationWithCachedTokensNotIncludedInCost(ModelPrice modelPrice,
                ...
                return modelPrice.inputPrice()
                                .multiply(
                                                BigDecimal.valueOf(usage.getOrDefault(inputTokensKey,
                                                                usage.getOrDefault("prompt_tokens", 0))))
                                .add(modelPrice.outputPrice()
                                                .multiply(BigDecimal.valueOf(
                                                                usage.getOrDefault(outputTokensKey,
                                                                                usage.getOrDefault("completion_tokens",
                                                                                                0)))))
                                .add(modelPrice.cacheCreationInputTokenPrice()
                                                .multiply(BigDecimal.valueOf(
                                                                usage.getOrDefault(cacheCreationInputTokensKey, 0))))
                                .add(modelPrice.cacheReadInputTokenPrice()
                                                .multiply(BigDecimal.valueOf(
                                                                usage.getOrDefault(cacheReadInputTokensKey, 0))));
        }
        ...
        public static BigDecimal speechTtsCost(ModelPrice modelPrice, Map<String, Integer> usage) {
                if (usage.containsKey("character_count")) {
                        return modelPrice.inputCharacterPrice()
                                        .multiply(BigDecimal.valueOf(usage.get("character_count")));
                }
                return BigDecimal.ZERO;
        }
}
