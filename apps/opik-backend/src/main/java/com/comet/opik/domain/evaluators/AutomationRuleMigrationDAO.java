...
public interface AutomationRuleMigrationDAO {
    ...
    @SqlQuery("SELECT project_id FROM automation_rule_projects WHERE rule_id = :ruleId AND workspace_id = :workspaceId ORDER BY project_id")
    List<UUID> findJunctionProjectIds(@Bind("ruleId") UUID ruleId, @Bind("workspaceId") String workspaceId);
