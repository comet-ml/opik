...
public class RequestyModule extends AbstractModule {
    @Provides
    @Singleton
    @Named("requestyGenerator")
    public OpenAIClientGenerator clientGenerator(
        ...
        customConfig.setOpenAiClient(new LlmProviderClientConfig.OpenAiClientConfig(config.getRequestyUrl()));
    ...
    public LlmServiceProvider llmServiceProvider(@NonNull LlmProviderFactory llmProviderFactory,
            @NonNull @Named("requestyGenerator") OpenAIClientGenerator clientGenerator) {
        return new RequestyLlmServiceProvider(clientGenerator, llmProviderFactory);
    }
}
