    ...
    private static final String GET_TRACE_KPI_CARDS = """
            ...
            traces_filtered AS (
                ...
                FROM (
                    ...
                    AND id \\<= :uuid_to_time
                    AND (toDate32(id_at) - toIntervalDay(toDayOfWeek(id_at, 1))) >= toMonday(UUIDv7ToDateTime(toUUID(:uuid_from_time), 'UTC'))
                    AND (toDate32(id_at) - toIntervalDay(toDayOfWeek(id_at, 1))) \\<= toMonday(UUIDv7ToDateTime(toUUID(:uuid_to_time), 'UTC'))
