...
public interface DashboardDAO {
    ...
    @SqlQuery("""
            ...
            """)
    Optional<Dashboard> findByName(@Bind("name") String name, @Bind("workspaceId") String workspaceId);
    ...
    @SqlQuery("""
            ...
            """)
    List<Dashboard> findByWorkspaceId(@Bind("workspaceId") String workspaceId,
    ...
    // Dashboard Sections
    @SqlUpdate("""
            INSERT INTO dashboard_sections (id, dashboard_id, title, position_order, is_expanded, created_by, last_updated_by)
            VALUES (:id, :dashboardId, :title, :positionOrder, :isExpanded, :userName, :userName)
            """)
    void insertSection(@Bind("id") UUID id,
            @Bind("dashboardId") UUID dashboardId,
            @Bind("title") String title,
            @Bind("positionOrder") Integer positionOrder,
            @Bind("isExpanded") Boolean isExpanded,
            @Bind("userName") String userName);

    @SqlUpdate("""
            UPDATE dashboard_sections
            SET title = :title, position_order = :positionOrder, is_expanded = :isExpanded, last_updated_by = :userName
            WHERE id = :id
            """)
    ...
    @SqlQuery("""
            SELECT id, dashboard_id, title, position_order, is_expanded, created_at, created_by, last_updated_at, last_updated_by
    ...
    @SqlUpdate("""
            ...
            SET name = :name, type = :type, configuration = :configuration, layout = :layout, last_updated_by = :userName
            WHERE id = :id
            """)
    ...
    @SqlUpdate("""
            ...
            WHERE section_id IN (SELECT id FROM dashboard_sections WHERE dashboard_id = :dashboardId)
            """)
    void deletePanelsByDashboardId(@Bind("dashboardId") UUID dashboardId);
    ...
    @SqlQuery("""
            ...
            """)
    List<DashboardPanel> findPanelsBySectionId(@Bind("sectionId") UUID sectionId);
    ...
    class DashboardSectionMapper implements org.jdbi.v3.core.mapper.RowMapper<DashboardSection> {
        ...
        public DashboardSection map(java.sql.ResultSet rs, org.jdbi.v3.core.statement.StatementContext ctx)
            ...
            return DashboardSection.builder()
                    ...
                    .isExpanded(rs.getBoolean("is_expanded"))
                    .createdAt(rs.getTimestamp("created_at").toInstant())
                    .createdBy(rs.getString("created_by"))
                    .lastUpdatedAt(rs.getTimestamp("last_updated_at").toInstant())
                    .lastUpdatedBy(rs.getString("last_updated_by"))
    ...
    class DashboardPanelMapper implements org.jdbi.v3.core.mapper.RowMapper<DashboardPanel> {
        ...
        public DashboardPanel map(java.sql.ResultSet rs, org.jdbi.v3.core.statement.StatementContext ctx)
            ...
            try {
                ...
                throw new RuntimeException("Error mapping dashboard panel", e);
            }
        }
    }
