 ...
 * statement's parameter names, so binding alone is O(n²) and dominates the insert. Streaming
 * JSONEachRow removes parameter binding from the picture entirely: one HTTP body, parsed server-side
 * in ClickHouse's fast path. {@code ExperimentAggregatesDAOImpl} established this pattern for
 ...
 * which matters because a batch can be large and its free-text columns unbounded, and would otherwise
 * be held in full (twice over, converting chars to UTF-8) per concurrent request. Rows are built with
 * Jackson, not string concatenation: the payload carries user-supplied content, so escaping has to be
 * correct by construction. A failure logs the table, the row count and the exception as its cause —
 * never the rows.
 *
    ...
    public <T> Mono<Long> insert(@NonNull String table,
        ...
        return Mono.fromCallable(() -> {
            ...
            try (InsertResponse response = clickHouseClient.insert(table, out -> {
                ...
                // would hold each row twice more, which for free-text columns is the allocation cost
                // this streaming path exists to avoid.
