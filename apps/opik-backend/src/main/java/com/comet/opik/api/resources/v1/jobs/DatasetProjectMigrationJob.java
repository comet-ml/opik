...
import static com.comet.opik.domain.DatasetProjectMigrationService.RESULT_ERROR;
import static com.comet.opik.domain.DatasetProjectMigrationService.RESULT_KEY;
import static com.comet.opik.infrastructure.lock.LockService.Lock;

@Singleton
@Slf4j
@DisallowConcurrentExecution
public class DatasetProjectMigrationJob extends Job implements InterruptableJob {

    private static final Lock JOB_LOCK = new Lock("opik_job", DatasetProjectMigrationJob.class.getSimpleName());
    ...
    public DatasetProjectMigrationJob(
        ...
        this.cycleDuration = meter
                ...
                .setDescription(
                        "Duration of a dataset project migration cycle, tagged by result")
                .setUnit("ms")
                .ofLongs()
                .build();
    }
    ...
    public void doJob(JobExecutionContext context) {
        if (!config.enabled()) {
            log.debug("Dataset project migration job is disabled, skipping");
            return;
        }
        if (interrupted.get()) {
            log.info("Dataset project migration job was interrupted before execution, skipping");
            return;
        }
        log.info("Starting dataset project migration job");
        var startMillis = System.currentTimeMillis();
        var subscription = lockService.bestEffortLock(
                JOB_LOCK,
                Mono.defer(() -> {
                    if (interrupted.get()) {
                        log.info("Dataset project migration was interrupted before processing, skipping");
                        return Mono.empty();
                    }
                    return migrationService.runMigrationCycle()
                            ...
                            .doOnSuccess(unused -> {
                                ...
                                log.info("Dataset project migration cycle finished, duration='{}'", duration);
                            });
                }),
                Mono.defer(() -> {
                    long durationMillis = System.currentTimeMillis() - startMillis;
                ...
                .onErrorResume(throwable -> {
                    ...
                    if (interrupted.get()) {
                        log.warn("Dataset project migration was interrupted, duration='{}'", duration, throwable);
                    } else {
                        log.error("Dataset project migration failed, duration='{}'", duration, throwable);
                    }
                    return Mono.empty();
                })
    ...
    public void interrupt() {
        log.info("Interrupting dataset project migration job");
        interrupted.set(true);
        var execution = currentExecution.get();
        if (execution != null && !execution.isDisposed()) {
            execution.dispose();
            log.info("Dataset project migration job interrupted successfully");
        }
    }
}
