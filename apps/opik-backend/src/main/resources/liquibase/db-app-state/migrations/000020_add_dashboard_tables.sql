...
CREATE TABLE dashboard_sections (
    ...
    INDEX `dashboard_sections_position_order` (dashboard_id, position_order)
);
...
CREATE TABLE dashboard_panels (
    ...
    CONSTRAINT `dashboard_panels_section_id_fk` FOREIGN KEY (section_id) REFERENCES dashboard_sections(id) ON DELETE CASCADE,
    INDEX `dashboard_panels_section_id` (section_id),
    INDEX `dashboard_panels_type` (type)
);
...
CREATE TABLE experiment_dashboards (
    ...
    INDEX `experiment_dashboards_dashboard_id` (dashboard_id),
    INDEX `experiment_dashboards_workspace_id` (workspace_id)
);
