...
ALTER TABLE ${ANALYTICS_DB_DATABASE_NAME}.spans ON CLUSTER '${ANALYTICS_DB_CLUSTER_NAME}'
    ADD COLUMN IF NOT EXISTS `duration` Nullable(Float64) MATERIALIZED
        if(end_time IS NOT NULL AND start_time IS NOT NULL
            ...
            (dateDiff('microsecond', start_time, end_time) / 1000.0),
            NULL);
--rollback ALTER TABLE ${ANALYTICS_DB_DATABASE_NAME}.traces ON CLUSTER '${ANALYTICS_DB_CLUSTER_NAME}' DROP COLUMN IF EXISTS `duration`;
--rollback ALTER TABLE ${ANALYTICS_DB_DATABASE_NAME}.spans ON CLUSTER '${ANALYTICS_DB_CLUSTER_NAME}' DROP COLUMN IF EXISTS `duration`;
