...
--rollback ALTER TABLE ${ANALYTICS_DB_DATABASE_NAME}.spans ON CLUSTER '${ANALYTICS_DB_CLUSTER_NAME}' DROP COLUMN IF EXISTS truncation_threshold, DROP COLUMN IF EXISTS truncated_input, DROP COLUMN IF EXISTS truncated_output;
--rollback ALTER TABLE ${ANALYTICS_DB_DATABASE_NAME}.traces ON CLUSTER '${ANALYTICS_DB_CLUSTER_NAME}' DROP COLUMN IF EXISTS truncation_threshold, DROP COLUMN IF EXISTS truncated_input, DROP COLUMN IF EXISTS truncated_output;
