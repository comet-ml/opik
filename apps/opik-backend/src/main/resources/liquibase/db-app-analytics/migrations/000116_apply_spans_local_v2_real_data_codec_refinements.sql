...
-- Five columns, in three groups. Each is at worst neutral against the codec 000115 ships, so adopting now is low-regret.
--   * usage: ZSTD(1) -> ZSTD(3), 19.3% smaller. 000115 assigned ZSTD(1) as an explicit best guess, this being the one
...
--     traces. Separately: 000115's comment justifies LowCardinality from a 4M-row sample showing 53 models and 14
--     providers; the true global cardinality is 6,711 and 2,209. Still inside low_cardinality_max_dictionary_size
--     (8192, per part), so the choice holds, but with far less headroom than that comment implies. 000115 is merged and
--     its checksum is immutable, so the correction is recorded here.
-- Columns are ordered as declared in 000115.
...
-- 000115 codecs is never a wanted recovery step (matches 000106 / 000107 on traces_local_v2). Recovery, if it were ever
