...
-- duplicate/replayed rows are expected. The version is the SOURCE span's last_updated_at
-- (set by the consumer, not insert time) so the latest source state wins regardless of
-- delivery order. ORDER BY mirrors `spans` (workspace_id, project_id, trace_id, id): the
-- time-ordered UUIDv7 trace_id/span_id let reads prune via `trace_id BETWEEN`/`id BETWEEN`
-- ranges, trace_id is also the join key to cipx_trace_identity, and span_id makes the key
-- unique per span so dedup is exact. start_time stays a row-level filter, as in `spans`.
CREATE TABLE IF NOT EXISTS ${ANALYTICS_DB_DATABASE_NAME}.cipx_spend ON CLUSTER '{cluster}'
(
    workspace_id     String,
...
ENGINE = ReplicatedReplacingMergeTree(
    '/clickhouse/tables/{shard}/${ANALYTICS_DB_DATABASE_NAME}/cipx_spend',
    '{replica}',
    last_updated_at
)
ORDER BY (workspace_id, project_id, trace_id, span_id);

--rollback DROP TABLE IF EXISTS ${ANALYTICS_DB_DATABASE_NAME}.cipx_spend ON CLUSTER '{cluster}';
