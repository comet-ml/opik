...
--changeset thiagohora:000115_apply_spans_local_v2_real_data_codec_refinements
...
--     traces. Separately: 000114's comment justifies LowCardinality from a 4M-row sample showing 53 models and 14
--     providers; the true global cardinality is 6,711 and 2,209. Still inside low_cardinality_max_dictionary_size
--     (8192, per part), so the choice holds, but with far less headroom than that comment implies. 000114 is merged and
--     its checksum is immutable, so the correction is recorded here.
-- Columns are ordered as declared in 000114.
