...
#   - Custom Charts (additionally, when TOGGLE_CUSTOM_CHARTS_WORKSPACES is non-empty) - the same three tables plus
...
charts_user="${ANALYTICS_DB_READ_ONLY_CHARTS_USER:-comet_readonly_charts_sql_user}"
charts_pass="${ANALYTICS_DB_READ_ONLY_CHARTS_PASS:-opik}"
...
# Custom Charts user. Provisioned only when at least one workspace is allowlisted, so an install that never turns
# the feature on carries no extra account. Grants cover the five tables Agent Insights lacks; the row policies are
# workspace-only on everything except traces/spans, which stay project-bound (A3).
if [ -n "$charts_workspaces" ]; then
    echo "Provisioning Custom Charts read-only ClickHouse user '${charts_user}'..."
    statements+=(
        "CREATE USER IF NOT EXISTS ${charts_user} IDENTIFIED BY '${charts_pass}'"
        "CREATE SETTINGS PROFILE IF NOT EXISTS comet_readonly_charts_sql_profile SETTINGS readonly = 1, max_execution_time = 180, max_memory_usage = 8589934592, max_result_rows = 100000, result_overflow_mode = 'throw', max_rows_to_read = 100000000, read_overflow_mode = 'throw', max_concurrent_queries_for_user = 5, use_skip_indexes_if_final = 1, SQL_workspace_id = '' CHANGEABLE_IN_READONLY, SQL_project_id = '' CHANGEABLE_IN_READONLY TO ${charts_user}"
    )
    ...
    for tbl in spans traces; do
        statements+=(
            "GRANT SELECT ON ${ch_db}.${tbl} TO ${charts_user}"
            "CREATE ROW POLICY IF NOT EXISTS ${tbl}_charts_workspace_project_isolation ON ${ch_db}.${tbl} FOR SELECT USING workspace_id = getSetting('SQL_workspace_id') AND project_id = getSetting('SQL_project_id') AS RESTRICTIVE TO ${charts_user}"
        )
    done
    for tbl in authored_feedback_scores feedback_scores experiments experiment_items dataset_items trace_threads; do
        statements+=(
            "GRANT SELECT ON ${ch_db}.${tbl} TO ${charts_user}"
            "CREATE ROW POLICY IF NOT EXISTS ${tbl}_charts_workspace_isolation ON ${ch_db}.${tbl} FOR SELECT USING workspace_id = getSetting('SQL_workspace_id') AS RESTRICTIVE TO ${charts_user}"
        )
    done
fi
