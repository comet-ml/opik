...
# Extended free-form SQL user, provisioned alongside the one above rather than behind its own condition: the
# account is inert until a workspace is allowlisted, so making its existence depend on that toggle would only mean
# a config flip needs a redeploy to take effect. Grants cover the five tables Agent Insights lacks.
