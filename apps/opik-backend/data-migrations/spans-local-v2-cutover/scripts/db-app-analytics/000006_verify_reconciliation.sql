-- runbook spans-local-v2-cutover — POST-SWAP reconciliation POSTCONDITION (driven by ../reconcile.sh)
...
--
-- THE KEY IS (workspace_id, project_id, trace_id, id) — THE DESTINATION'S DEDUP KEY — ON BOTH SIDES, AND THE OLD-SCHEMA
-- SIDE IS REDUCED TO IT. `spans` orders by that plus parent_span_id, so it can hold two live rows for one span where the
-- successor holds one (migration 000115, and 000005's header at length). Whichever side carries the old schema is
-- therefore collapsed with `argMax(<fingerprint>, last_updated_at) GROUP BY` the key, after FINAL — picking the same
-- winner ReplacingMergeTree picked — while the new-schema side needs FINAL alone, its own key already being the
-- comparison key. Without that reduction a faithful reconciliation would report `payload_mismatch_keys` (or, through the
-- JOIN's fan-out, counts larger than the key set) on every span whose parent changed.
--
...
--
-- WHAT `newer_keys` MEANS, and why it is not a gate. It is EXPECTED to be non-zero on a busy estate: a gap-window span
...
-- timestamps, same absent-end_time -> 0 and absent-ttft -> 'nan' canonicalization, same parent_span_id length guard on
-- the old side and CAST-to-String on the new, same sorted-usage encoding, same toString on enums/ids/LowCardinality/
-- Decimal, same '\x1f' tag delimiter, same cityHash64. A fingerprint that diverged from the one verify.sh computes would
-- make these two tools disagree about the same rows, and would classify keys here by a fingerprint the fidelity compare
-- does not use. The VERSION is compared as a microsecond epoch for the same reason it is hashed as one: the copy
-- truncates nanoseconds to microseconds, so comparing the raw columns would report every faithfully-copied row with a
...
-- acceptable because it is bounded by the gap window and its skip indexes, not by the table. max_bytes_before_
-- external_group_by is carried for the reduction's GROUP BY, which 000005's header explains.
--
...
--   ${LIVE_TABLE}                   `spans`, or `spans_local` on a wrapped estate (see 000006_post_swap_reconciliation.sql)
...
-- Forward: parked = spans_pre_cutover_backup (OLD schema — Nullable, nanosecond, parent_span_id in the sort key);
-- live = the successor (sentinels, microsecond). The exclusion arm mirrors the forward sweep's: a gap-window key deleted
-- AFTER the swap is legitimately absent from the live table, so counting it as `missing_keys` would make the gate
-- unreachable. Keys deleted BEFORE the swap are NOT excluded here, deliberately — one that is live in the frozen backup
-- was re-created before the freeze and must be present live, and one that is not live in the backup never enters this
-- set at all (FINAL + the delete mask).
WITH
    parked AS (
        SELECT
            key,
            ...
            argMax(cityHash64(
                ...
                toString(source),
                toString(environment)), last_updated_at) AS parked_fingerprint
        FROM (
            SELECT (workspace_id, project_id, trace_id, id) AS key, *
            FROM ${ANALYTICS_DB_DATABASE_NAME}.spans_pre_cutover_backup FINAL
...
SETTINGS join_use_nulls = 1,
         ...
         log_comment = 'spans_local_v2_cutover:reconcile:verify_forward';
-- >>> END verify-forward

-- >>> BEGIN verify-reverse
-- Reverse: parked = spans_post_rollback_backup (NEW schema — sentinels, microsecond, parent_span_id out of the sort key);
-- live = the restored original (Nullable, nanosecond, parent_span_id in it). The shapes are the mirror image of the
-- forward block, so the two normalization arms swap sides — AND SO DOES THE REDUCTION: here it is the LIVE side that
-- carries the old schema and therefore needs the argMax collapse to the destination key, while the parked successor
-- needs FINAL alone.
--
...
WITH
    parked AS (
        ...
        FROM ${ANALYTICS_DB_DATABASE_NAME}.spans_post_rollback_backup FINAL
        WHERE (created_at >= toDateTime64('${GAP_START}', 6, 'UTC')
            OR last_updated_at >= toDateTime64('${GAP_START}', 6, 'UTC'))
          AND (workspace_id, project_id, id) NOT IN (
              SELECT
    ...
    live AS (
        SELECT
            ...
            argMax(cityHash64(
                ...
                if(ttft IS NULL, 'nan', toString(ttft)),
                toString(source),
                toString(environment)), last_updated_at) AS live_fingerprint
        FROM (
            SELECT (workspace_id, project_id, trace_id, id) AS key, *
            FROM ${ANALYTICS_DB_DATABASE_NAME}.${LIVE_TABLE} FINAL
            WHERE (workspace_id, project_id, trace_id, id) IN (SELECT key FROM parked)
...
SETTINGS join_use_nulls = 1,
         use_skip_indexes_if_final = 1,
         max_bytes_before_external_group_by = 4000000000,
         log_comment = 'spans_local_v2_cutover:reconcile:verify_reverse';
-- >>> END verify-reverse
...
--
-- THE KEY HERE IS (workspace_id, project_id, id), NOT THE DEDUP KEY, because that is what the BRIDGE records — a
-- deletion event carries no trace_id. A span id identifies a span, so matching on the triple is exact for every path the
-- product writes; and the version comparison below is what makes the verdict precise regardless.
--
...
-- them are wanted — including, on spans, the versions filed under a different parent_span_id, which the source's wider
-- sort key keeps as separate physical rows. That breadth is why no dedup reduction is applied on this side either.
-- Only the live side needs FINAL, to compare against the row the successor actually serves.
WITH
    bridged AS (
        SELECT
            workspace_id,
...
SETTINGS apply_deleted_mask = 0,
         use_skip_indexes_if_final = 1,
         log_comment = 'spans_local_v2_cutover:reconcile:leak_check_forward';
