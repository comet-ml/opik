-- runbook spans-local-v2-cutover — ROLLBACK sentinel repair (driven by ../rollback.sh --sentinel-repair-only)
-- The gate test SpansLocalV2CutoverTest reimplements this statement inline; keep the two in step (see its Javadoc).
--
-- Restores NULL on rows written into the still-Nullable original while spanColumnsNonNullable was true. The successor
...
-- original's MATERIALIZED duration computed a large NEGATIVE duration from them. That expression (migration 000055, the
-- same one it added to traces) does guard against the epoch, but on `start_time` only: `end_time` it checks for NULL
-- alone, which a sentinel is not.
--
...
--
-- THE BLAST RADIUS IS WIDER ON SPANS THAN ON TRACES, for one reason worth sizing before running it: the `ttft` arm hits
-- any span written without a ttft, and on spans that is the overwhelming majority — ttft is a per-LLM-call measurement
-- and most spans are not LLM calls at all. The `end_time` arm is narrower (an in-flight span). Read the two counts the
-- driver prints separately; a large sentinel_ttft against a small sentinel_end_time is the expected shape here, not a
-- sign the window is wrong.
--
...
-- remain. Measured on an internal environment during the traces cutover: the unbounded predicate matched roughly seven
-- times as many keys as the flag window had produced; every extra one carried a genuine client-sent value.
--
...
-- translates the ACCESS_DENIED). On a table this size that halving is the difference between one whole-table rewrite and
-- two, so budget the single pass generously: ../rollback.sh --receive-timeout is what bounds the mutations_sync = 2 wait.
--
-- Deliberately NOT `ON CLUSTER`, matching the reverse replay beside it. `spans` is a Replicated*MergeTree, so a
...
SETTINGS mutations_sync = 2,
         log_comment = 'spans_local_v2_rollback:sentinel_repair';
