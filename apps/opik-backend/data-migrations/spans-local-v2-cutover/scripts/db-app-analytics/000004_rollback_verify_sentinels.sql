-- runbook spans-local-v2-cutover — ROLLBACK sentinel-repair COUNTS (driven by ../rollback.sh, before and after the repair)
...
--
-- THE KEY IS (workspace_id, project_id, id), NOT THE TABLE'S DEDUP KEY. The source `spans` orders by
-- (workspace_id, project_id, trace_id, parent_span_id, id) and the successor drops parent_span_id, but neither is what
-- this count is about: it answers "how many spans still carry the flag's damage", and a span id identifies a span. Using
-- a wider key would count two versions of one span as two problems, and the operator acts per span.
--
...
-- "the flag's damage is gone" rather than "no such value exists anywhere". On spans the unbounded ttft count is
-- enormous by construction (most spans are not LLM calls and carry no ttft at all), so an unbounded run here is not
-- merely weaker than the windowed one — it is meaningless.
--
...
SETTINGS log_comment = 'spans_local_v2_rollback:verify_sentinels';
