#!/usr/bin/env bash
#
# Driver for rolling the spans cutover back (runbook: ../README.md).
#
# Runs the db-app-analytics/000004_rollback_* file(s) that match how far the cutover got. Pick the stage by the last
# step that completed:
#   --stage A   backfill/delta ran but the EXCHANGE did not — discard the shadow (live `spans` is untouched).
...
# and their topology guards refuse cleanly when aimed at an unwrapped one.
...
# spanColumnsNonNullable was true (they read back as an epoch/NaN sentinel, and its MATERIALIZED duration computed a
...
# Or --reverse-replay-only: re-apply just the reverse deletion replay against the current live `spans`. Use it when a
# stage B/C run's promote succeeded but its reverse-replay was interrupted — the promote leaves `spans` in the restored
...
#
# POST-CUTOVER WRITES: stages B/C promote the frozen pre-cutover backup back to live `spans`, so spans WRITTEN to the
# successor after cutover_start stop being live. They are NOT destroyed — the successor is parked as
# spans_post_rollback_backup and retained until finalize.sh. --accept-post-cutover-write-loss makes the operator
...
# EXCHANGE/DROP — running any file does exactly one stage. Before running, this asserts the live `spans` topology matches
...
# Stages B/C end with spans = original data live and the successor parked as spans_post_rollback_backup (a retained
# backup that finalize.sh later recycles into an empty spans_local_v2); stage A discards the empty spans_local_v2
# shadow and leaves spans untouched.
#
...
#                             databaseAnalyticsDataModel.spanColumnsNonNullable=false is live on EVERY backend
...
#                             The window spanColumnsNonNullable was live in. Only rows written inside it are repaired,
...
#                             spanColumnsNonNullable was live HERE, so the epoch/NaN values in `spans` are sentinels it
...
#                             the assertion: `spans_local_v2` exists on every installation, so its presence
#                             establishes nothing.
...
#                             the live `spans` has a brief cross-node ON CLUSTER propagation skew during which a query
...
if [[ "$CONFIRM_FLAG_REVERTED" == "1" && "$SENTINEL_REPAIR_ONLY" != "1" ]]; then
    ...
    echo "       spanColumnsNonNullable is reverted — reverting it is the next step they print — so asserting it here" >&2
...
if [[ "$SENTINEL_REPAIR_ONLY" == "1" ]]; then
    ...
    if [[ -z "$SENTINEL_WINDOW_FROM" || -z "$SENTINEL_WINDOW_TO" ]]; then
        ...
        echo "       spanColumnsNonNullable was live in. There is no safe default. An epoch end_time is not evidence" >&2
    ...
    if [[ "$CONFIRM_FLAG_REVERTED" != "1" ]]; then
        ...
        echo "       databaseAnalyticsDataModel.spanColumnsNonNullable=false and roll-restart every backend instance" >&2
...
if [[ "$UNWRAP_ONLY" == "1" ]]; then
    ...
    if [[ "$CONFIRM_MAINTENANCE" != "1" ]]; then
        echo "ERROR: --unwrap-only requires --confirm-maintenance. It renames the live 'spans': gapless per node, but with" >&2
        echo "       a brief cross-node ON CLUSTER skew during which a lagging replica still resolves the wrapper's" >&2
        echo "       'spans_local' target, which the already-renamed replicas no longer have — so a query routed there can" >&2
...
# (they are preserved in the parked spans_post_rollback_backup until finalize.sh, recoverable during the soak). This is
...
if [[ ( "$STAGE" == "B" || "$STAGE" == "C" ) && "$ACCEPT_WRITE_LOSS" != "1" ]]; then
    ...
    echo "       spans written to the successor after cutover_start non-live. They are NOT destroyed — the successor is" >&2
    echo "       parked as spans_post_rollback_backup until finalize.sh." >&2
    echo "       The flag acknowledges the loss; it does not commit you to it. After the promote this driver prints how" >&2
    echo "       many such spans there are and the 'reconcile.sh --confirm-reimport-successor-writes' command that merges" >&2
...
# Stage C and --unwrap-only both rotate `spans` out to 'spans_dist_old', and RENAME cannot overwrite an existing name. A
...
assert_spans_dist_old_free() {
    local context="$1" engine definition
    engine="$(spans_engine spans_dist_old)"
    [[ -n "$engine" ]] || return 0
    echo "ERROR: $context: '$DATABASE.spans_dist_old' already exists, so the atomic rotate cannot claim that name." >&2
    if [[ "$engine" == "Distributed" ]]; then
        ...
        definition="$(ch "SELECT engine_full FROM system.tables WHERE database = '$DATABASE' AND name = 'spans_dist_old' FORMAT TSVRaw" 2>/dev/null || true)"
        ...
        echo "         clickhouse-client ${CH_HOST:+--host $CH_HOST} ${CH_PORT:+--port $CH_PORT} --database $DATABASE --query \"DROP TABLE IF EXISTS $DATABASE.spans_dist_old ON CLUSTER '{cluster}' SYNC\"" >&2
    else
...
#   - `spans` has Nullable end_time — it is the restored ORIGINAL, not the live successor.
#   - `spans_post_rollback_backup` exists AND holds the successor schema (non-Nullable end_time) — a promote really
...
#   - `spans_pre_cutover_backup` is GONE — the promote's rename consumes it. Still present alongside the other two is a
...
assert_post_promote_state() {
    local mode="$1" spans_end_time backup_end_time pre_cutover line
    shift
    spans_end_time="$(spans_endtime_type spans)"
    backup_end_time="$(spans_endtime_type spans_post_rollback_backup)"
    pre_cutover="$(spans_engine spans_pre_cutover_backup)"
    if [[ "$spans_end_time" == Nullable* && -n "$backup_end_time" && "$backup_end_time" != Nullable* && -z "$pre_cutover" ]]; then
        return 0
    fi
    echo "ERROR: $mode requires the post-rollback state — 'spans' = the restored ORIGINAL (Nullable end_time), the" >&2
    echo "       successor parked as 'spans_post_rollback_backup' with the successor schema (non-Nullable end_time), and" >&2
    echo "       no 'spans_pre_cutover_backup' left, since a promote's rename consumes it." >&2
    echo "       Observed: spans end_time='$spans_end_time', spans_post_rollback_backup end_time='${backup_end_time:-<absent>}'," >&2
    echo "                 spans_pre_cutover_backup engine='${pre_cutover:-<absent>}'." >&2
    for line in "$@"; do
        echo "       $line" >&2
    done
...
#
# Only the first is TOPOLOGICALLY provable. `spans_post_rollback_backup` carrying the successor schema exists solely
# because a promote created it. There is no equivalent signal for the pre-EXCHANGE case — in particular NOT the presence
# of `spans_local_v2`, which Liquibase creates unconditionally on every installation (000115), so it says nothing about
...
# user-deleted spans that the rollback has resurrected.
assert_single_shard() {
    local shards
...
assert_sentinel_repair_state() {
    local spans_end_time backup_end_time
    spans_end_time="$(spans_endtime_type spans)"
    backup_end_time="$(spans_endtime_type spans_post_rollback_backup)"
    if [[ "$spans_end_time" != Nullable* ]]; then
        echo "ERROR: --sentinel-repair-only needs 'spans' on the ORIGINAL schema (Nullable end_time); observed" >&2
        echo "       end_time='${spans_end_time:-<absent>}'. On the successor the epoch/NaN values are that schema's own" >&2
    ...
    if [[ "$CONFIRM_FLAG_WAS_LIVE" == "1" ]]; then
        echo "NOTE: no parked successor, so proceeding on --confirm-flag-was-live: you assert spanColumnsNonNullable was" >&2
    ...
    echo "ERROR: --sentinel-repair-only found no parked successor ('spans_post_rollback_backup' carrying the successor" >&2
    ...
    echo "       values. 'spans_local_v2' is NOT evidence either way: Liquibase creates it on every installation." >&2
...
# The migration walks spans through three shapes; a stage is only valid in one of them:
#   pre-EXCHANGE       -> spans is a *MergeTree with Nullable end_time (the original schema)      -> stage A
#   post-EXCHANGE      -> spans is a *MergeTree with non-Nullable end_time (the successor schema) -> stage B
#   post-wrap          -> spans is a Distributed table                                           -> stage C
...
assert_topology() {
    ...
    engine="$(spans_engine spans)"
    end_time="$(spans_endtime_type spans)"
    [[ -n "$engine" ]] || { echo "ERROR: no 'spans' table found in database '$DATABASE'." >&2; exit 1; }

    case "$STAGE" in
        A)
            [[ "$engine" != "Distributed" && "$end_time" == Nullable* ]] || {
                echo "ERROR: stage A expects the pre-EXCHANGE state (spans = original schema), but spans is engine='$engine' end_time='$end_time'." >&2
                echo "       The EXCHANGE has already run — truncating the shadow now would destroy the parked original. Use stage B or C." >&2
                exit 1
            }
            [[ -n "$(spans_engine spans_local_v2)" ]] || { echo "ERROR: shadow table 'spans_local_v2' not found; nothing to discard." >&2; exit 1; }
            ;;
        B)
            [[ "$engine" != "Distributed" ]] || {
                echo "ERROR: stage B expects the post-EXCHANGE, pre-wrap state, but spans is Distributed (the wrap ran). Use stage C." >&2
                exit 1
            }
            [[ "$end_time" != Nullable* ]] || {
                echo "ERROR: stage B expects spans to hold the successor schema, but end_time is Nullable (the EXCHANGE has not run). Nothing to roll back; use stage A to discard the shadow." >&2
                exit 1
            }
            if [[ -z "$(spans_engine spans_pre_cutover_backup)" ]]; then
                if [[ -n "$(spans_engine spans_local_v2)" ]]; then
                    # State X: the forward EXCHANGE succeeded but its post-swap RENAME did not, so the parked original is
                    # still under spans_local_v2. Finish that RENAME (the same remediation exchange_and_wrap.sh prints),
                    # then stage B proceeds normally. Not auto-completed here: rollback does exactly one thing per run.
                    echo "ERROR: 'spans_pre_cutover_backup' not found but 'spans_local_v2' still exists — the forward" >&2
                    echo "       EXCHANGE's post-swap RENAME did not complete, so the parked original is still under" >&2
                    echo "       'spans_local_v2'. Finish that RENAME, then re-run stage B:" >&2
                    echo "         clickhouse-client ${CH_HOST:+--host $CH_HOST} ${CH_PORT:+--port $CH_PORT} --database $DATABASE --query \"RENAME TABLE $DATABASE.spans_local_v2 TO $DATABASE.spans_pre_cutover_backup ON CLUSTER '{cluster}'\"" >&2
                else
                    echo "ERROR: 'spans_pre_cutover_backup' (parked original) not found; cannot swap back." >&2
                fi
                exit 1
            fi
            ;;
        C)
            [[ "$engine" == "Distributed" ]] || {
                echo "ERROR: stage C expects the post-wrap state (spans = Distributed), but spans is engine='$engine'. The wrap was not applied — use stage B." >&2
                exit 1
            }
            [[ -n "$(spans_engine spans_local)" ]] || { echo "ERROR: 'spans_local' (successor data) not found; topology is not a clean post-wrap state." >&2; exit 1; }
            [[ -n "$(spans_engine spans_pre_cutover_backup)" ]] || { echo "ERROR: 'spans_pre_cutover_backup' (parked original) not found; topology is not a clean post-wrap state." >&2; exit 1; }
            assert_spans_dist_old_free "stage C"
            ;;
    esac
}
...
verify_replay_postcondition() {
    ...
    if ! [[ "$resurrected" =~ ^[0-9]+$ ]]; then
        ...
        echo "         on 'spans'. The rollback is NOT complete: those rows were deleted by users and are being served." >&2
    fi
...
# exclusion bound would wrongly drop a span deleted and then re-created on the successor before the promote — a span
...
# 000004_rollback_unwrap.sql). Guarded on the post-wrap shape. Deliberately does NOT require spans_pre_cutover_backup —
# that is what makes it the only wrap recovery still available after finalize.sh has dropped the backup.
if [[ "$UNWRAP_ONLY" == "1" ]]; then
    unwrap_engine="$(spans_engine spans)"
    [[ -n "$unwrap_engine" ]] || { echo "ERROR: no 'spans' table found in database '$DATABASE'." >&2; exit 1; }
    [[ "$unwrap_engine" == "Distributed" ]] || {
        echo "ERROR: --unwrap-only expects the post-wrap state (spans = Distributed), but spans is engine='$unwrap_engine'." >&2
    ...
    [[ -n "$(spans_engine spans_local)" ]] || {
        echo "ERROR: --unwrap-only: 'spans_local' (the successor shard the wrapper fronts) not found. The topology is not a" >&2
        echo "       clean post-wrap state, and promoting a missing table would leave 'spans' absent. Resolve by hand." >&2
        exit 1
    }
    # 'spans_local' must be the SUCCESSOR, not the original parked under that name by some earlier manual step. Promoting
    # the original here would silently revert the schema with none of the flag reverts or sentinel repair stage B/C carry.
    # Require the column to EXIST and be non-Nullable, in that order: spans_endtime_type returns an empty string for a
    ...
    unwrap_local_end_time="$(spans_endtime_type spans_local)"
    [[ -n "$unwrap_local_end_time" ]] || {
        echo "ERROR: --unwrap-only: 'spans_local' has no 'end_time' column, so it is not the successor this wrapper is" >&2
        echo "       meant to front. Promoting it would make an unrelated table the live 'spans'. Refusing — resolve the" >&2
        echo "       topology by hand." >&2
        exit 1
    }
    [[ "$unwrap_local_end_time" != Nullable* ]] || {
        echo "ERROR: --unwrap-only: 'spans_local' has Nullable end_time, i.e. it holds the ORIGINAL schema, not the" >&2
    ...
    assert_spans_dist_old_free "--unwrap-only"
    ...
    echo "Un-wrap done: the Distributed wrapper is gone and 'spans' is the partitioned successor again."
    echo
    echo "NEXT, in this order:"
    echo "  1. Set databaseAnalyticsDataModel.spansDistributedWrapEnabled=false and roll-restart every backend instance."
    ...
    echo "     on the same side: until the restart completes, span DELETES target the now-absent 'spans_local' and fail"
    echo "     with Code 60 UNKNOWN_TABLE. Reverting the flag first instead would point them at a 'spans' that is still"
    ...
    echo "  2. Leave databaseAnalyticsDataModel.spanColumnsNonNullable=true: the live table keeps the successor's sentinel"
    echo "     schema. Step 1's wrap flag is the only one this stage reverts, and span-delete partition pruning is not a"
    ...
    if [[ -n "$(spans_engine spans_pre_cutover_backup)" ]]; then
        ...
        echo "  (flip spansDistributedWrapEnabled back to true first, per the runbook's toggle/wrap ordering note)."
    else
        ...
        echo "  NOT AVAILABLE as-is: 'spans_pre_cutover_backup' is gone (finalize.sh ran), and exchange_and_wrap.sh"
...
# Recovery mode: re-apply only the reverse-replay against the current live `spans`, for a stage B/C run whose promote
...
# deletion of live rows. So assert the post-promote shape, not just "non-Distributed": `spans` is the ORIGINAL schema
# (Nullable end_time) AND the successor is parked as spans_post_rollback_backup (only a completed promote creates it).
if [[ "$REVERSE_REPLAY_ONLY" == "1" ]]; then
    [[ -n "$CUTOVER_START" ]] || { echo "ERROR: --cutover-start is required for --reverse-replay-only" >&2; exit 2; }
    reverse_replay_engine="$(spans_engine spans)"
    [[ -n "$reverse_replay_engine" ]] || { echo "ERROR: no 'spans' table found in database '$DATABASE'." >&2; exit 1; }
    [[ "$reverse_replay_engine" != "Distributed" ]] || {
        echo "ERROR: 'spans' is Distributed, so the wrap is still applied — the reverse-replay runs on the restored" >&2
    ...
    echo "Reverse-replay-only done: bridged deletes since cutover_start re-applied to the live 'spans'."
...
if [[ "$SENTINEL_REPAIR_ONLY" == "1" ]]; then
    sentinel_engine="$(spans_engine spans)"
    [[ -n "$sentinel_engine" ]] || { echo "ERROR: no 'spans' table found in database '$DATABASE'." >&2; exit 1; }
    [[ "$sentinel_engine" != "Distributed" ]] || {
        echo "ERROR: 'spans' is Distributed, so the wrap is still applied and the repair would target a routing" >&2
    ...
    echo "  READ THE TWO SEPARATELY ON SPANS. A ttft count far larger than the end_time one is the EXPECTED shape here,"
    echo "  not a sign the window is wrong: ttft is a per-LLM-call measurement and most spans are not LLM calls, so the"
    echo "  flag stamped the NaN sentinel on nearly everything written in the window while the epoch end_time arm only"
    echo "  caught spans that were still in flight. The no-window ttft figure is enormous by construction for the same"
    echo "  reason, so it is useful ONLY against the end_time column when judging whether the bounds are right."
    ...
    if ! run_file 000004_rollback_sentinel_repair.sql; then
        ...
        echo "       unfinished mutation on 'spans' in system.mutations, let it finish, then re-run this mode to read" >&2
    ...
    if ! [[ "$after_end_time" =~ ^[0-9]+$ && "$after_ttft" =~ ^[0-9]+$ && "$after_stale" =~ ^[0-9]+$ ]]; then
        ...
        if (( after_end_time > 0 || after_ttft > 0 )); then
            ...
            echo "         spanColumnsNonNullable=true and minting fresh ones behind the repair; --confirm-flag-reverted" >&2
...
if [[ "$STAGE" == "B" || "$STAGE" == "C" ]]; then
    ...
    # parked, and the post-cutover deletes never re-applied, which is user-deleted spans resurrected -- with the stage
    # no longer re-runnable because the tables have already been swapped.
    assert_single_shard
    echo "NOTE: promoting the frozen backup now. Spans the successor accepted after cutover_start ($CUTOVER_START) will" >&2
    echo "      stop being live; recover them from the parked spans_post_rollback_backup (kept until finalize.sh) if needed." >&2
    echo "NOTE: the promote is a single ON CLUSTER RENAME of the live 'spans' — synchronous across the shard's replicas, but" >&2
    echo "      with a brief sub-second cross-replica read skew as it propagates (a read on a lagging replica sees the" >&2
    echo "      pre-rollback 'spans'). On a multi-replica cluster run this in a maintenance moment / with reads quiesced, as for the wrap." >&2
fi

case "$STAGE" in
    A)
        run_file 000004_rollback_stage_a_discard_shadow.sql
        echo "Stage A done: shadow 'spans_local_v2' discarded (now empty); live 'spans' was untouched."
        echo "Nothing to soak or finalize — re-run the backfill to retry the cutover, or stop here."
        echo
        echo "BUT the live 'spans' may still need repairing before you walk away. spanColumnsNonNullable was rolled"
        echo "out before the EXCHANGE, so any span written while it was true carries an epoch/NaN sentinel and a large"
        ...
        echo "  1. Set databaseAnalyticsDataModel.spanColumnsNonNullable=false and roll-restart every backend instance."
        ...
        echo "     It reports 'nothing to repair' and exits 0 if no span landed in that window, so it is safe to run."
        ;;
    B)
...
if [[ "$STAGE" == "B" || "$STAGE" == "C" ]]; then
    echo "Now in the canonical state: spans = original data (live), spans_post_rollback_backup = successor data (parked)."
    echo
    ...
    discarded="$(ch "SELECT count() FROM $DATABASE.spans_post_rollback_backup
    ...
    if [[ "$discarded" =~ ^[0-9]+$ ]]; then
        echo "POST-CUTOVER WRITES NOW NON-LIVE: $discarded row(s) in spans_post_rollback_backup were written after"
        ...
        echo "spans_post_rollback_backup with created_at or last_updated_at at/after cutover_start."
    fi
    ...
    bound_week="$(ch "SELECT least(
                      ...
                      ) FROM spans" 2>/dev/null || true)"
    echo "Verify with the POST-ROLLBACK table pair — the verify.sh defaults no longer apply (spans_local_v2 is gone), and"
    ...
    if [[ "$bound_week" =~ ^-?[0-9]+$ ]] && (( bound_week >= 0 )); then
        echo "  ./verify.sh --database $DATABASE ${CH_HOST:+--host $CH_HOST} ${CH_PORT:+--port $CH_PORT} --old-table spans --new-table spans_post_rollback_backup --to-week $bound_week"
    elif [[ "$bound_week" =~ ^-?[0-9]+$ ]]; then
        echo "  (none: every row in 'spans' predates no earlier week than the cutover window's own, so there is nothing"
        echo "   to compare below it. Skip the bounded compare.)"
    else
        echo "  ./verify.sh --database $DATABASE ${CH_HOST:+--host $CH_HOST} ${CH_PORT:+--port $CH_PORT} --old-table spans --new-table spans_post_rollback_backup --to-week <N>"
        echo "  where N could not be computed just now: it is the whole weeks between toMonday(min(created_at)) on"
        echo "  'spans' and cutover_start's Monday, minus 1."
    fi
    ...
    echo "was about."
    echo
    echo "A mismatch inside that bound is NOT automatically corruption. Any write touching a PRE-EXISTING span after"
    ...
    echo
    echo "NEXT, on the restored original (see README 'Rolling back the spanColumnsNonNullable flip'):"
    if [[ "$STAGE" == "C" ]]; then
        echo "  0. Set databaseAnalyticsDataModel.spansDistributedWrapEnabled=false and roll-restart. Stage C removed the"
        echo "     wrapper and parked 'spans_local', so a stale true sends span DELETEs at a table that no longer exists"
    ...
    echo "     It needs ALTER UPDATE(end_time) / ALTER UPDATE(ttft) on 'spans', which the rollback grant set does not"
    echo "     carry — the mode explains the ACCESS_DENIED if you hit it."
    echo
    echo "LAST, and only once every step above has landed: finalize.sh recycles spans_post_rollback_backup into an"
    echo "empty spans_local_v2. That is the irreversible step — it destroys the only copy of the post-cutover writes"
