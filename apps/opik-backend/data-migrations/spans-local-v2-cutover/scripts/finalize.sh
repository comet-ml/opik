#!/usr/bin/env bash
#
# Finalizes the cutover once the parked backup has soaked and the live `spans` is confirmed healthy (runbook:
...
#   * after a successful cutover -> the old original is parked as `spans_pre_cutover_backup` (the live successor is
#                                   `spans`, or `spans_local` behind the Distributed wrapper). DROP it to commit to the
#                                   new layout.
#   * after a rollback           -> the abandoned successor is parked as `spans_post_rollback_backup` (the original is
#                                   live as `spans`). That table IS the migration-000115 `spans_local_v2` object,
#                                   renamed (a replica path is fixed at CREATE and survives renames). RECYCLE it into an
#                                   EMPTY `spans_local_v2` (TRUNCATE + RENAME): discards the successor data but restores
#                                   the exact 000115 shadow — schema, codecs (000116) and replica path — so the
#                                   estate matches the applied Liquibase state and a retry starts from a clean shadow.
# Both are `*_backup` names — retained until this script runs; the working `spans_local_v2` shadow is never detected as a
# backup. This detects whichever parked table is present and never touches the live `spans` / `spans_local` shard.
...
# loudly instead. It also refuses if the live `spans` is empty ACROSS THE CLUSTER (max rows over replicas) while the
...
# recycle — if `spans_local_v2` already exists (recycle renames the backup INTO that name; a stray shadow means a retry
...
#                     AFTER A CUTOVER, for the DROP of spans_pre_cutover_backup, which holds every span written to the
...
# group by host and take the most-caught-up one; the post-cutover DROP path's Distributed `spans` already aggregates the
...
classify spans
[[ "$CLUSTER_HAS" == "1" ]] || { echo "ERROR: live 'spans' table not found on all replicas in '$DATABASE'." >&2; exit 1; }

# Detect the parked backup by name: spans_pre_cutover_backup (post-successful-cutover) or spans_post_rollback_backup
# (post-rollback). They never co-exist in a clean flow; if both are present the estate is ambiguous — refuse.
classify spans_pre_cutover_backup;   HAS_PRECUTOVER="$CLUSTER_HAS"
classify spans_post_rollback_backup; HAS_POST_ROLLBACK="$CLUSTER_HAS"

if [[ "$HAS_PRECUTOVER" == "1" && "$HAS_POST_ROLLBACK" == "1" ]]; then
    echo "ERROR: both 'spans_pre_cutover_backup' and 'spans_post_rollback_backup' exist — ambiguous state." >&2
    echo "       Expected exactly one parked backup. Investigate and drop the correct one by hand." >&2
    exit 1
elif [[ "$HAS_PRECUTOVER" == "1" ]]; then
    BACKUP="spans_pre_cutover_backup"
elif [[ "$HAS_POST_ROLLBACK" == "1" ]]; then
    BACKUP="spans_post_rollback_backup"
else
    echo "Nothing to finalize: no parked backup ('spans_pre_cutover_backup' or 'spans_post_rollback_backup') exists."
    exit 0
fi
...
if [[ "$BACKUP" == "spans_pre_cutover_backup" ]]; then
...
if [[ "$CONFIRM" == "1" && "$FLAG_GIVEN" != "1" ]]; then
    ...
    if [[ "$BACKUP" == "spans_pre_cutover_backup" ]]; then
        echo "       This table holds every span written to the old table between the last delta pass and the EXCHANGE." >&2
...
if [[ "$LIVE_ROWS" == "0" && "$BACKUP_ROWS" != "0" ]]; then
    echo "ERROR: live 'spans' is empty but '$BACKUP' has $BACKUP_ROWS rows. Refusing to drop the backup —" >&2
    echo "       verify the live table is the healthy one before finalizing." >&2
    exit 1
fi

echo "Live 'spans': $LIVE_ROWS rows. Parked '$BACKUP': $BACKUP_ROWS rows."
...
if [[ "$BACKUP" == "spans_post_rollback_backup" ]]; then
    # Rollback finalize: recycle the parked successor — physically the 000115 `spans_local_v2` object (its replica path
    # is fixed at CREATE and unchanged by the rename) — back into an empty `spans_local_v2`. ClickHouse has no single
    # truncate-and-rename, so this is two statements, each atomic PER HOST and ON CLUSTER; ordered TRUNCATE-then-RENAME so
    # the only state a crash between them can leave is an empty `spans_post_rollback_backup` — which re-running finalize
    ...
    # silently corrupting (RENAME-first could strand a populated `spans_local_v2` that a retry backfill would mis-skip). ACROSS the
    ...
    # backup / disposable shadow — never the live `spans` — so unlike the rollback promote and the wrap (which rename live
    # `spans`) the brief cross-replica skew is invisible to readers, and finalize needs no maintenance window.
    #
    # Guard the destination first: recycle renames the backup INTO `spans_local_v2`, and ClickHouse RENAME fails on an
    # existing target. A stray `spans_local_v2` here means a retry cutover started before this rollback was finalized —
    # refuse (cluster-wide) BEFORE truncating, so we fail early with a clear message instead of after the TRUNCATE.
    classify spans_local_v2
    if [[ "$CLUSTER_HAS" != "0" ]]; then
        echo "ERROR: 'spans_local_v2' already exists — cannot recycle '$BACKUP' into it (RENAME will not overwrite)." >&2
        echo "       This usually means a retry cutover began before the rollback was finalized. Resolve the estate" >&2
        echo "       (inspect/drop 'spans_local_v2') before recycling." >&2
        exit 1
    fi
    if [[ "$CONFIRM" != "1" ]]; then
        echo "DRY RUN: would recycle $DATABASE.$BACKUP into an empty $DATABASE.spans_local_v2 (TRUNCATE + RENAME)."
    ...
    ch "RENAME TABLE $BACKUP TO spans_local_v2 ON CLUSTER '{cluster}'"
    echo "Recycled $DATABASE.$BACKUP into an empty $DATABASE.spans_local_v2. The rollback is finalized."
else
    if [[ "$CONFIRM" != "1" ]]; then
