...
while (( PASS < MAX_PASSES )); do
    ...
    if gate_is_clean; then
        ...
        if [[ "$DIRECTION" == "forward" ]]; then
            ...
            echo "     EXPECT A MISMATCH ON A HEALTHY CUTOVER; a clean PASS is not generally obtainable. Two"
            echo "     post-swap shapes cause it, and only the first can be quiesced away:"
            echo "       * a span DELETED after the swap: masked here, still live in the frozen backup, and 000005"
            echo "         knows nothing of the deletion bridge."
            echo "       * a span REWRITTEN after the swap -- the same shape newer_keys reports above. Its later"
            echo "         created_at both drops it out of this window on the live side (both sides are bounded on"
            echo "         created_at) and changes its fingerprint. Quiescing cannot undo a rewrite already made."
            echo "     Read the differing keys, not the verdict: against deletion_events_local at/after --swap-done,"
            echo "     and against the live table for a created_at past --swap-done. The write-loss gate is the three"
            echo "     gating counts above, not that compare."
