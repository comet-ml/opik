...
while (( PASS < MAX_PASSES )); do
    ...
    if gate_is_clean; then
        ...
        if [[ "$DIRECTION" == "forward" ]]; then
            ...
            echo "     EXPECT A MISMATCH ON A HEALTHY CUTOVER; a clean PASS is not generally obtainable. ANY"
            echo "     post-swap change to a gap-window span causes one, and only a delete can be quiesced away:"
            echo "       * DELETED after the swap: masked here, still live in the frozen backup (000005 knows"
            echo "         nothing of the deletion bridge). src_rows > dst_rows."
            echo "       * RE-CREATED after the swap: the delete masked the prior row, so the insert had nothing to"
            echo "         merge onto and stamped a fresh created_at. Both sides are bounded on created_at, so it"
            echo "         leaves this window on the live side. src_rows > dst_rows."
            echo "       * PATCHED after the swap: the merge PRESERVES created_at, so the row stays in the window"
            echo "         and the row counts stay EQUAL -- only the checksums differ."
            echo "     Neither of the last two can be quiesced away. Note which counts they reach: only a PATCH"
            echo "     reaches newer_keys above -- a bridged delete is dropped from the parked set BEFORE bucketing,"
            echo "     so a DELETE and a RE-CREATE are reported in no count at all. A key this compare flags and the"
            echo "     counts say nothing about is the expected shape of a bridged delete, not a gap in the gate."
