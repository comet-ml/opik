#!/usr/bin/env bash
#
# POST-SWAP reconciliation driver for the spans cutover (runbook: ../README.md).
#
...
# window — the procedure takes no ingestion-path hold (OPIK-8239) — so every span written to the old `spans` in it is
...
#
#   forward (after a cutover)  sweep spans_pre_cutover_backup   -> the live successor, then re-apply bridged deletes.
#   reverse (after a rollback) sweep spans_post_rollback_backup -> the restored original, then re-run the reverse replay.
#
...
#                             excluded from the sweep, so a gap-window span deleted after the swap is not brought back.
...
#                                 span stays missing under a clean gate.
...
#                             INSERT (throw_on_max_partitions_per_insert_block = 1). Pass the value backfill.sh's
#                             backfill used, which is what this default already is: like the delta, the sweep has no
#                             created_at window to bound it (a gap-window row's id can be any age), so one sweep
...
#                             runs inside the cutover window, where peak memory matters more than the part count its
#                             small volume produces. It is the bound that binds on spans — rows are wide uncompressed,
#                             so the row-count cap rarely fires first — and unlike the partition cap it is NOT inert on
#                             the reverse path.
...
#                             which only re-applies BRIDGED keys, does not mask it again. The span comes back.
...
#                             created_at, so an OLD span updated during the gap window sits inside the sweep's window
#                             and inside retention's id range at once.
#   --settle-timeout N        seconds the replication-settle gate polls before giving a verdict. Default 600, capped at
#                             7200; 0 takes a single sample. Unlike the pre-swap gate this one sits AFTER the swap, so
#                             its wait costs no cutover tail — but every second is a second the gap-window spans are
...
#
#                             600 RATHER THAN exchange_and_wrap.sh's 1800, AND THE CONSEQUENCE IS STATED RATHER THAN
#                             HIDDEN. The stuck-AGE threshold is 1800s on both drivers, sized from spans' very large parts
#                             (see exchange_and_wrap.sh --settle-timeout). At a 600s budget an entry therefore cannot
#                             age past that threshold within one gate, so post-swap the QUEUE verdict degrades to
#                             retries and last_exceptions only. That is an acceptable trade HERE and not pre-swap,
#                             because post-swap the hazard the gate primarily guards is different: an unfinished
#                             MUTATION on the frozen parked table — a user delete that fired just before the swap and
#                             would otherwise be read as live by the mask-honored sweep — and that verdict is
#                             UNCONDITIONAL and fully reachable inside 600s, since such mutations drain rather than
#                             recur. The pre-swap gate has no equivalent second signal, which is why it spends the
#                             longer budget. Raise this to 1800 to restore the age verdict, at
#                             (1 + --max-passes) x 1800 = up to two hours of worst case.
...
FORCE=0
SETTLE_TIMEOUT=600        # seconds the settle gate polls before deciding. See --settle-timeout.
SETTLE_TIMEOUT_MAX=7200   # its accepted ceiling; the validation below explains why the check is lexical.

# Stuck-ness thresholds for the replication queue, and the poll interval. Deliberately not flags, and the same values
# exchange_and_wrap.sh uses: they describe what "a replica is genuinely lagging" means, not a per-run choice — and
# keeping them identical across the two drivers is what stops the shared gate meaning two different things on either
...
LIVE_TABLE=""             # spans, or spans_local on a wrapped estate
PARKED_TABLE=""           # spans_pre_cutover_backup | spans_post_rollback_backup
...
[[ -n "$SWAP_DONE" ]] || {
    ...
    echo "       cutover, 'RECORD promote_done=' after a rollback — and it is what stops the sweep resurrecting a span" >&2
...
ch() {
    clickhouse-client "${CH_ARGS[@]}" --log_comment 'spans_local_v2_cutover:reconcile' \
...
# The live table this run reads, writes and mutates. On a wrapped estate `spans` is a Distributed wrapper: it accepts
# INSERT but REJECTS mutations (code 36/48), so the deletion replay after the sweep would fail. Resolve ONE name the way
# databaseAnalyticsDataModel.spansDistributedWrapEnabled does in SpanDAO — spans_local when `spans` is Distributed —
...
resolve_live_table() {
    ...
    if [[ "$engine" == "Distributed" ]]; then
        LIVE_TABLE="spans_local"
        [[ -n "$(spans_engine spans_local)" ]] || {
            echo "ERROR: 'spans' is a Distributed wrapper but 'spans_local' (the shard it fronts) does not exist, so" >&2
...
detect_direction() {
    ...
    if [[ -n "$pre_cutover_engine" && -n "$post_rollback_end_time_type" ]]; then
        echo "ERROR: both 'spans_pre_cutover_backup' and 'spans_post_rollback_backup' exist, so the direction is" >&2
    ...
    if [[ -n "$pre_cutover_engine" ]]; then
        DIRECTION="forward"
        PARKED_TABLE="spans_pre_cutover_backup"
        ...
        # and for the same reason: spans_endtime_type returns an empty string for a missing column, and "" is not
        ...
        [[ -n "$live_end_time_type" ]] || {
            echo "ERROR: the live '$LIVE_TABLE' has no 'end_time' column, so it is not the spans table this driver can" >&2
        ...
        [[ "$live_end_time_type" != Nullable* ]] || {
            echo "ERROR: 'spans_pre_cutover_backup' is parked, but the live '$LIVE_TABLE' still has a Nullable end_time —" >&2
    ...
    if [[ -n "$post_rollback_end_time_type" ]]; then
        [[ "$post_rollback_end_time_type" != Nullable* ]] || {
            echo "ERROR: 'spans_post_rollback_backup' exists but carries the ORIGINAL schema (Nullable end_time), so it is" >&2
        ...
        DIRECTION="reverse"
        PARKED_TABLE="spans_post_rollback_backup"
        MUTATING_BLOCKS=(reverse-sweep)
        [[ "$live_end_time_type" == Nullable* ]] || {
            echo "ERROR: the successor is parked as 'spans_post_rollback_backup', but the live '$LIVE_TABLE' is not the" >&2
    ...
    # EXCHANGE committed but its post-swap RENAME did not, so the parked original is still sitting under spans_local_v2.
    # exchange_and_wrap.sh and rollback.sh print the same remediation.
    if [[ -n "$live_end_time_type" && "$live_end_time_type" != Nullable* && -n "$(spans_engine spans_local_v2)" ]]; then
        echo "ERROR: the EXCHANGE ran (the live '$LIVE_TABLE' holds the successor schema) but 'spans_local_v2' still" >&2
    ...
    echo "ERROR: no parked backup found — neither 'spans_pre_cutover_backup' nor 'spans_post_rollback_backup' exists." >&2
...
#     has not applied on the replica the sweep reads, the sweep sees the row as live and copies a deleted span back.
#
...
#     advisory does not report it. A genuinely deleted span comes back while the gate reports clean, and no other
...
assert_replication_settled() {
    ...
    if (( mutations != 0 )); then
        ...
        echo "       it mask-honored, so sweeping now would copy a deleted span back into the live table." >&2
...
validate_blocks() {
    ...
    [[ "$DIRECTION" != "reverse" ]] || validate_block "$SWEEP_SQL" reverse-usage-range-check
...
# REVERSE ONLY. The one narrowing anywhere in this procedure: the successor's usage is Map(String, Int64) and the
# restored original's is Map(String, Int32), so the reverse sweep converts on insert. ClickHouse converts rather than
# refusing, so an out-of-range value would land as a wrapped, silently wrong token count in a column the product reads
# as one — and unlike everything else the reverse sweep does, that would not be recoverable from the parked successor
# once it is finalized, because the wrong value would look like data.
#
# Expected to be 0 on any real estate: `usage` holds LLM token counts and 2,147,483,647 tokens for one span is not a
# number a model produces. So this normally proves the narrowing is a no-op, at the cost of one gap-window read. It is
# a REFUSAL rather than a warning on the mutating path — importing a knowingly-corrupt value is not a trade an operator
# should be able to make by not reading a line of output.
assert_reverse_usage_in_range() {
    local out
    [[ "$DIRECTION" == "reverse" ]] || return 0
    out="$(clickhouse-client "${CH_ARGS[@]}" --format TabSeparated \
        --query "$(render "$(extract "$SWEEP_SQL" reverse-usage-range-check)")" 2>/dev/null || true)"
    if ! [[ "$out" =~ ^[0-9]+$ ]]; then
        echo "ERROR: the usage-range precheck could not be read, so it is unknown whether the reverse sweep would" >&2
        echo "       narrow a Map(String, Int64) value out of Int32 range. Treat this as unverified, not as clean:" >&2
        echo "       fix connectivity and re-run — no mutation was issued." >&2
        exit 1
    fi
    if [[ "$out" == "0" ]]; then
        echo "  usage_out_of_int32_range=0 — the Map(String, Int64) -> Map(String, Int32) narrowing is a no-op here."
        return 0
    fi
    if [[ "$REPORT_ONLY" == "1" ]]; then
        echo "  WARNING: usage_out_of_int32_range=$out — a real run would REFUSE. See below." >&2
    fi
    echo "ERROR: $out row(s) in the gap window carry a 'usage' value outside Int32. The reverse sweep writes the" >&2
    echo "       successor's Map(String, Int64) back into the original's Map(String, Int32), and ClickHouse converts" >&2
    echo "       rather than refusing — so those values would land WRAPPED and silently wrong. Refusing to import." >&2
    echo "       List them out of '$PARKED_TABLE' (retained until finalize.sh) and resolve them by hand:" >&2
    echo "         SELECT workspace_id, project_id, trace_id, id, usage FROM $DATABASE.$PARKED_TABLE" >&2
    echo "         WHERE (created_at >= toDateTime64('$EFFECTIVE_GAP_START', 6, 'UTC')" >&2
    echo "             OR last_updated_at >= toDateTime64('$EFFECTIVE_GAP_START', 6, 'UTC'))" >&2
    echo "           AND arrayExists(v -> v > 2147483647 OR v < -2147483648, mapValues(usage))" >&2
    exit 1
}
...
print_counts() {
    ...
    echo "  newer_keys=$NEWER  — informational, and EXPECTED to be non-zero: a gap-window span written again after the"
...
if [[ "$DIRECTION" == "forward" ]]; then
    ...
    [[ "$LIVE_TABLE" == "spans" ]] || {
        echo "ERROR: the reverse direction expects the restored original live under 'spans', but this estate resolves its" >&2
        echo "       live table to '$LIVE_TABLE' (i.e. 'spans' is a Distributed wrapper). A rollback removes the wrapper," >&2
...
if [[ "$REPORT_ONLY" != "1" && "$CONFIRM_RETENTION_PAUSED" != "1" ]]; then
    ...
    echo "       this driver cannot see them: one that fired after '$PARKED_TABLE' froze leaves its span masked on" >&2
...
# Run BEFORE the --report-only exit, so a report says whether a real run would be refused rather than leaving that to
# be discovered later — and before the first pass, so nothing is imported on the strength of an unchecked narrowing.
# A no-op in the forward direction, which writes no Map into a narrower column.
assert_reverse_usage_in_range
...
while (( PASS < MAX_PASSES )); do
    ...
    if gate_is_clean; then
        ...
        if [[ "$DIRECTION" == "forward" ]]; then
            echo "Every span written to the old table in the [$EFFECTIVE_GAP_START, swap) gap is live on '$LIVE_TABLE'."
            echo
            echo "NEXT:"
            ...
            echo "     EXPECT A MISMATCH ON A HEALTHY CUTOVER; a clean PASS is not generally obtainable, and"
            echo "     quiescing undoes no change already made. Three properties of a post-swap write decide what"
            echo "     you see here -- reason from these, not from a catalogue of shapes:"
            echo "       * DID IT WIN? Both sides read under FINAL/argMax on last_updated_at, which is CLIENT-"
            echo "         SUPPLIED and need not advance. A rewrite carrying an OLDER one leaves the parked row"
            echo "         selected on both sides and this compare EQUAL -- equality is not evidence that no"
            echo "         post-swap write happened."
            echo "       * DID created_at SURVIVE? The merge preserves it when a row survived to merge onto, and"
            echo "         stamps a fresh one when a delete masked the prior row first. Both sides are bounded on"
            echo "         created_at: preserved keeps the key in the window with EQUAL counts and differing"
            echo "         checksums; freshly stamped moves it out, src_rows > dst_rows."
            echo "       * WAS A DELETE BRIDGED at/after --swap-done? Those keys are dropped from the parked set"
            echo "         BEFORE bucketing, so they reach NO count. A key this compare flags and the counts say"
            echo "         nothing about is the expected shape of a bridged delete, not a gap in the gate."
            echo "     Do not diagnose on row counts alone: equal counts do not mean the compare agreed. Read the"
            echo "     keys, not the verdict -- against deletion_events_local at/after --swap-done, against the live"
            echo "     table for a created_at past it, and for the rest against last_updated_at past it. The"
            echo "     write-loss gate is the three gating counts above, not that compare."
