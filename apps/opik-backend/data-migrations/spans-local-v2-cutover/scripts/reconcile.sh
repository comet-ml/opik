...
while (( PASS < MAX_PASSES )); do
    ...
    if gate_is_clean; then
        ...
        if [[ "$DIRECTION" == "forward" ]]; then
            ...
            echo "     The last two are what newer_keys reports above, and neither can be quiesced away. Do not"
            echo "     diagnose on row counts alone: equal counts do not mean the compare agreed. Read the differing"
