...
6. **QA over the reconciled range — `verify.sh --window-from/--window-to`** for the payload-level picture over exactly
   ...
   > `--swap-done`, against the live table for a `created_at` past it, and for the rest against `last_updated_at`
   > past it — and read step 5's `missing_keys`, which applies exactly the exclusion this compare lacks, rather than
   > writing a query to tell them apart. See ["When the cutover is done"](#when-the-cutover-is-done).
...
| the **replication queue** on `spans` / `spans_local_v2` **and `deletion_events_local`** | **Stuck-ness, not depth.** Counts only `GET_PART`/`ATTACH_PART` — the entries that mean a replica lacks data. Merges and mutations also sit in this queue and say nothing about completeness; counting them would fail the gate on the large merges that follow a backfill. It passes the moment the queue drains to 0. If it has not drained by the deadline, the gate reports the oldest entry's age, the highest `num_tries` and whether any entry carries a `last_exception` — an entry older than 60s, more than 3 retries, or any recorded exception means a replica is genuinely lagging and the gate **fails loudly, printing the offending entries per replica**. A queue that is busy but not stuck is accepted, with the numbers printed so the operator sees what was accepted. The **bridge** is in scope for the same reason it is post-swap: the final deletion replay this driver runs immediately before the swap reads `deletion_events_local` for both its bridge match and its resurrection guard, and [`000002`](scripts/db-app-analytics/000002_delta_and_deletion_replay.sql) rests that statement's correctness on the bridge being identical on every node. A replica short of a bridge part masks fewer keys, and `lightweight_deletes_sync = 2` does not catch it — it waits for each replica to finish its own mutation, not for them to have masked the same keys. |
