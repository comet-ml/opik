...
if [[ -z "$MAX_INSERT_THREADS" ]]; then
    ...
    if [[ -n "${shared//[[:space:]]/}" ]]; then
        echo "ERROR: max_insert_threads shares a SETTINGS line with another setting in $SQL_FILE:" >&2
        echo "       $shared" >&2
    ...
    executable="$(grep -v '^[[:space:]]*--' <<<"$sql" || true)"
    if ! grep -qF '${MAX_INSERT_THREADS}' <<<"$executable"; then
        echo "ERROR: --max-insert-threads $MAX_INSERT_THREADS was requested, but $SQL_FILE has no" >&2
