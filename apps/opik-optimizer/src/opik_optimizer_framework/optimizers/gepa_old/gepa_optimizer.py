...
class GepaLegacyOptimizer:
    ...
    def run(
        ...
        try:
            ...
            raise ImportError(
                "The 'gepa' package is required for GepaLegacyOptimizer. "
        ...
        prompt_messages = context.baseline_config.get("prompt_messages", [])
        seed_candidate = build_seed_candidate(prompt_messages)
        ...
        logger.info(
            "GepaLegacyOptimizer finished: %d trials recorded",
            len(state.trials),
        )
