...
class TestFrameworkGEPAAdapterEvaluate:
    ...
    @patch("gepa.core.adapter.EvaluationBatch")
    def test_experiment_type_minibatch_for_reflection(self, mock_eb_cls):
        """Minibatch reflection eval (capture_traces=True) → 'mini-batch'."""
        mock_eb_cls.side_effect = lambda **kwargs: SimpleNamespace(**kwargs)
        mock_eval_adapter = MagicMock()
        trial = _make_trial("c-1", 0.9)
        mock_eval_adapter.evaluate_with_details.return_value = (trial, None)
    ...
    def test_experiment_type_mutation_for_new_candidate(self, mock_eb_cls):
        """Mutated candidate eval (capture_traces=False) → 'mutation'."""
        mock_eb_cls.side_effect = lambda **kwargs: SimpleNamespace(**kwargs)
        mock_eval_adapter = MagicMock()
        trial = _make_trial("c-1", 0.9)
        mock_eval_adapter.evaluate_with_details.return_value = (trial, None)
    ...
    @patch("gepa.core.adapter.EvaluationBatch")
    def test_experiment_type_none_for_validation(self, mock_eb_cls):
        """During iteration without on_evaluation_start metadata, experiment_type is None (full trial)."""
        mock_eb_cls.side_effect = lambda **kwargs: SimpleNamespace(**kwargs)
        mock_eval_adapter = MagicMock()
        trial = _make_trial("c-1", 0.9)
        mock_eval_adapter.evaluate_with_details.return_value = (trial, None)
