...
def _make_trial(candidate_id: str, score: float, config_hash: str = "h") -> TrialResult:
    return TrialResult(
...
class TestResultAggregator:
    def test_record_trial_adds_to_state(self):
        ...
        trial = _make_trial("c1", 0.8, "hash1")
    ...
    def test_record_trial_updates_best(self):
        state = OptimizationState()
        t1 = _make_trial("c1", 0.5, "h1")
        t2 = _make_trial("c2", 0.9, "h2")
        t3 = _make_trial("c3", 0.7, "h3")
    ...
    def test_get_best_returns_highest(self):
        state = OptimizationState()
        record_trial(state, _make_trial("c1", 0.3, "h1"))
        record_trial(state, _make_trial("c2", 0.9, "h2"))
        record_trial(state, _make_trial("c3", 0.6, "h3"))
