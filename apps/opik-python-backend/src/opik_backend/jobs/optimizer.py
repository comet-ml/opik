...
OPTIMIZER_RUNNER_PATH = os.path.join(os.path.dirname(__file__), "optimizer_runner.py")
...
def _mark_optimization_error(context: OptimizationJobContext) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "Marked optimization %s as ERROR from parent worker",
                context.optimization_id,
            )
        finally:
            # Release the httpx connection pool — this client is created per call.
            client.end()
        ...
        with CancellationHandle(
            ...
            try:
                ...
                try:
                    ...
                    user_message = subprocess_user_message or to_user_facing_message(
                        exc
                    )
