...
# Score code templates for different metric types
SCORE_CODE_GEVAL_TEMPLATE = """
    return metric.score(
        input=dataset_item,
        output=llm_output
    )
...
SCORE_CODE_JSON_SCHEMA_TEMPLATE = """
...
USER_CODE_TEMPLATE = """
# Generated by Opik's Optimization Studio
...
def format_python_value(value: Any, indent: int = 0) -> str:
    ...
    This is a custom implementation rather than using a library because:
    - We need to generate Python code strings (not JSON), requiring specific formatting
    - We need proper indentation for nested structures (dicts, lists)
    - We need to escape strings appropriately for Python code (not JSON)
    - Standard libraries like json.dumps() or repr() don't meet these requirements
    - The function handles edge cases like nested structures and proper escaping
...
class MetricCodeGenerator:
    ...
    def _generate_from_introspection(
        ...
        if metric_type == "geval":
            score_code = SCORE_CODE_GEVAL_TEMPLATE
        elif metric_type == "json_schema_validator":
            schema_key = metric_params.get("schema_key", "json_schema")
            score_code = SCORE_CODE_JSON_SCHEMA_TEMPLATE.format(
                schema_key=format_python_value(schema_key),
                reason=format_python_value(
                    f"Missing schema in dataset item key '{schema_key}'"
                ),
            )
        else:
            # equals, levenshtein_ratio
            reference_key = metric_params.get("reference_key", "answer")
            score_code = SCORE_CODE_REFERENCE_TEMPLATE.format(
                reference_key=format_python_value(reference_key)
            )
