...
import { Plus, RefreshCw, Code, Play, Edit, Trash2, Copy, ExternalLink } from 'lucide-react';
...
const CustomApisPage: React.FC = () => {
  ...
  const loadInstances = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: `Failed to load custom APIs: ${error instanceof Error ? error.message : 'Unknown error'}`,
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)}>
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="list" className="flex items-center space-x-2">
            ...
            <span>APIs ({instances.length})</span>
          </TabsTrigger>
