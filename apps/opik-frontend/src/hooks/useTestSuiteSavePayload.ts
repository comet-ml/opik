...
interface UseTestSuiteSavePayloadOptions {
  suiteId: string;
...
export function useTestSuiteSavePayload({
  suiteId,
  suite,
  versionEvaluators,
}: UseTestSuiteSavePayloadOptions) {
  ...
  const buildPayload = useCallback(
    ({
      ...
      const state = useTestSuiteDraftStore.getState();
      ...
      const isTestSuite = suite.type === DATASET_TYPE.TEST_SUITE;
  ...
  const buildInitialVersionPayload = useCallback(
    ({ tags, changeDescription }: BuildPayloadOptions) => {
      ...
      if (isTestSuite && state.suiteAssertions !== null) {
        const originalEvaluator = versionEvaluators[0];
