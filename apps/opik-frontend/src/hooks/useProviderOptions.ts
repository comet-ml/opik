...
export interface ProviderEnabledMap {
  ...
  [PROVIDER_TYPE.REQUESTY]: boolean;
...
export function useProviderEnabledMap(): ProviderEnabledMap {
  ...
  const isRequestyEnabled = useIsFeatureEnabled(
    FeatureToggleKeys.REQUESTY_PROVIDER_ENABLED,
  );
  ...
  return useMemo(
    () => ({
      ...
      [PROVIDER_TYPE.REQUESTY]: isRequestyEnabled,
