...
describe("useChartTickDefaultConfig", () => {
  ...
  describe("configuration options", () => {
    it("should respect custom numberOfTicks", () => {
      const { result } = renderHook(() =>
        useChartTickDefaultConfig([0, 10, 20, 30], { numberOfTicks: 3 }),
      );

      expect(result.current.ticks.length).toBeLessThanOrEqual(3);
    });

    it("should respect explicit tickPrecision override", () => {
      const { result } = renderHook(() =>
        useChartTickDefaultConfig([1.23456, 2.34567, 3.45678], {
          tickPrecision: 1,
        }),
      );

      const formatted = result.current.yTickFormatter(1.23456);
