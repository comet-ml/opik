...
import { RUNNERS_KEY } from "@/api/api";
...
const PairingDialog: React.FunctionComponent<PairingDialogProps> = ({
  ...
  const { data: runner } = useMyRunner({
    refetchInterval: pairingCode ? 2000 : false,
    enabled: open && !!pairingCode,
  });

  useEffect(() => {
    if (open && runner?.status === "connected") {
      setOpen(false);
      toast({ description: `Connected to ${runner.name}` });
    }
  }, [runner, open, setOpen, toast]);
  ...
  const handleGenerate = useCallback(() => {
    pairMutation.mutate(undefined, {
      onSuccess: (data) => {
        ...
        if (data.runner_id) {
          setStoredRunnerId(data.runner_id);
          queryClient.invalidateQueries({ queryKey: [RUNNERS_KEY] });
        }
      },
    });
  }, [pairMutation, queryClient]);
