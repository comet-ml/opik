...
const AnnotationQueuesTab: React.FunctionComponent<AnnotationQueuesTabProps> = ({
  ...
  const columnsDef: ColumnData<AnnotationQueue>[] = useMemo(() => {
    return [
      ...
      },
      {
        ...
        type: COLUMN_TYPE.string,
        accessorFn: (row) => {
          if (!row.feedback_scores || row.feedback_scores.length === 0) return "-";
          const avgScore = row.feedback_scores.reduce((sum, score) => sum + score.value, 0) / row.feedback_scores.length;
      ...
      },
      {
        ...
        label: "Scope",
        type: COLUMN_TYPE.string,
        accessorFn: (row) => row.scope === AnnotationQueueScope.TRACE ? "Traces" : "Threads",
      },
      {
  ...
  const handleRowClick = useCallback((queue: AnnotationQueue) => {
    navigate({
