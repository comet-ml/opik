...
import { X, Save, Trash2, TestTube, CheckCircle, XCircle, Loader, Eye, RefreshCw } from 'lucide-react';
...
interface ConfigPanelProps {
  ...
  onRefresh?: (currentConfig: any) => void;
}

const ConfigPanel: React.FC<ConfigPanelProps> = ({ widget, onClose, onUpdate, onRefresh }) => {
  ...
  const handleSave = () => {
    console.log('🔵 ConfigPanel handleSave - Save Changes button clicked!');
    console.log('🔵 ConfigPanel handleSave - Widget ID:', widget?.id);
    console.log('🔵 ConfigPanel handleSave - Config being saved:', JSON.stringify(config, null, 2));
    
    try {
      validateWidgetConfig(config);
      console.log('🔵 ConfigPanel handleSave - Validation passed, calling onUpdate');
      onUpdate({ config });
      console.log('🔵 ConfigPanel handleSave - onUpdate called successfully');
      setErrors([]);
    } catch (error) {
      console.log('🔴 ConfigPanel handleSave - Validation error:', error);
