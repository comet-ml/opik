...
const PrettyLLMMessageHeader: React.FC<PrettyLLMMessageHeaderProps> = ({
  ...
  return (
    <CustomAccordionTrigger
      className={cn(
        "flex items-center justify-between gap-1 rounded-sm p-1 transition-colors group-hover/llm-message:bg-primary-foreground [&[data-state=open]>div>svg:first-child]:rotate-90",
        className,
      )}
    >
