...
const VertexAIModelConfigs = ({
  ...
  return (
    <div className="flex w-72 flex-col gap-6">
      ...
      <SliderInputControl
        value={configs.maxRetries ?? DEFAULT_VERTEX_AI_CONFIGS.MAX_RETRIES}
        onChange={(v) => onChange({ maxRetries: v })}
        id="maxRetries"
        min={0}
        max={10}
        step={1}
        defaultValue={DEFAULT_VERTEX_AI_CONFIGS.MAX_RETRIES}
        label="Max retries"
        tooltip={
      ...
      <SliderInputControl
        value={configs.timeout ?? DEFAULT_VERTEX_AI_CONFIGS.TIMEOUT}
        onChange={(v) => onChange({ timeout: v })}
        id="timeout"
        min={1}
        max={300}
        step={1}
        defaultValue={DEFAULT_VERTEX_AI_CONFIGS.TIMEOUT}
      ...
      <SliderInputControl
        ...
        max={5}
        step={0.1}
        defaultValue={DEFAULT_VERTEX_AI_CONFIGS.BACKOFF_EXPONENT}
