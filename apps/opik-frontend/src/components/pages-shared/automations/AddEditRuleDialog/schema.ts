...
const LLMJudgeBaseSchema = z.object({
  ...
  config: z.object({
    ...
    throttling: z.number().min(0).optional().nullable(),
    maxConcurrentRequests: z.number().int().min(1).optional().nullable(),
  }),
...
export const convertLLMJudgeObjectToLLMJudgeData = (data: LLMJudgeObject) => {
  return {
    ...
    config: {
      ...
      throttling: data.model?.throttling ?? null,
      maxConcurrentRequests: data.model?.max_concurrent_requests ?? null,
    },
...
export const convertLLMJudgeDataToLLMJudgeObject = (
  ...
  const { temperature, seed, custom_parameters, throttling, maxConcurrentRequests } = data.config;
