...
import {
  ...
  filterField,
  FILTER_FIELDS,
} from "@/components/pages-shared/traces/TraceDetailsPanel/TraceTreeViewer/helpers";
...
const TraceTreeViewer: React.FunctionComponent<TraceTreeViewerProps> = ({
  ...
  const noSearch = !search && !filters.length;
  ...
  const predicate = useCallback(
    (data: Span | Trace) => {
      ...
      const isSearchValid = searchValue
        ? searchFunction(searchValue, data)
        : true;

      return (
        isSearchValid &&
        filters.every(({ field, value, operator }) =>
          value
            ? filterField(
                field as FILTER_FIELDS,
                data,
  ...
  return (
    ...
    >
      <div className="mt-4 min-w-[400px] max-w-full">
        ...
        <div className="sticky top-0 z-10 flex flex-wrap items-center justify-between gap-x-8 gap-y-2 bg-white px-4 py-2">
