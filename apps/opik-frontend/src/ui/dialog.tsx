...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Close
    ref={ref}
    className={cn(
      "opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground",
      className,
    )}
    {...props}
  >
...
>(({ className, children, hideOverlay, ...props }, ref) => {
  ...
  return (
    <DialogPortal container={container}>
      <DialogOverlay
        style={hideOverlay ? { visibility: "hidden" } : undefined}
      />
      ...
      >
        {children}
        <DialogCloseButton className="absolute right-8 top-7" />
      </DialogPrimitive.Content>
    </DialogPortal>
  );
});
