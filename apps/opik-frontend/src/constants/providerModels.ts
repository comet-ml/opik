 ...
 *
 * PROVIDER_MODELS is no longer the runtime source of truth — the registry
 * is now fetched from the backend via `useLlmModels` (see
 * `hooks/useLLMProviderModelsData.ts`). This constant is kept in-tree for
 * two reasons:
 *
 * 1. Rollback handle. If the CDN flow misbehaves in prod, switching the
 *    hook to read this map is a one-line change.
 * 2. Hydration-window seed. `lib/modelRegistryStore.ts` initializes its
 *    snapshot + flag cells from this constant so pre-fetch reads
 *    (`getProviderFromModel`, `isReasoningModel`) return the right
 *    answers for every model that existed at release time, instead of
 *    falling through to `PROVIDER_TYPE.OPEN_AI` / `reasoning: false`
 *    defaults during the first few hundred ms of every cold load.
 *
 * OPIK-5022 will delete both this constant and the Java enums once the
 * registry has soaked in prod.
 */
import {
  PROVIDER_MODEL_TYPE,
