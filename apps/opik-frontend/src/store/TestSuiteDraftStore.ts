...
const useTestSuiteDraftStore = create<TestSuiteDraftState>((set, get) => ({
  ...createInitialState(),
...
export const selectIsDraftMode = (state: TestSuiteDraftState) =>
  state.addedItems.size > 0 ||
  state.editedItems.size > 0 ||
  state.deletedIds.size > 0;

export const selectHasAssertionChanges = (
  state: TestSuiteDraftState,
): boolean =>
  state.suiteAssertions !== null ||
  state.itemAssertions.size > 0 ||
  state.executionPolicy !== null;

export const selectHasDraft = (state: TestSuiteDraftState): boolean =>
  selectIsDraftMode(state) || selectHasAssertionChanges(state);

// Item CRUD hooks

export const useAddItem = () =>
  useTestSuiteDraftStore((state) => state.addItem);
export const useBulkAddItems = () =>
  useTestSuiteDraftStore((state) => state.bulkAddItems);
export const useEditItem = () =>
  useTestSuiteDraftStore((state) => state.editItem);
export const useDeleteItem = () =>
  useTestSuiteDraftStore((state) => state.deleteItem);
export const useBulkDeleteItems = () =>
  useTestSuiteDraftStore((state) => state.bulkDeleteItems);
export const useBulkEditItems = () =>
  useTestSuiteDraftStore((state) => state.bulkEditItems);
export const useClearDraft = () =>
  useTestSuiteDraftStore((state) => state.clearDraft);
export const useAddedItems = () =>
  useTestSuiteDraftStore((state) => state.addedItems);
export const useEditedItems = () =>
  useTestSuiteDraftStore((state) => state.editedItems);
export const useDeletedIds = () =>
  useTestSuiteDraftStore((state) => state.deletedIds);

export const useAddedDatasetItemById = (id?: string) =>
  useTestSuiteDraftStore((state) => state.addedItems.get(id || ""));
export const useEditedDatasetItemById = (id?: string) =>
  useTestSuiteDraftStore((state) => state.editedItems.get(id || ""));
export const useIsDraftMode = () => useTestSuiteDraftStore(selectIsDraftMode);
export const useHasDraft = () => useTestSuiteDraftStore(selectHasDraft);

export const useIsAllItemsSelected = () =>
  useTestSuiteDraftStore((state) => state.isAllItemsSelected);
export const useSetIsAllItemsSelected = () =>
  useTestSuiteDraftStore((state) => state.setIsAllItemsSelected);

// Assertion hooks

export const useSuiteAssertions = () =>
  useTestSuiteDraftStore((state) => state.suiteAssertions);
export const useItemAssertions = (itemId: string) =>
  useTestSuiteDraftStore((state) => state.itemAssertions.get(itemId));
export const useItemAssertionsMap = () =>
  useTestSuiteDraftStore((state) => state.itemAssertions);
export const useUpdateItemAssertions = () =>
  useTestSuiteDraftStore((state) => state.updateItemAssertions);

// Execution policy hooks

export const useDraftExecutionPolicy = () =>
  useTestSuiteDraftStore((state) => state.executionPolicy);
...
export default useTestSuiteDraftStore;
