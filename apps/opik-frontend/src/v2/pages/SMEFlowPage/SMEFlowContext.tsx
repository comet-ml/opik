...
const getItemState = (
  ...
  if (getDistinctAnnotatorCount(item, feedbackScoreNames) >= annotatorsPerItem) {
...
export const SMEFlowProvider: React.FunctionComponent<{
  ...
  const { itemStates, processedCount, totalCount, canStartAnnotation, defaultItemIds, allItemIds } =
    useMemo(() => {
      ...
      queueItems.forEach((item) => {
        ...
        const state = getItemState(item, feedbackScoreNames, currentUserName, annotatorsPerItem);
    ...
    }, [queueItems, annotationQueue?.feedback_definition_names, currentUserName, annotatorsPerItem]);
  ...
  const updateFeedbackScore = useCallback(
    (update: UpdateFeedbackScoreData) => {
      setCurrentAnnotationState((prev) => {
        const existingIdx = prev.scores.findIndex((s) => s.name === update.name);
  ...
  useEffect(() => {
    ...
    const lastComment = getLastCommentByUser(currentItem.comments, currentUserName);
  ...
  }, [currentItem, currentUserName, annotationQueue?.feedback_definition_names, resetLastSaved]);
  ...
  }, [isItemsLoading, queueItems.length, defaultItemIds.length, currentView, setCurrentView]);
