...
import { AnnotationQueue, ANNOTATION_QUEUE_SCOPE } from "@/types/annotation-queues";
...
export function useAnnotationPersistence({
  ...
  const { mutate: deleteThreadComments } = useThreadCommentsBatchDeleteMutation();
  const { mutate: setTraceFeedbackScore } = useTraceFeedbackScoreSetMutation();
  const { mutate: setThreadFeedbackScore } = useThreadFeedbackScoreSetMutation();
  const { mutate: deleteTraceFeedbackScore } = useTraceFeedbackScoreDeleteMutation();
  const { mutate: deleteThreadFeedbackScore } = useThreadFeedbackScoreDeleteMutation();
  ...
  const saveNow = useCallback(() => {
    ...
    if (currentText !== lastText && !commentCreatingRef.current) {
      if (currentText && !lastId) {
        ...
        if (isThread) {
          ...
          createThreadComment(
            { threadId: getAnnotationQueueItemId(thread), projectId: thread.project_id, text: currentText },
        ...
        lastSavedCommentRef.current.text = currentText;
        if (isThread) {
          updateThreadComment({ commentId: lastId, projectId: (item as Thread).project_id, text: currentText });
        } else {
          updateTraceComment({ commentId: lastId, traceId: item.id, text: currentText });
        }
      } else if (!currentText && lastId) {
        lastSavedCommentRef.current = { text: "" };
        if (isThread) {
          const thread = item as Thread;
          deleteThreadComments({ ids: [lastId], threadId: getAnnotationQueueItemId(thread), projectId: thread.project_id });
        } else {
      ...
      if (isThread) {
        ...
        if (changedScores.length) {
          setThreadFeedbackScore({
            ...
            scores: changedScores.map((s) => ({
              name: s.name, categoryName: s.category_name, value: s.value, reason: s.reason,
            })),
          });
        }
      } else {
        ...
        changedScores.forEach((score) => {
          setTraceFeedbackScore({
            traceId: item.id, name: score.name,
            categoryName: score.category_name, value: score.value, reason: score.reason,
          });
        });
      }
    ...
    isThread, annotationQueue, annotationStateRef, currentItemRef,
    createTraceComment, createThreadComment,
    updateTraceComment, updateThreadComment,
    deleteTraceComments, deleteThreadComments,
    setTraceFeedbackScore, deleteTraceFeedbackScore,
    setThreadFeedbackScore, deleteThreadFeedbackScore,
    onCommentCreated, onCommentCreateError,
  ]);
  ...
  const resetLastSaved = useCallback(
    (comment: { id?: string; text?: string } | undefined, scores: TraceFeedbackScore[]) => {
      if (timerRef.current) {
        clearTimeout(timerRef.current);
        timerRef.current = null;
      }
      commentCreatingRef.current = false;
      lastSavedCommentRef.current = { id: comment?.id, text: comment?.text?.trim() || "" };
