...
const KpiCard: React.FC<KpiCardProps> = ({
  ...
  <Card className="flex min-h-[68px] min-w-0 flex-1 flex-col px-3 py-2">
    <div className="flex w-full items-center gap-1 pb-2">
      ...
      {loading ? trend && <Skeleton className="h-5 w-12 rounded-md" /> : trend}
    </div>
    {loading ? (
      <Skeleton className="my-0.5 h-5 w-20" />
    ) : (
