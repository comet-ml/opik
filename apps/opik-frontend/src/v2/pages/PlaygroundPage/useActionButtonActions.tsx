  ...
  const runAllViaBackend = useCallback(async () => {
    ...
    try {
      ...
      const response = await runExperimentExecution.mutateAsync({
        ...
        prompts,
        projectName,
        experimentNames: Object.fromEntries(
          prompts.map((p) => [p.id, getExperimentNameForPrompt(p.id)]),
        ),
      });
      ...
      const experiments: LogExperiment[] = response.experiments.map((exp) => {
        ...
        return {
          ...
          name: getExperimentNameForPrompt(promptId),
          datasetName,
          datasetVersionId,
    ...
    async (promptId: string) => {
      ...
      try {
        const response = await runExperimentExecution.mutateAsync({
          ...
          projectName,
          experimentNames: {
            [prompt.id]: getExperimentNameForPrompt(prompt.id),
          },
        });
