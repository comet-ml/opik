...
// Matches [image] placeholders (generated by backend ClickHouse truncation) and
// truncated data URIs (substring-cut at 10,001 chars, with optional leading " from ClickHouse JSON encoding)
const TRUNCATED_MEDIA_REGEX =
  /\[image(?:_\d+)?]|^"?data:(image|video|audio)\//i;

export const containsTruncatedMedia = (value: unknown): boolean => {
  if (typeof value === "string") {
    return TRUNCATED_MEDIA_REGEX.test(value);
  }

  if (Array.isArray(value)) {
    return value.some(containsTruncatedMedia);
  }

  if (isObject(value)) {
    return Object.values(value).some(containsTruncatedMedia);
  }
