...
| frontend-design-fidelity | `frontend-design-fidelity/` | Pre-design-review checklist for `apps/opik-frontend` changes, mined from the design team's recurring `#code-review` feedback. Use when reviewing an FE diff before it goes to a designer — flags wrong component variant/size, off-scale typography (raw px, weight 600), arbitrary spacing/hex vs tokens, missing hover/active/empty/loading/error states, missing tooltips, dark-mode contrast, and inconsistent copy/casing. Baz auto-discovers it and path-scopes it to FE PRs. |
