...
def _extract_system_text(candidate: dict[str, str], fallback: str) -> str:
    """Extract system prompt text from a candidate dictionary."""
...
def _apply_system_text(
    ...
    """Apply system text to a ChatPrompt object."""
...
class OpikGEPAAdapter(GEPAAdapter[OpikDataInst, dict[str, Any], dict[str, Any]]):
    """
    GEPA adapter for Opik Optimizer that routes evaluation through Opik's metric system.

    This adapter implements the GEPAAdapter interface to integrate GEPA's optimization
    algorithm with Opik's evaluation framework. It handles:
    - Converting Opik dataset items to GEPA data instances
    - Evaluating candidates using Opik's metric functions
    - Creating reflective datasets for GEPA's reflection mechanism
    """
    ...
    def __init__(
        ...
        """
        Initialize the Opik GEPA adapter.

        Args:
            base_prompt: Base ChatPrompt template to use for evaluations
            optimizer: Reference to the optimizer instance (for tracking/metrics)
            metric: Metric function that takes (dataset_item, llm_output) and returns a score
            system_fallback: Fallback system prompt text if candidate doesn't provide one
        """
    ...
    def evaluate(
        ...
        """
        Evaluate a candidate prompt on a batch of data instances.

        Args:
            batch: List of OpikDataInst instances to evaluate on
            candidate: Candidate dictionary containing prompt components (e.g., {"system_prompt": "..."})
    ...
    def make_reflective_dataset(
        ...
        Args:
            ...
            eval_batch: The evaluation batch containing trajectories
            components_to_update: List of component names to update (e.g., ["system_prompt"])
