...
def _format_pareto_note(note: str) -> str:
    """Format Pareto front notes for display."""
    try:
...
class RichGEPAOptimizerLogger:
    ...
    def log(self, message: str) -> None:
        """Log a message from GEPA, formatting it for Rich console output."""
        if self.verbose < 1:
            return
...
def baseline_evaluation(verbose: int = 1) -> Any:
    """Context manager for baseline evaluation reporting."""
    if verbose >= 1:
...
def start_gepa_optimization(verbose: int = 1, max_trials: int = 10) -> Any:
    """Context manager for GEPA optimization reporting with progress tracking."""
    if verbose >= 1:
...
def display_candidate_update(
    ...
    *,
    ...
    """Display an update about a candidate during optimization."""
    if verbose < 1:
        return
