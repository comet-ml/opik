...
class GepaOptimizerAdapter(BaseOptimizer):
    """
    GEPA Optimizer using the adapter pattern for cleaner integration.

    This optimizer uses GEPA's adapter-based approach, which provides a more modular
    and maintainable integration with the GEPA optimization algorithm. The adapter
    pattern allows for better separation of concerns between Opik's evaluation
    framework and GEPA's optimization logic.

    The GEPA (Genetic-Pareto) algorithm uses genetic algorithms with Pareto optimization
    ...
    def get_optimizer_metadata(self) -> dict[str, Any]:
        """Get metadata about this optimizer instance."""
        return {
            "model": self.model,
    ...
    def _build_data_insts(
        ...
        """Convert Opik dataset items to GEPA data instances."""
    ...
    def _infer_dataset_keys(self, dataset: Dataset) -> tuple[str, str]:
        """Infer input and output keys from dataset structure."""
    ...
    def _extract_system_text(self, prompt: chat_prompt.ChatPrompt) -> str:
        """Extract system text from a ChatPrompt."""
    ...
    def optimize_prompt(
        ...
        **kwargs: Any,
        ...
        """
        Optimize a prompt using GEPA (Genetic-Pareto) algorithm via adapter pattern.
        ...
        try:
            ...
            # Create adapter with base prompt
            ...
            try:
                ...
                raise ImportError(
                    "gepa package is required for GepaOptimizerAdapter"
                ) from exc
    ...
    def _evaluate_prompt_logged(
        ...
        """Evaluate a prompt using Opik's evaluation framework."""
        if prompt.model is None:
            prompt.model = self.model
