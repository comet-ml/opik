...
class SpanDuration(base_metric.BaseMetric):
    ...
    """
    def __init__(self, name: str = "total_span_duration", track: bool = True, project_name: Optional[str] = None) -> None:
        super().__init__(name=name, track=track, project_name=project_name)

    def score(self, task_span: emulation_models.SpanModel, **ignored_kwargs: Any) -> score_result.ScoreResult:
        if task_span.end_time is None or task_span.start_time is None:
            raise opik.exceptions.MetricComputationError("Span end time or start time is not set")
