    ...
    *,
    ...
    for message in messages:
        ...
        if message.get("role") == "assistant" and not inserted:
            print(
                f"[mcp-second-pass] inserting tool summary for {tool_name} after assistant message",
                flush=True,
            )
            with_tool.append(
                {
    ...
    if not inserted:
        print(
            f"[mcp-second-pass] no assistant message found; appending summary for {tool_name}",
            flush=True,
        )
        with_tool.append(
            {
...
class MCPSecondPassCoordinator:
    ...
    def record_summary(self, summary: str) -> None:
        print(f"[mcp-second-pass] recording summary for {self.tool_name}", flush=True)
    ...
    def build_second_pass_messages(
        ...
        *,
        ...
        if not summary:
            print(
                f"[mcp-second-pass] no summary available for {self.tool_name}; skipping second pass",
                flush=True,
            )
            return None

        print(
            f"[mcp-second-pass] have summary for {self.tool_name}: {bool(summary)}",
            flush=True,
        )

        print(
            f"[mcp-second-pass] sample tool content for {self.tool_name}: {summary[:160].replace(chr(10), ' ')}",
            flush=True,
        )
        ...
        print(
            f"[mcp-second-pass] follow-up appended for {self.tool_name}: "
            f"{follow_up[:120] if follow_up else 'None'}",
            flush=True,
        )
