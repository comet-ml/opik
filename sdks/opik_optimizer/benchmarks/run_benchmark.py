...
if __name__ == "__main__":
    ...
    parser.add_argument("--demo-datasets", type=str, nargs='*', default=None, help=f"Space-separated list of dataset keys to run. Available: {list(benchmark_config.DATASET_CONFIG.keys())}")
    parser.add_argument("--optimizers", type=str, nargs='*', default=None, help=f"Space-separated list of optimizer keys to run. Available: {list(benchmark_config.OPTIMIZER_CONFIGS.keys())}")
    parser.add_argument("--models", type=str, nargs='*', default=None, help=f"Space-separated list of model keys to run. Available: {list(benchmark_config.MODELS)}")
    ...
    parser.add_argument("--checkpoint-dir", type=str, default=DEFAULT_CHECKPOINT_DIR, help="Directory to save benchmark results")
    parser.add_argument("--max-workers", type=int, default=DEFAULT_MAX_WORKERS, help="Maximum number of worker threads")
    
    parser.add_argument("--retry-failed-run-id", type=str, default=None, metavar="RUN_ID", help="Specify a previous RUN_ID to retry only its failed tasks. Successful tasks from that run will be skipped.")
    parser.add_argument("--resume-run-id", type=str, default=None, metavar="RUN_ID", help="Specify a previous RUN_ID to resume from. Successful tasks from that run will be skipped.")
