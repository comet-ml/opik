...
class BenchmarkLogger():
    ...
    def update_active_task_status(self, future, dataset_name, optimizer_name, model_name, status):
    ...
    def _generate_live_display_message(self) -> Group:
        ...
        for status_info in self.tasks_status.values():
            ...
            if status_info["status"] == "Running":    
                active_list.append(
                    Text.assemble((display_text, "yellow"), (f" [{optimizer_name}]", "dim"))
                )
        ...
        nb_active_tasks = len([x for x in self.tasks_status.values() if x["status"] == "Running"])
        ...
        summary_line = Text(f"Run: {self.run_id} | Tasks: {nb_success_tasks + nb_failed_tasks}/{self.total_tasks} | Success: {nb_success_tasks} | Failed: {nb_failed_tasks} | Active: {nb_active_tasks}", style="dim")
    ...
    def add_result_panel(
        ...
        border_style_obj = STYLES.get(task_detail_data.status.lower(), STYLES.get("default", Style(color="yellow"))) 
        status_text_styled = Text(task_detail_data.status.upper(), style=border_style_obj)
        panel_title_text = Text.assemble(status_text_styled, f" {optimizer_name} on {dataset_name}")
        ...
        if task_detail_data.timestamp_end:
            table.add_row("Time Taken:", f"{task_detail_data.timestamp_end - task_detail_data.timestamp_start:.2f}s")
        else:
            table.add_row("Time Taken:", "[dim]N/A[/dim]")
        
        llm_calls_count = task_detail_data.llm_calls_total_optimization
        llm_calls_str = f"[bold cyan]{llm_calls_count}[/bold cyan]" if llm_calls_count is not None else "[dim]N/A[/dim]"
        table.add_row("LLM Calls (Opt):", llm_calls_str)

        # Process scores
        initial_evaluation_metrics = task_detail_data.initial_evaluation.metrics if task_detail_data.initial_evaluation else []
        final_evaluation_metrics = task_detail_data.optimized_evaluation.metrics if task_detail_data.optimized_evaluation else []
        
        all_metric_names = set([x['metric_name'] for x in initial_evaluation_metrics] + [x['metric_name'] for x in final_evaluation_metrics]) 
        initial_scores_grp = []
        final_scores_grp = []
        percent_changes_grp = []
        for metric_name in all_metric_names:
            initial_score_obj = next((x for x in initial_evaluation_metrics if x["metric_name"] == metric_name), None)
            initial_score = initial_score_obj['score'] if initial_score_obj else None
            optimized_score_obj = next((x for x in final_evaluation_metrics if x["metric_name"] == metric_name), None)
            optimized_score = optimized_score_obj['score'] if optimized_score_obj else None

            # Process initial scores
            style = STYLES["success"] if isinstance(initial_score, (float, int)) else STYLES["warning"]
            value_str = f"{initial_score:.4f}" if isinstance(initial_score, (float, int)) else ("[dim]N/A[/dim]" if initial_score is None else str(initial_score))
            initial_scores_grp.append(Text.assemble(f" • {metric_name}: ", (value_str, style)))

            # Process final scores
            final_style = STYLES["success"] if isinstance(optimized_score, (float, int)) else STYLES["warning"]
            final_value_str = f"{optimized_score:.4f}" if isinstance(optimized_score, (float, int)) else ("[dim]N/A[/dim]" if optimized_score is None else str(optimized_score))
            final_scores_grp.append(Text.assemble(f" • Final {metric_name}: ", (final_value_str, final_style)))

            # Process percentage change
            percent_change_text = self._calculate_percentage_change(initial_score, optimized_score, metric_name)
            percent_changes_grp.append(Text.assemble(f" • {metric_name}: ", percent_change_text))
        ...
        self.result_panels.append(Panel(main_content_group, title=panel_title_text, border_style=border_style_obj, padding=(1, 2), expand=True))
        return None

    def print_benchmark_footer(self, results: List["TaskResult"], total_duration: float):
        ...
        if results:
            ...
            for task_summary in results: # task_summary is an item from self.results
                ...
                if task_summary.timestamp_end:
                    time_taken = task_summary.timestamp_end - task_summary.timestamp_start
                else:
                    time_taken = 0
                ...
                processed_data_for_table[table_key]["llm_calls_total_optimization"] = task_summary.llm_calls_total_optimization

                initial_eval_metrics = task_summary.initial_evaluation.metrics if task_summary.initial_evaluation else []
                for metric_entry in initial_eval_metrics:
                    ...
                    processed_data_for_table[table_key]["initial"][metric_display_name] = metric_entry.get("score") # type: ignore

                final_eval_metrics = task_summary.optimized_evaluation.metrics if task_summary.optimized_evaluation else []
                for metric_entry in final_eval_metrics:
                    metric_display_name = metric_entry.get("metric_name", "Unknown")
                    all_metrics_names.add(metric_display_name)
                    processed_data_for_table[table_key]["final"][metric_display_name] = metric_entry.get("score") # type: ignore
            ...
            for i, key_tuple in enumerate(sorted_table_keys):
                ...
                for metric_i, metric_name_to_display in enumerate(sorted(list(all_metrics_names))):
                    initial_val = scores_by_metric["initial"].get(metric_name_to_display) # type: ignore
...
def log_console_output_to_file():
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            dataset_name = kwargs.get('dataset_name', args[0] if len(args) > 0 else 'unknown')
            ...
            file_path = os.path.join(checkpoint_folder, f"optimization_{sanitized_dataset_name}_{sanitized_optimizer_name}_{sanitized_model_name}_{timestamp}.log")
