...
class TestGepaOptimizerValidation:
    ...
    def test_model_validation(self):
        ...
        # Test None model
        with pytest.raises(ValueError, match="model parameter is required and cannot be None"):
            GepaOptimizer(model=None)
        
        # Test empty string model
        with pytest.raises(ValueError, match="model cannot be empty or whitespace-only"):
            GepaOptimizer(model="")
    ...
    def test_project_name_validation(self):
        ...
        with pytest.raises(ValueError, match="project_name must be a string or None, got int"):
    ...
    def test_reflection_model_validation(self):
        ...
        optimizer = GepaOptimizer(model="openai/gpt-4", reflection_model="openai/gpt-3.5-turbo")
        assert optimizer.reflection_model == "openai/gpt-3.5-turbo"
        ...
        with pytest.raises(ValueError, match="reflection_model must be a string or None, got int"):
    ...
    def test_model_kwargs_passthrough(self):
        ...
        assert optimizer.num_threads == 8  # num_threads is extracted to instance variable
