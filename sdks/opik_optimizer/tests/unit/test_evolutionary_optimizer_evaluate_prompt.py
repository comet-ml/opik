...
def test_evaluate_prompt_uses_evolved_prompt_in_experiment_config():
    ...
    evolved_prompt_messages = [{"role": "user", "content": "Evolved optimized prompt content"}]
    ...
    def mock_task_evaluator_evaluate(**kwargs):
        nonlocal captured_experiment_config
        captured_experiment_config = kwargs.get("experiment_config", {})
        return 0.85
    
    # Execute the method under test
    with patch("opik_optimizer.task_evaluator.evaluate", side_effect=mock_task_evaluator_evaluate):
    ...
    assert captured_experiment_config["agent_config"]["messages"] == evolved_prompt_messages
    assert captured_experiment_config["configuration"]["prompt"] == evolved_prompt_messages
    
    # Verify original prompt is NOT in the experiment_config
    assert captured_experiment_config["agent_config"]["messages"] != original_prompt_messages
    assert captured_experiment_config["configuration"]["prompt"] != original_prompt_messages