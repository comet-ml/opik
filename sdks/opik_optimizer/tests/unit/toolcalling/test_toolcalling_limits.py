...
def _make_tools(n: int) -> list[dict[str, object]]:
    tools = []
    for idx in range(n):
        tools.append(
            {
...
def test_prepare_tool_optimization_enforces_limit(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_prepare_tool_optimization_allows_limit(monkeypatch: pytest.MonkeyPatch) -> None:
