...
def test_encode_image_to_base64_uri_passthrough() -> None:
    ...
    assert (
        encode_image_to_base64_uri(payload, image_format="PNG") == payload
...
def test_encode_image_to_base64_uri_saveable_object() -> None:
    class DummyImage:
        ...
        def save(self, buffer, **_kwargs) -> None:
            buffer.write(self._payload)

    raw = b"jpeg-bytes"
    encoded = encode_image_to_base64_uri(
        DummyImage(raw, "JPEG"), image_format="PNG"
    )
