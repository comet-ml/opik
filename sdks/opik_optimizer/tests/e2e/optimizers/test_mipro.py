...
def test_mipro_optimizer():
    ...
    assert results.optimizer == "MiproOptimizer", f"Expected MiproOptimizer, got {results.optimizer}"
    ...
    assert results.initial_score is None, f"Initial score should be None"
    ...
    if 'mipro_prompt' in results.details:
        ...
        assert isinstance(mipro_prompt, (str, dict, list)), f"Mipro prompt should be str/dict/list, got {type(mipro_prompt)}"
    
    if 'tool_prompts' in results.details:
        tool_prompts = results.details['tool_prompts']
        assert isinstance(tool_prompts, (dict, list)), f"Tool prompts should be dict/list, got {type(tool_prompts)}"
