...
def test_few_shot_optimizer():
    ...
    assert results.optimizer == "FewShotBayesianOptimizer", f"Expected FewShotBayesianOptimizer, got {results.optimizer}"
    ...
    if 'min_examples' in results.details:
        assert results.details['min_examples'] == 1, f"Expected min_examples 1, got {results.details['min_examples']}"
    
    if 'max_examples' in results.details:
        assert results.details['max_examples'] == 2, f"Expected max_examples 2, got {results.details['max_examples']}"
    ...
    assert "FewShotBayesianOptimizer" in result_str, "String should contain optimizer name"
