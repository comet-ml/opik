...
/**
 * Default few-shot examples for answer relevance evaluation with context
 */
export const FEW_SHOT_EXAMPLES_WITH_CONTEXT: FewShotExampleAnswerRelevanceWithContext[] =
  [
    {
      ...
      reason:
        "The answer provides information about the Eiffel Tower, which is related to France, but fails to address the specific question about the capital city. It doesn't directly answer the user's query, resulting in low relevance.",
    },
    {
      ...
      reason:
        "The answer mentions Paris, which is the correct capital, but it's presented as just one of many cities rather than explicitly stating it's the capital. The response is partially relevant but lacks directness in addressing the specific question.",
    },
    {
      ...
      reason:
        "The answer directly and correctly identifies Paris as the capital of France, which is highly relevant to the user's question. It also provides additional context about the Eiffel Tower, which aligns with the provided context. The response is comprehensive and relevant, though slightly more detailed than necessary, preventing a perfect score.",
    },
  ];

/**
 * Default few-shot examples for answer relevance evaluation without context
 */
export const FEW_SHOT_EXAMPLES_NO_CONTEXT: FewShotExampleAnswerRelevanceNoContext[] =
  [
    {
      title: "Low Relevance Score",
      input: "What's the capital of France?",
...
/**
 * Formats few-shot examples with context for the answer relevance prompt
 */
function formatExamplesWithContext(
  examples: FewShotExampleAnswerRelevanceWithContext[]
): string {
  return examples
    .map((example, i) => {
      return `#### Example ${i + 1}: ${example.title}
...
- **Result:**
  \`\`\`json
  {
    "answer_relevance_score": ${example.answer_relevance_score},
...
/**
 * Formats few-shot examples without context for the answer relevance prompt
 */
function formatExamplesNoContext(
  examples: FewShotExampleAnswerRelevanceNoContext[]
): string {
  return examples
    .map((example, i) => {
      return `#### Example ${i + 1}: ${example.title}
...
/**
 * Generates the answer relevance evaluation query with context.
 *
 * @param input - The user's input/question
 * @param output - The LLM's output/answer to evaluate
 * @param context - List of context strings
 * @param fewShotExamples - Few-shot examples to include in the prompt
...
export function generateQueryWithContext(
  ...
  const examplesStr = formatExamplesWithContext(fewShotExamples);
...
/**
 * Generates the answer relevance evaluation query without context.
 *
 * @param input - The user's input/question
 * @param output - The LLM's output/answer to evaluate
 * @param fewShotExamples - Few-shot examples to include in the prompt
 * @returns The formatted prompt string
 */
export function generateQueryNoContext(
  input: string,
  output: string,
  fewShotExamples: FewShotExampleAnswerRelevanceNoContext[]
): string {
  const examplesStr = formatExamplesNoContext(fewShotExamples);
