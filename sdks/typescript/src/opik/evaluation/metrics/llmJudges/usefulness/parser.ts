...
/**
 * Parses the LLM model output for the Usefulness metric.
 *
...
export function parseModelOutput(
  ...
  try {
    ...
    if (isNaN(score) || score < 0.0 || score > 1.0) {
      throw new Error(
        `Usefulness score must be between 0.0 and 1.0, got ${score}`
      );
    }
    ...
    throw new MetricComputationError(
      "Failed to calculate usefulness score. The model output could not be parsed.",
