...
export class Projects {
     ...
     */
    public upsertProjectConfigurations(
        id: string,
        request: OpikApi.Configuration,
        requestOptions?: Projects.RequestOptions,
    ): core.HttpResponsePromise<void> {
        return core.HttpResponsePromise.fromPromise(this.__upsertProjectConfigurations(id, request, requestOptions));
    }

    private async __upsertProjectConfigurations(
        id: string,
        request: OpikApi.Configuration,
        requestOptions?: Projects.RequestOptions,
    ): Promise<core.WithRawResponse<void>> {
        const _response = await core.fetcher({
            url: urlJoin(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
                    environments.OpikApiEnvironment.Default,
                `v1/private/projects/${encodeURIComponent(id)}/configurations`,
            ),
            method: "PUT",
            headers: {
                "Comet-Workspace":
            ...
            contentType: "application/json",
            requestType: "json",
            body: serializers.Configuration.jsonOrThrow(request, { unrecognizedObjectKeys: "strip" }),
            timeoutMs: requestOptions?.timeoutInSeconds != null ? requestOptions.timeoutInSeconds * 1000 : 60000,
            maxRetries: requestOptions?.maxRetries,
            withCredentials: true,
            abortSignal: requestOptions?.abortSignal,
        ...
        switch (_response.error.reason) {
            ...
            case "timeout":
                throw new errors.OpikApiTimeoutError(
                    "Timeout exceeded when calling PUT /v1/private/projects/{id}/configurations.",
                );
            case "unknown":
                throw new errors.OpikApiError({
                    message: _response.error.errorMessage,
