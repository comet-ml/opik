...
class TestBuildChecklist:
    def test_build_checklist__project_root__returned_as_string(self, tmp_path: Path) -> None:
        result = build_checklist(tmp_path, command=None)

        assert result["project_root"] == str(tmp_path)

    def test_build_checklist__empty_repo__all_expected_keys_present(self, tmp_path: Path) -> None:
        result = build_checklist(tmp_path, command=None)
    ...
    def test_build_checklist__repo_with_instrumented_files__tree_and_matches_populated(self, tmp_path: Path) -> None:
    ...
    def test_build_checklist__command_none__serialized_as_none(self, tmp_path: Path) -> None:
        result = build_checklist(tmp_path, command=None)
