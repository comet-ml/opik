...
class TestPythonSyntax:
    def test_valid_python__returns_none(self) -> None:
        code = "def hello():\n    return 42\n"
        assert check_syntax("app.py", code) is None
    ...
    def test_empty_file__returns_none(self) -> None:
        assert check_syntax("empty.py", "") is None
    ...
    def test_valid_python__complex_file__returns_none(self) -> None:
        code = (
            "import os\n"
            "from typing import Optional\n\n"
            "class Foo:\n"
        ...
        assert check_syntax("module.py", code) is None


class TestTypescriptSyntax:
    def test_valid_ts__returns_none(self) -> None:
    ...
    def test_cts_extension__checks_ts(self) -> None:
        ...
        assert check_syntax("module.cts", code) is None


class TestJavaScriptSyntax:
    def test_valid_js__returns_none(self) -> None:
        code = "function hello(name) {\n    return name;\n}\n"
        assert check_syntax("app.js", code) is None
    ...
    def test_cjs_extension__checks_js(self) -> None:
        code = "const x = 42;\n"
        assert check_syntax("module.cjs", code) is None
...
class TestUnsupportedFiles:
    @pytest.mark.parametrize(
        "path",
        ["readme.md", "config.yaml", "data.json", "style.css", "image.png", "noext"],
    )
...
class TestTreeSitterUnavailable:
    def test_js_without_tree_sitter__returns_none(self) -> None:
        ...
        old_ts = mod._ts_parser
        old_tsx = mod._tsx_parser
        old_js = mod._js_parser
        try:
            mod._tree_sitter_available = False
            mod._ts_parser = None
            mod._tsx_parser = None
            mod._js_parser = None
            result = check_syntax("app.js", "function hello( {")
            assert result is None
        finally:
            mod._tree_sitter_available = old_available
            mod._ts_parser = old_ts
            mod._tsx_parser = old_tsx
            mod._js_parser = old_js
...
class TestHandlerIntegration:
    ...
    def test_write_file__valid_python__syntax_ok(self, tmp_path: "pytest.TempPathFactory") -> None:
        ...
        handler = WriteFileHandler(tmp_path, FileLockRegistry())  # type: ignore[arg-type]
        result = handler.execute(
