...
@patch("builtins.open", new_callable=mock_open)
@patch("pathlib.Path.expanduser", return_value=Path("/fake/path/config.ini"))
def test_save_to_file_with_environment(mock_expanduser, mock_open_file):
    config = OpikConfig(
        url_override="http://test-url",
        workspace="test_workspace",
        environment="staging",
    ...
    assert parsed_config["opik"]["environment"] == "staging"


@patch("builtins.open", new_callable=mock_open)
@patch("pathlib.Path.expanduser", return_value=Path("/fake/path/config.ini"))
def test_save_to_file_without_environment(mock_expanduser, mock_open_file):
    config = OpikConfig(url_override="http://test-url", workspace="test_workspace")
