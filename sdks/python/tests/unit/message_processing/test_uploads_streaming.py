...
def test_streamer__attachment_uploads__flush__ok(temp_file_15mb):
    ...
    with patch("opik.file_upload.upload_manager.file_uploader") as uploader_mock:
        ...
        # we have timeout greater than upload time 3 > 2, thus all uploads will be completed
        assert tested.flush(timeout=3, upload_sleep_time=1) is True
...
def test_streamer__attachment_uploads__flush__timeout(temp_file_15mb):
    ...
    with patch("opik.file_upload.upload_manager.file_uploader") as uploader_mock:
        ...
        # we have timeout less than upload time 2 < 3, thus not all uploads will be completed
        assert tested.flush(timeout=2, upload_sleep_time=1) is False
