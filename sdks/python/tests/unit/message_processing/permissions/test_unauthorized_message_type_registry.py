...
from opik.message_processing.permissions.UnauthorizedMessageTypeRegistry import (
...
class TestIsAuthorized:
    def test_is_authorized__message_type_not_in_registry__returns_true(self):
        registry = UnauthorizedMessageTypeRegistry(
    ...
    def test_is_authorized__elapsed_time_equals_retry_interval__returns_false(self):
        registry = UnauthorizedMessageTypeRegistry(
            retry_interval_seconds=RETRY_INTERVAL
        )
        registry.add(MESSAGE_TYPE_A, attempt_time=NOW)
