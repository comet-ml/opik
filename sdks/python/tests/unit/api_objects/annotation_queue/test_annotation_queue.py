...
from opik.api_objects.annotation_queue.annotation_queue import AnnotationQueue
from opik.rest_api.types import trace_public, trace_thread
from opik.exceptions import OpikException, AnnotationQueueScopeMismatch


class TestAnnotationQueueProperties:
    ...
    def test_scope__returns_scope(self):
        mock_rest_client = Mock()
        queue = AnnotationQueue(
            id="queue-123",
            name="test_queue",
...
class TestAnnotationQueueUpdate:
...
class TestAnnotationQueueDelete:
...
class TestAnnotationQueueAddTraces:
    ...
    def test_add_traces__wrong_scope__raises_exception(self):
        mock_rest_client = Mock()
        queue = AnnotationQueue(
            id="queue-123",
            name="test_queue",
        ...
        with pytest.raises(AnnotationQueueScopeMismatch) as exc_info:
            queue.add_traces([trace])

        assert exc_info.value.item_type == "traces"
        assert exc_info.value.queue_scope == "thread"
...
class TestAnnotationQueueAddThreads:
    ...
    def test_add_threads__wrong_scope__raises_exception(self):
        mock_rest_client = Mock()
        queue = AnnotationQueue(
            id="queue-123",
            name="test_queue",
        ...
        assert exc_info.value.item_type == "threads"
        assert exc_info.value.queue_scope == "trace"
...
class TestAnnotationQueueRemoveTraces:
    ...
    def test_remove_traces__wrong_scope__raises_exception(self):
        mock_rest_client = Mock()
        queue = AnnotationQueue(
            id="queue-123",
            name="test_queue",
        ...
        assert exc_info.value.queue_scope == "thread"
        assert "traces" in str(exc_info.value)
        mock_rest_client.annotation_queues.remove_items_from_annotation_queue.assert_not_called()


class TestAnnotationQueueRemoveThreads:
    ...
    def test_remove_threads__wrong_scope__raises_exception(self):
        mock_rest_client = Mock()
        queue = AnnotationQueue(
            id="queue-123",
            name="test_queue",
