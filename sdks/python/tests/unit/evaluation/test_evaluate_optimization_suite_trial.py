...
def test_evaluate_optimization_suite_trial__raises_when_dataset_is_not_evaluation_suite():
    mock_dataset = mock.MagicMock()
    mock_dataset.name = "regular-dataset"
    mock_dataset.dataset_type = "dataset"

    with mock.patch.object(
        opik_client, "get_client_cached", return_value=mock.MagicMock()
    ):
        try:
            evaluator_module.evaluate_optimization_suite_trial(
                optimization_id="opt-123",
                dataset=mock_dataset,
                task=lambda item: {"output": "result"},
                experiment_name="trial-exp",
                verbose=0,
            )
            assert False, "Expected ValueError"
        except ValueError as e:
            assert "not configured as an evaluation suite" in str(e)


def test_evaluate_optimization_suite_trial__custom_experiment_type(
    fake_backend,
):
    mock_dataset = mock.MagicMock(
        spec=[
    ...
    with mock.patch.object(
        ...
        with mock.patch.object(
            ...
            evaluator_module.evaluate_optimization_suite_trial(
                ...
                dataset=mock_dataset,
                task=lambda item: {"output": "result"},
                experiment_name="mini-batch-exp",
                experiment_type="mini-batch",
                verbose=0,
            )

    call_kwargs = mock_create_experiment.call_args[1]
    assert call_kwargs["type"] == "mini-batch"
    assert call_kwargs["optimization_id"] == "opt-456"
