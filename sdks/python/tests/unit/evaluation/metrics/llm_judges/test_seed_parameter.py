...
from opik.evaluation.metrics.llm_judges.trajectory_accuracy.metric import TrajectoryAccuracy
from opik.evaluation.metrics.llm_judges.usefulness.metric import Usefulness
from opik.evaluation.metrics.llm_judges.structure_output_compliance.metric import StructuredOutputCompliance
...
class TestSeedParameter:
    ...
    def test_answer_relevance_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.answer_relevance.metric.models_factory.get') as mock_factory:
            ...
            with patch('opik.evaluation.metrics.llm_judges.answer_relevance.parser.parse_model_output') as mock_parser:
    ...
    def test_context_precision_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.context_precision.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = ContextPrecision(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.context_precision.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_context_recall_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.context_recall.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = ContextRecall(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.context_recall.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_g_eval_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.g_eval.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            ...
            with patch('opik.evaluation.metrics.llm_judges.g_eval.parser.parse_model_output_string') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_hallucination_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.hallucination.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = Hallucination(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.hallucination.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_moderation_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.moderation.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = Moderation(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.moderation.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_trajectory_accuracy_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.trajectory_accuracy.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = TrajectoryAccuracy(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.trajectory_accuracy.parser.parse_evaluation_response') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_usefulness_seed_parameter_passing(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.usefulness.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = Usefulness(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.usefulness.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_structured_output_compliance_seed_parameter_passing(self, test_seed: int) -> None:
        """Test that StructuredOutputCompliance passes seed parameter to model generation."""
        with patch('opik.evaluation.metrics.llm_judges.structure_output_compliance.metric.models_factory.get') as mock_factory:
            mock_model = Mock(spec=base_model.OpikBaseModel)
            mock_factory.return_value = mock_model
            
            metric = StructuredOutputCompliance(seed=test_seed, track=False)
            
            with patch('opik.evaluation.metrics.llm_judges.structure_output_compliance.parser.parse_model_output') as mock_parser:
                mock_parser.return_value = score_result.ScoreResult(
                    name="test", value=0.8, reason="Test reason"
                )
    ...
    def test_all_metrics_accept_seed_parameter(self, test_seed: int) -> None:
        ...
        with patch('opik.evaluation.metrics.llm_judges.answer_relevance.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.context_precision.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.context_recall.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.g_eval.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.hallucination.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.moderation.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.trajectory_accuracy.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.usefulness.metric.models_factory.get'), \
             patch('opik.evaluation.metrics.llm_judges.structure_output_compliance.metric.models_factory.get'):
    ...
    def test_seed_parameter_documentation(self) -> None:
        ...
        for metric_class in metrics_with_seed:
            ...
            if docstring is not None:  # Only check if docstring exists
                ...
                assert "reproducible" in docstring.lower() or "deterministic" in docstring.lower()
