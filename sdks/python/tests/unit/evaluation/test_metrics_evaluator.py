...
class TestLLMJudgeMerged:
    ...
    def test_uses_first_judge_settings(self):
        j1 = _make_judge(["A"], temperature=0.5, seed=42, name="suite_judge")
        j2 = _make_judge(["B"], temperature=0.9, seed=99, name="item_judge")
...
class TestMergeLlmJudges:
    ...
    def test_mixed_metrics__preserves_non_judges(self):
        m1 = _make_non_judge_metric("m1")
        j1 = _make_judge(["A"])
        j2 = _make_judge(["B"])
        m2 = _make_non_judge_metric("m2")

        result = _merge_llm_judges([m1, j1, j2, m2])

        assert len(result) == 3
        assert result[0] is m1
        assert result[1] is m2
        assert isinstance(result[2], llm_judge.LLMJudge)
        assert result[2].assertions == ["A", "B"]
