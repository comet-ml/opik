...
@pytest.mark.asyncio
async def test_adk__transfer_to_agent__tracked_and_span_created(
    fake_backend,
):
    ...
    translator_to_english = adk_agents.Agent(
        ...
        model=MODEL_NAME,
        description="Translates text to English.",
        instruction="Translate text to English and call 'Summarizer' to summarize it.",
        before_agent_callback=opik_tracer.before_agent_callback,
        after_agent_callback=opik_tracer.after_agent_callback,
        before_model_callback=opik_tracer.before_model_callback,
        after_model_callback=opik_tracer.after_model_callback,
    ...
    root_agent = LlmAgent(
        name="Text_Assistant",
        model=MODEL_NAME,
        instruction="Use 'Translator' agent to translate text to English.",
        sub_agents=[translator_to_english, summarizer],
        before_agent_callback=opik_tracer.before_agent_callback,
        after_agent_callback=opik_tracer.after_agent_callback,
        before_model_callback=opik_tracer.before_model_callback,
    ...
    _ = await _async_extract_final_response_text(events_generator)
    ...
    assert_dict_has_keys(
        trace_tree.spans[2].spans[2].spans[0].usage, EXPECTED_USAGE_KEYS_GOOGLE
    )
