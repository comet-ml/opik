...
class TestSeedParameterIntegration:
    ...
    def test_same_seed_produces_consistent_results(self, test_inputs: Dict[str, Any], model: str):
        ...
        for result in results[1:]:
            assert result.value == first_result.value, "Same seed should produce identical values"
            assert result.reason == first_result.reason, "Same seed should produce identical reasons"
            assert result.name == first_result.name, "Same seed should produce identical names"

    def test_different_seeds_can_produce_different_results(self, test_inputs: Dict[str, Any], model: str):
        ...
        assert values_unique or reasons_unique, "Different seeds should generally produce different results"

    def test_seed_consistency_across_metric_types(self, test_inputs: Dict[str, Any], model: str):
        ...
        for metric_class, score_kwargs in metric_configs:
            ...
            first_result = results[0]
            for result in results[1:]:
                assert result.value == first_result.value, f"Same seed should produce identical values for {metric_class.__name__}"
                assert result.reason == first_result.reason, f"Same seed should produce identical reasons for {metric_class.__name__}"

    def test_seed_none_vs_seed_integer_behavior(self, test_inputs: Dict[str, Any], model: str):
        ...
        assert all(v == seeded_values[0] for v in seeded_values), "All seeded results should be identical"
    ...
    def test_g_eval_seed_consistency(self, model: str):
        ...
        first_result = results[0]
        for result in results[1:]:
            assert result.value == first_result.value, "GEval with same seed should produce identical values"
            assert result.reason == first_result.reason, "GEval with same seed should produce identical reasons"
    ...
    def test_structured_output_compliance_seed_consistency(self, model: str):
        ...
        results = []
        for _ in range(num_runs):
            result = metric.score(output='{"name": "John", "age": 30, "city": "New York"}')
            results.append(result)
            time.sleep(0.1)
        ...
        first_result = results[0]
        for result in results[1:]:
            assert result.value == first_result.value, "StructuredOutputCompliance with same seed should produce identical values"
            assert result.reason == first_result.reason, "StructuredOutputCompliance with same seed should produce identical reasons"
    ...
    def test_trajectory_accuracy_seed_consistency(self, model: str):
        ...
        first_result = results[0]
        for result in results[1:]:
            assert result.value == first_result.value, "TrajectoryAccuracy with same seed should produce identical values"
            assert result.reason == first_result.reason, "TrajectoryAccuracy with same seed should produce identical reasons"
    ...
    def test_seed_parameter_with_langchain_model(self, test_inputs: Dict[str, Any]):
        ...
        first_result = results[0]
        for result in results[1:]:
            assert result.value == first_result.value, "LangchainChatModel with same seed should produce identical values"
            assert result.reason == first_result.reason, "LangchainChatModel with same seed should produce identical reasons"

    def test_seed_parameter_performance_impact(self, test_inputs: Dict[str, Any], model: str):
        ...
        assert 0.5 <= time_ratio <= 2.0, f"Seed parameter should not significantly impact performance. Ratio: {time_ratio}"
