...
# =============================================================================
# REGRESSION: get_evaluation_suite without project_name (OPIK-5614)
# =============================================================================


def test_get_evaluation_suite_without_project_name__returns_items_from_correct_project(
    opik_client: opik.Opik, dataset_name: str, temporary_project_name: str
):
    """Regression test for OPIK-5614: get_evaluation_suite without project_name
    must still return suite items when the suite was created under a non-default
    project.

    The SDK used to store the caller's default project_name on the Dataset
    object instead of the dataset's actual project, causing item streaming
    ...
    # Retrieve the suite WITHOUT specifying project_name.
    retrieved_suite = opik_client.get_evaluation_suite(name=dataset_name)

    assert retrieved_suite.dataset.project_name == temporary_project_name

    items = retrieved_suite.dataset.get_items()
    assert len(items) == 2
