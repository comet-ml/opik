...
original_stream_iter_method = together.Stream.__iter__
original_async_stream_aiter_method = together.AsyncStream.__aiter__
...
def patch_sync_stream(
    ...
    """Patch a ``together.Stream`` so iterating over it ends the Opik span.
    ...
    def Stream__iter__decorator(dunder_iter_func: Callable) -> Callable:
        ...
        def wrapper(self: together.Stream) -> Iterator[StreamItem]:
            try:
                ...
                LOGGER.debug(
                    "Exception raised from together.Stream: %s",
                    str(exception),
                    exc_info=True,
                )
    ...
    together.Stream.__iter__ = Stream__iter__decorator(original_stream_iter_method)
...
def patch_async_stream(
    stream: together.AsyncStream,
    span_to_end: span.SpanData,
    trace_to_end: Optional[trace.TraceData],
    generations_aggregator: Callable[[List[StreamItem]], Optional[AggregatedResult]],
    finally_callback: generator_wrappers.FinishGeneratorCallback,
) -> together.AsyncStream:
    """Patch a ``together.AsyncStream`` so iterating over it ends the Opik span."""

    def AsyncStream__aiter__decorator(dunder_aiter_func: Callable) -> Callable:
        @functools.wraps(dunder_aiter_func)
        async def wrapper(self: together.AsyncStream) -> AsyncIterator[StreamItem]:
            try:
                accumulated_items: List[StreamItem] = []
                error_info: Optional[ErrorInfoDict] = None
                ...
                LOGGER.debug(
                    "Exception raised from together.AsyncStream: %s",
                    str(exception),
                    exc_info=True,
                )
                error_info = error_info_collector.collect(exception)
    ...
    together.AsyncStream.__aiter__ = AsyncStream__aiter__decorator(
