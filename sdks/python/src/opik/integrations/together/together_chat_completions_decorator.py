...
import together
from together import _types as _together_types
from together.types import chat as chat_completion
...
class TogetherChatCompletionsTrackDecorator(base_track_decorator.BaseTrackDecorator):
    """
    An implementation of BaseTrackDecorator for tracking calls of Together's
    ...
    def _end_span_inputs_preprocessor(
        ...
        if result_dict.get("usage") is not None:
            # Together returns an OpenAI-shaped usage payload, so it is parsed with
            # the OpenAI converter. Here "openai" denotes the usage payload format,
            # not the span's provider (which is recorded as "together").
            opik_usage = llm_usage.try_build_opik_usage_or_log_error(
                provider=LLMProvider.OPENAI,
                usage=result_dict["usage"],
                logger=LOGGER,
                error_message="Failed to log token usage from together call",
            )
    ...
    def _streams_handler(  # type: ignore
        ...
        assert generations_aggregator is not None, (
            "Together decorator will always get aggregator function as input"
        )

        if isinstance(output, together.Stream):
        ...
        if isinstance(output, together.AsyncStream):
            span_to_end, trace_to_end = base_track_decorator.pop_end_candidates()
...
def _remove_not_given_sentinel_values(dict_: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        if value is not _together_types.NOT_GIVEN
        and not isinstance(value, _together_types.Omit)
    }
