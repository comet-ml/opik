...
class RecursiveCallbackInjector:
    def __init__(self) -> None:
        self._tracked_agents: weakref.WeakSet[agents.BaseAgent] = weakref.WeakSet()
    ...
    def inject(
        ...
        self._process_agent(root_agent, tracer)

    def _add_callbacks_to_agent(
        self, agent: agents.BaseAgent, tracer: opik_tracer.OpikTracer
    ) -> None:
        callback_fields = {
    ...
    def _process_agent(
        ...
        if agent in self._tracked_agents:
            return

        self._add_callbacks_to_agent(agent, tracer)
        self._process_sub_agents(agent, tracer)
        self._process_tools(agent, tracer)

        self._tracked_agents.add(agent)
    ...
    def _process_sub_agents(
        ...
        for sub_agent in agent.sub_agents:
            try:
                self._process_agent(sub_agent, tracer)
    ...
    def _process_tools(
        ...
        for tool in agent.tools:
            if not isinstance(tool, agent_tool.AgentTool):
                continue
            try:
                self._process_agent(tool.agent, tracer)
            except Exception as e:
                LOGGER.warning(f"Failed to track agent tool: {e}")


_recursive_callback_injector = RecursiveCallbackInjector()
...
def track_adk_agent_recursive(
    ...
    _recursive_callback_injector.inject(root_agent, tracer)
