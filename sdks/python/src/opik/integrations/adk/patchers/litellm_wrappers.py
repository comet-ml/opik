...
def generate_content_response_decorator(func: Callable) -> Callable:
    ...
    def wrapper(*args: Any, **kwargs: Any) -> adk_models.LlmResponse:
        ...
        LOGGER.debug("generate_content_response_decorator: provider_and_model=%s", provider_and_model)

        if provider_and_model is None:
            return result

        if result.custom_metadata is None:
            LOGGER.debug("generate_content_response_decorator: result.custom_metadata is None, creating new custom metadata")
            result.custom_metadata = {}

        provider, model = parse_provider_and_model(provider_and_model)
        LOGGER.debug("generate_content_response_decorator: provider=%s, model=%s", provider, model)
        ...
        LOGGER.debug("generate_content_response_decorator finished: result.custom_metadata=%s", result.custom_metadata)
        return result
...
def litellm_client_acompletion_decorator(func: Callable) -> Callable:
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> "litellm.types.utils.ModelResponse":
        ...
        LOGGER.debug("litellm_client_acompletion_decorator called: provider_and_model=%s", result.provider_and_model)
        return result
