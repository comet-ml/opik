...
_original_next = litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper.__next__
_original_anext = litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper.__anext__
...
def _initialize_stream_tracking(stream: Any, accumulator_attr: str, error_attr: str) -> None:
    if not hasattr(stream, accumulator_attr):
        setattr(stream, accumulator_attr, [])
        setattr(stream, error_attr, None)


def _cleanup_stream_tracking(stream: Any, tracking_marker_attr: str, accumulator_attr: str, error_attr: str) -> None:
...
def _create_sync_next_wrapper(
    ...
    def wrapper(self: litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper) -> StreamItem:
            ...
            if hasattr(self, accumulator_attr):
                try:
                    _finalize_stream_tracking(
                        self, accumulator_attr, error_attr,
                        "span_to_end", "trace_to_end",
                        generations_aggregator, finally_callback
                    )
                finally:
                    _cleanup_stream_tracking(self, tracking_marker, accumulator_attr, error_attr)
            raise
...
def _create_async_next_wrapper(
    ...
    async def wrapper(self: litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper) -> StreamItem:
        ...
        try:
            ...
            if hasattr(self, accumulator_attr):
                try:
                    _finalize_stream_tracking(
                        self, accumulator_attr, error_attr,
                        "span_to_end_async", "trace_to_end_async",
                        generations_aggregator, finally_callback
                    )
                finally:
                    _cleanup_stream_tracking(self, tracking_marker, accumulator_attr, error_attr)
...
def patch_stream(
    ...
    litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper.__next__ = (
        _create_sync_next_wrapper(_original_next, generations_aggregator, finally_callback)
    )
    litellm.litellm_core_utils.streaming_handler.CustomStreamWrapper.__anext__ = (
        _create_async_next_wrapper(_original_anext, generations_aggregator, finally_callback)
    )
