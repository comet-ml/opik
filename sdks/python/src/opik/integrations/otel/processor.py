...
def _resolve_inherited(parent_context: Optional[Context]) -> Optional[InheritedContext]:
    ...
    if parent_span is not None and parent_span is not trace.INVALID_SPAN:
        ...
        if (
            isinstance(parent_trace_id, str)
            and parent_trace_id
    ...
    if isinstance(baggage_trace_id, str) and baggage_trace_id:
        ...
        return InheritedContext(
            ...
            parent_span_id=(
                ...
                if isinstance(baggage_parent_span_id, str)
                else None
            ),
        )
...
class OpikSpanProcessor(SpanProcessor):
    ...
    def on_start(self, span: Span, parent_context: Optional[Context] = None) -> None:
        ...
        try:
            ...
            Exception
        ):  # pragma: no cover - per OTel contract on_start must not throw
            pass
