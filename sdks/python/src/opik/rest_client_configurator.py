...
def _configure_retries(rest_client: rest_api.OpikApi) -> None:
    ...
    for domain_client_name in domain_client_names:
        _decorate_public_instance_methods(getattr(rest_client, domain_client_name), connection_retry)


def _decorate_public_instance_methods(instance: Any, decorator: Callable[[Callable], Callable]) -> None:
