...
to the local ``uvx opik-mcp`` server only when no hosted one is available. Rather
than hardcode which deployments offer one, we probe the configured deployment for
the capability — so any environment that runs (or later gains) a hosted server is
...
def detect_hosted_mcp_server(
    ...
    deployment running the MCP auth server serves valid OAuth metadata there; a
    deployment without it returns a 404 or the frontend's HTML catch-all, in which
    ...
    try:
        ...
        # Network/TLS/timeout failures mean "no reachable hosted server" — fall
        # back to the local server. Anything else (bugs, KeyboardInterrupt) is
        # left to propagate.
        LOGGER.debug(
    ...
    try:
        ...
        # `response.json()` raises (JSONDecodeError is a ValueError) — treat
        # anything that is not OAuth metadata JSON as "no hosted server".
        return None
