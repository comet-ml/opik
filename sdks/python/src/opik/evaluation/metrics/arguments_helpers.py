...
def raise_if_score_arguments_are_missing(
    ...
    if len(missing_required_arguments) > 0:
        raise exceptions.ScoreMethodMissingArguments(
            ...
            f"These keys were not present in the dataset item, you can use the `scoring_key_mapping` parameter "
            f"to map the dataset item keys to the scoring metric parameters. Dataset item keys found: {list(kwargs.keys())}."
        )
