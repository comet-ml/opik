...
def export_all_prompts(
    ...
    """Export all prompts from the workspace."""
    try:
        if debug:
            debug_print("Fetching all prompts...", debug)

        # Get all prompts
        prompts_response = client.rest_client.prompts.get_prompts()
        prompts = prompts_response.content or []

        if debug:
            debug_print(f"Found {len(prompts)} prompts", debug)
        ...
        for prompt in prompts:
            ...
            if name_pattern and not _matches_name_pattern(prompt.name, name_pattern):
                continue
            ...
            try:
                ...
                result = export_single_prompt(
                    client, prompt, prompts_dir, max_results, force, debug, format
                )
                if result > 0:
                    exported_count += 1
                else:
        ...
        if debug:
            debug_print(
                f"Exported {exported_count} prompts, skipped {skipped_count}", debug
            )
        ...
        if debug:
            debug_print(f"Error details: {e}", debug)
        return {"prompts": 0, "prompts_skipped": 0}
