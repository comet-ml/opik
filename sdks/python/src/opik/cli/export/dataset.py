...
def export_all_datasets(
    ...
    """Export all datasets from the workspace."""
    try:
        if debug:
            debug_print("Fetching all datasets...", debug)

        # Get all datasets
        datasets_response = client.rest_client.datasets.find_datasets()
        datasets = datasets_response.content or []

        if debug:
            debug_print(f"Found {len(datasets)} datasets", debug)
        ...
        for dataset in datasets:
            ...
            if name_pattern and not _matches_name_pattern(dataset.name, name_pattern):
                continue
            ...
            try:
                # Create a proper Dataset object from DatasetPublic
        ...
        if debug:
            debug_print(
                f"Exported {exported_count} datasets, skipped {skipped_count}", debug
            )
        ...
        if debug:
            debug_print(f"Error details: {e}", debug)
        return {"datasets": 0, "datasets_skipped": 0}
