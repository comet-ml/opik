...
def export_all_experiments(
    ...
    """Export all experiments from the workspace."""
    try:
        if debug:
            debug_print("Fetching all experiments...", debug)
        ...
        if debug:
            debug_print(f"Found {len(experiments)} experiments", debug)
        ...
        for experiment in experiments:
            ...
            try:
                ...
                console.print(
                    f"[red]Error exporting experiment {experiment.name}: {e}[/red]"
                )
                if debug:
        ...
        if debug:
            debug_print(
                f"Exported {exported_count} experiments, skipped {skipped_count}", debug
            )
        ...
        if debug:
            debug_print(f"Error details: {e}", debug)
        return {"experiments": 0, "experiments_skipped": 0}
